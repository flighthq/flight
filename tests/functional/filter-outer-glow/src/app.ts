// filter-outer-glow — validates OuterGlowFilter on a centered white square.
//
...
// Glow mask: green tint, blurred silhouette. Composite mask first, then source on top.
...
applyOuterGlowFilterToSurface(
  ...
  createOuterGlowFilter({ color: 0x00ff00, blurX: 8, blurY: 8, strength: 2 }),
);
...
export function assertRender(frame: Readonly<Surface>): void {
  ...
  const glowF = createOuterGlowFilter({ color: 0x00ff00, blurX: 8, blurY: 8, strength: 2 });
  if (glowF.color === undefined) {
    throw new Error('[filter-outer-glow] expected outerGlow filter to carry a color property');
  }
  ...
  // Just outside each edge (~2px) the green glow ring must be present and green-dominant.
  // The glow falls off with distance, so sample near the edge where it is brightest.
  const off = 2;
  ...
  for (const [lx, ly] of edges) {
    ...
    const b = got & 255;
    if (!(g > 50 && g > r && g > b)) {
      throw new Error(`[filter-outer-glow] glow ring missing at (${lx},${ly}); expected green, got #${hex(got)}`);
    }
  }
