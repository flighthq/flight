...
// WebGPU parity column for the same dither intent as render.webgl.ts. WebGPU render-state init is
...
registerWebGPURenderEffect(state, 'dither', defaultWebGPUDitherEffectRunner);
...
export function render(root: DisplayObject): void {
  ...
  endWebGPURenderEffectPipeline(state, pipeline, [createDitherEffect({ levels: 4 })]);
