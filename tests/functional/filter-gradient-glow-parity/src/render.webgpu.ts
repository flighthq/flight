// WebGPU backend of the filter-gradient-glow-parity test. Mechanical port of render.webgl.ts to the WebGPU offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyGradientGlowFilterToWebGPU over offscreen targets, then composite the
...
import {
  applyGradientGlowFilterToWebGPU,
...
function renderFrame(state: WebGPURenderState, root: DisplayObject, specs: readonly NativeGradientGlowSpec[]): void {
...
function compositeNative(
  ...
  spec: Readonly<NativeGradientGlowSpec>,
  ...
  applyGradientGlowFilterToWebGPU(state, sourceTarget, destTarget, [scratch0, scratch1, scratch2], spec.filter);
