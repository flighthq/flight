...
// WebGPU parity column for the same sketch intent as render.webgl.ts. WebGPU render-state init is
...
registerWebGPURenderEffect(state, 'sketch', defaultWebGPUSketchEffectRunner);
...
export function render(root: DisplayObject): void {
  ...
  endWebGPURenderEffectPipeline(state, pipeline, [createSketchEffect({ strength: 1 })]);
