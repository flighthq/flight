// WebGPU backend of the filter-convolution-parity test. Mechanical port of render.webgl.ts to the WebGPU offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyConvolutionFilterToWebGPU over offscreen targets, then composite the
...
import {
  applyConvolutionFilterToWebGPU,
...
function renderFrame(state: WebGPURenderState, root: DisplayObject, specs: readonly NativeConvolutionSpec[]): void {
...
function compositeNative(
  ...
  spec: Readonly<NativeConvolutionSpec>,
  ...
  applyConvolutionFilterToWebGPU(state, sourceTarget, destTarget, spec.filter);
