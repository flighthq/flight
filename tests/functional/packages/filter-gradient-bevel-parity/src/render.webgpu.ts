// Wgpu backend of the filter-gradient-bevel-parity test. Mechanical port of render.webgl.ts to the Wgpu offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyGradientBevelFilterToWgpu over offscreen targets, then composite the
...
import {
  applyGradientBevelFilterToWgpu,
...
function renderFrame(state: WgpuRenderState, root: DisplayObject, specs: readonly NativeGradientBevelSpec[]): void {
...
function compositeNative(
  ...
  applyGradientBevelFilterToWgpu(state, sourceTarget, destTarget, [scratch0, scratch1, scratch2], spec.filter);
