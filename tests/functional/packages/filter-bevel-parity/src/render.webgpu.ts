// Wgpu backend of the filter-bevel-parity test. Mechanical port of render.webgl.ts to the Wgpu offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyBevelFilterToWgpu over offscreen targets, then composite the
...
function renderFrame(state: WgpuRenderState, root: DisplayObject, specs: readonly NativeBevelSpec[]): void {
...
function compositeNative(state: WgpuRenderState, spec: Readonly<NativeBevelSpec>, toDestroy: WgpuRenderTarget[]): void {
