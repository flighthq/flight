// Wgpu backend of the filter-gradient-glow-parity test. Mechanical port of render.webgl.ts to the Wgpu offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyGradientGlowFilterToWgpu over offscreen targets, then composite the
...
function renderFrame(state: WgpuRenderState, root: DisplayObject, specs: readonly NativeGradientGlowSpec[]): void {
...
function compositeNative(
  ...
  applyGradientGlowFilterToWgpu(state, sourceTarget, destTarget, [scratch0, scratch1, scratch2], spec.filter);
