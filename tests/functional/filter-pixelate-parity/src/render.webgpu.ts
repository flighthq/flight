// WebGPU backend of the filter-pixelate-parity test. Mechanical port of render.webgl.ts to the WebGPU offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyPixelateFilterToWebGPU over offscreen targets, then composite the
...
function renderFrame(state: WebGPURenderState, root: DisplayObject, specs: readonly NativePixelateSpec[]): void {
...
function compositeNative(
  ...
  applyPixelateFilterToWebGPU(state, sourceTarget, destTarget, { blockSize: spec.blockSize });
