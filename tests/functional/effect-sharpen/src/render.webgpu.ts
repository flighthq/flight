...
// WebGPU parity column for the same sharpen intent as render.webgl.ts. WebGPU render-state init is
...
registerWebGPURenderEffect(state, 'sharpen', defaultWebGPUSharpenEffectRunner);
...
export function render(root: DisplayObject): void {
  ...
  endWebGPURenderEffectPipeline(state, pipeline, [createSharpenEffect({ amount: 1.5 })]);
