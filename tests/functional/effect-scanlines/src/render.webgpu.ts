...
// WebGPU parity column for the same scanlines intent as render.webgl.ts. WebGPU render-state init is
...
registerWebGPURenderEffect(state, 'scanlines', defaultWebGPUScanlinesEffectRunner);
...
export function render(root: DisplayObject): void {
  ...
  endWebGPURenderEffectPipeline(state, pipeline, [createScanlinesEffect({ count: 240, intensity: 0.5 })]);
