// WebGPU backend of the filter-drop-shadow-parity test. Mechanical port of render.webgl.ts to the WebGPU offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyDropShadowFilterToWebGPU over offscreen targets, then composite the
...
function renderFrame(state: WebGPURenderState, root: DisplayObject, specs: readonly NativeDropShadowSpec[]): void {
...
function compositeNative(
  ...
  spec: Readonly<NativeDropShadowSpec>,
  ...
  applyDropShadowFilterToWebGPU(state, sourceTarget, destTarget, [scratch0, scratch1, scratch2], filterParams);
