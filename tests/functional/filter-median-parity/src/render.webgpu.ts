// WebGPU backend of the filter-median-parity test. Mechanical port of render.webgl.ts to the WebGPU offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyMedianFilterToWebGPU over offscreen targets, then composite the
...
function renderFrame(state: WebGPURenderState, root: DisplayObject, specs: readonly NativeMedianSpec[]): void {
...
function compositeNative(
  ...
  spec: Readonly<NativeMedianSpec>,
  ...
  applyMedianFilterToWebGPU(state, sourceTarget, destTarget, { radius: spec.radius });
