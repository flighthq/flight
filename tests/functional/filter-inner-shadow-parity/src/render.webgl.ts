// WebGL backend of the inner-shadow-parity test — the meaningful comparison this test exists for.
//
// Native path: the real WebGL inner shadow is a multi-pass shader (applyInnerShadowFilterToWebGL) run
...
// run the GPU passes, composite the result via drawWebGLRenderTargetResult.
//
// Unlike the blur native path, applyInnerShadowFilterToWebGL takes its scratch as a SINGLE array of three
...
//   2. applyInnerShadowFilterToWebGL(state, source, dest, [s0,s1,s2], filter) — the shader inner shadow.
...
import type { Bitmap, DisplayObject, InnerShadowFilter, WebGLRenderState, WebGLRenderTarget } from '@flighthq/sdk';
import {
...
export function createParityTarget(width: number, height: number, background: number): ParityTarget {
  ...
  return {
    ...
    height,
    scale: pixelRatio,
    // No CSS-filter path on WebGL — the inner shadow is the GPU shader pass below.
...
function compositeNativeInnerShadow(state: WebGLRenderState, spec: Readonly<NativeInnerShadowSpec>): void {
  ...
  // applyInnerShadowFilterToWebGL ping-pongs across THREE scratch targets passed as one array.
  const scratch = [
    createWebGLRenderTarget(state, { width: size, height: size }),
    createWebGLRenderTarget(state, { width: size, height: size }),
    createWebGLRenderTarget(state, { width: size, height: size }),
  ];
  ...
  // The real WebGL inner shadow: invert-tint → blur → offset → clip-to-source-alpha → composite
  // (source + clipped shadow) into destTarget. Same filter config the surface reference used, minus the
  // `type` tag the WebGL entry omits.
  applyInnerShadowFilterToWebGL(state, sourceTarget, destTarget, scratch, {
  ...
  for (const s of scratch) destroyWebGLRenderTarget(state, s);
}
