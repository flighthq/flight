...
//   - WebGL: the native inner shadow is a multi-pass shader (applyInnerShadowFilterToWebGL) into offscreen
//     render targets, composited onto the screen after the scene. `drawNativeInnerShadow(spec)` runs that
//     GPU pass; it is a no-op on DOM/Canvas. WebGL is the meaningful comparison this test exists for.
...
export interface ParityTarget {
  ...
  // Shader backends (WebGL): run the offscreen multi-pass inner shadow and composite it at the native
