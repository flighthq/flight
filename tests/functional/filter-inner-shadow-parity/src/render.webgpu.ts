// WebGPU backend of the filter-inner-shadow-parity test. Mechanical port of render.webgl.ts to the WebGPU offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyInnerShadowFilterToWebGPU over offscreen targets, then composite the
...
import {
  applyInnerShadowFilterToWebGPU,
...
function renderFrame(state: WebGPURenderState, root: DisplayObject, specs: readonly NativeInnerShadowSpec[]): void {
...
function compositeNative(
  ...
  applyInnerShadowFilterToWebGPU(state, sourceTarget, destTarget, [scratch0, scratch1, scratch2], {
