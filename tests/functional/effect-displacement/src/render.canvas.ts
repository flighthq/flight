...
// Canvas parity column. Displacement's per-pixel channel separation + hashed tears have no 2D draw-op path, so
...
registerCanvasRenderEffect(state, 'displacement', defaultCanvasDisplacementEffectRunner);
...
export function render(root: DisplayObject): void {
  ...
  endCanvasRenderEffectPipeline(state, pipeline, [createDisplacementEffect({ intensity: 10, frequency: 14, seed: 2 })]);
}
