...
// WebGPU parity column for the same pixelate intent as render.webgl.ts. WebGPU render-state init is
...
registerWebGPURenderEffect(state, 'pixelate', defaultWebGPUPixelateEffectRunner);
...
export function render(root: DisplayObject): void {
  ...
  endWebGPURenderEffectPipeline(state, pipeline, [createPixelateEffect({ size: 24 })]);
