// WebGPU backend of the filter-color-matrix-parity test. Mechanical port of render.webgl.ts to the WebGPU offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyColorMatrixFilterToWebGPU over offscreen targets, then composite the
...
import {
  applyColorMatrixFilterToWebGPU,
...
function renderFrame(state: WebGPURenderState, root: DisplayObject, specs: readonly NativeColorMatrixSpec[]): void {
...
function compositeNative(
  ...
  spec: Readonly<NativeColorMatrixSpec>,
  ...
  applyColorMatrixFilterToWebGPU(state, sourceTarget, destTarget, { matrix: filter.matrix });
