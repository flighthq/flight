// WebGL backend of the color-matrix-parity test.
//
// Native path: the real WebGL color-matrix is a SINGLE-PASS fragment shader (applyColorMatrixFilterToWebGL)
// run once over an offscreen render target, then composited onto the screen as a positioned quad. This
// mirrors the engine's render-cache flow (packages/render-webgl/src/webglCache.ts): render content into a
// target, run the GPU pass, composite the result via drawWebGLRenderTargetResult.
//
...
//   2. applyColorMatrixFilterToWebGL(state, source, dest, { matrix }) — the single-pass shader. The
...
//      its world×device transform, and drawWebGLRenderTargetResult(state, proxy, dest, identity) to
...
import type { Bitmap, ColorMatrixFilter, DisplayObject, WebGLRenderState, WebGLRenderTarget } from '@flighthq/sdk';
import {
...
function compositeNativeColorMatrix(state: WebGLRenderState, spec: Readonly<NativeColorMatrixSpec>): void {
  ...
  // The real WebGL color-matrix: one fullscreen fragment pass. The filter's `matrix` is the same 20-value
  ...
  applyColorMatrixFilterToWebGL(state, sourceTarget, destTarget, { matrix: filter.matrix });

  // The pass (drawWebGLFullscreenPass) leaves the dest framebuffer bound and a tile-sized viewport — it
