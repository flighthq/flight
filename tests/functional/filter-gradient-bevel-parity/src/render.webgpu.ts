// WebGPU backend of the filter-gradient-bevel-parity test. Mechanical port of render.webgl.ts to the WebGPU offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyGradientBevelFilterToWebGPU over offscreen targets, then composite the
...
import {
  applyGradientBevelFilterToWebGPU,
...
function renderFrame(state: WebGPURenderState, root: DisplayObject, specs: readonly NativeGradientBevelSpec[]): void {
...
function compositeNative(
  ...
  spec: Readonly<NativeGradientBevelSpec>,
  ...
  applyGradientBevelFilterToWebGPU(state, sourceTarget, destTarget, [scratch0, scratch1, scratch2], spec.filter);
