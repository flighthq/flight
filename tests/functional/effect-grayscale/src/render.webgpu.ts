...
import {
  ...
  defaultWebGPUGrayscaleEffectRunner,
...
// WebGPU parity column for the same full-frame grayscale grade as render.webgl.ts: fully desaturates the frame to luminance.
...
registerWebGPURenderEffect(state, 'grayscale', defaultWebGPUGrayscaleEffectRunner);
...
export function render(root: DisplayObject): void {
  ...
  endWebGPURenderEffectPipeline(state, pipeline, [createGrayscaleEffect({ intensity: 1 })]);
