...
## 6. Directional shadow depth pass frame lifecycle — resolved

`beginWgpuFrame(state)` now creates and resets the frame encoder without opening the canvas pass.
`renderWgpuBackground` lazily calls it for ordinary loops, while multi-pass recipes use:

`beginWgpuFrame → drawWgpuSceneShadowMap → renderWgpuBackground → prepare/draw → submit`.

The shadow depth and forward passes therefore share one valid encoder without nesting render passes.
