...
- [host composition model](host-composition-model.md) — before renaming host types/consts, adding a function that consumes a host capability, or creating a new host leaf/group const. The `Host*Provider` type, `webHost*` const, and `hostImage` parameter naming conventions.
