...
- [entity construction model](entity-construction-model.md) — **ratified 2026-09-07.** before adding a `create*` factory or changing how a hierarchical entity is constructed. The allocate-initialize discipline: one allocation, base-to-derived initialization, strict per-layer field ownership. Enforced by `npm run construction:check`.
- [host composition model](host-composition-model.md) — **ratified 2026-09-16.** before renaming host types/consts, adding a function that consumes a host capability, creating a new host group or slot, or decomposing a capability. The `Host*Capability` type naming, three-level host structure, flat group inventory, window decomposition, graphics stack layering, and slot boundary tests.
