...
**Status: RESOLVED.** The fix (element screenshot replacing page screenshot in the DOM branch) unifies the two artifacts. This record is retained as the analysis that motivated the fix and as a reminder of the divergence class.
...
Five DOM scenes requested dimensions smaller than the hardcoded 800x600 capture viewport. The `page.screenshot()` captured the full viewport, producing screenshots with white gaps around the scene's container div. The element screenshot for the fingerprint was clipped to the container and clean.
...
- `captureEntry.ts` — the DOM screenshot branch (now element screenshot).
