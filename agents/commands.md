...
- `npm run reachability:check` checks that every declared default runner/renderer and thin per-kind registrar is reachable through its package root. Intentional backend internals use an exact, reasoned `ALLOW` entry; `reachability:json` exposes the audit for tooling.
...
- Run `npm run reachability:check` after adding a default runner/renderer or a per-kind registrar so a composition point cannot exist only in source or the sibling contract lane. A genuinely internal backend seam requires an exact, reasoned `ALLOW` entry.
