...
Accepting "X is clean" closes investigation and cancels any peer still working the same axis — so a false clean erases the finding it claims is absent. The `MediaSessionBackend` lifecycle audit showed the cost concretely: the initial clean claim passed the structural gate (setter reaches `destroy`, rejection-axis tests present), but reading the source revealed that `destroyMediaSessionBackend` cleared both the `_custom` and `_host` slots and invoked `releaseMediaSessionBackends` on the pair — so destroying one slot over-released the foreign object in the other, and an aliased host object destroyed through the custom slot left the host slot holding an invalid reference. Minutes of source/test verification disproved the claim and preserved a real foreign/global-state over-release finding that no gate was red for. Had the clean claim been accepted on the strength of passing gates alone, the finding would have been cancelled silently: the investigating peer would have been redirected, and no remaining work item would have named the defect.
