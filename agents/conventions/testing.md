...
- **A guard that lands with the fix or feature it protects must be seen failing against the old behavior.** After the combined change lands, there is no revision containing the new guard and the old subject, so a green run proves only that the two coexist — not that the guard discriminates. Use one temporary revert-and-restore cycle before committing:

  1. Restore the old subject behavior without removing or weakening the new guard.
  2. Run the narrow command that exercises the guard.
  3. Require it to go red **by the guard's own name**, with the actual wrong values in the failure. "Something failed" is not evidence that this guard noticed the subject.
  4. Restore the fix and confirm the working diff contains the intended new behavior again.
  5. Run the same command and require green.

  Never commit the temporary old behavior. This differs operationally from the functional mirror-scene pattern even though the evidence principle is the same: a mirror scene can be run against the already-existing pre-fix renderer before its fix lands; a guard and subject authored together have no such old tree, so the author must temporarily construct that state. Three independent instances converged on this rule in one session: a `-1` tolerance control proved a capture comparison could go red, functional mirror scenes failed by name with their pre-fix pixel values before the renderer repairs, and the `TextFormat` migration temporarily restored its old subject representation so the simultaneously-authored guard failed before the migration was restored and passed. A simultaneously-landed guard that has only ever been green retains the same unproven, inert-gate shape as a verifier that was never shown a bad input.
- **A gate hardening lands with the fixes for what it newly catches.** The inverse of the rule above, at repository scale: a gate that newly reds the tree has not finished, it has shipped a red and deferred the work. The two halves are one change, and separating them means whoever pulls next meets the broken half with no way to diagnose it — a red window is not a neutral cost, because the reason for it lives in an unmerged stack the reader cannot see. Size is not an argument for splitting: a large stack that goes green → green beats a one-commit hardening that goes green → red and waits. Worked case: a `check-documented-commands` hardening found five genuinely dead `npm run …` citations left behind when the Rust port moved to another repository; the fixes existed, attested, in a different agent's clone, and the two landed as one merge. Corollary for the reviewer: **a hardened checker producing new failures is exactly when "found real rot" and "produced false positives" look identical from outside.** Measure the before-state on the unmodified tree, then open every new finding by name — here, that the cited scripts were absent from the manifest and the citing files untouched by the commit.
...
- **A timeout is a hang detector, not a performance budget — so give it headroom, do not tune it to just pass.** The instance above recurred: `capability-arrival.test.ts`'s four-route video test passed at base and timed out at a tip that added one more heavy structural-gate test, always that one test, on a 30,000 ms budget. Adding a test did not make it slower; parallel contention pushed an already-marginal budget over. Raising 30s to whatever just barely passes reproduces the same fragility one load increase later. Set it well clear — four times headroom still catches a genuinely wedged test, which is the only job a timeout has. If a gate's *speed* matters, assert it explicitly; do not encode it in a timeout constant as a side effect.
- **A timeout tells you the test did not finish; it never tells you how long the test needs.** "Timed out in 30,000 ms (observed 33.9 s)" does not establish the test would finish at 45 s — the observed wall time includes the abort and teardown, so the true completion time is unknown. Never bump-and-assume: raise the budget, run it, and **record the real completion time under load**. If it still does not complete with large headroom, it is a hang, not a slow test — a different and more serious problem that a bigger constant would have hidden.
- **A widened return type is a silent regression with no runtime signal, and no test can see it.** Reconciling two shared assemblers during a merge weakened a return from `Host & HasDialogMessage & HasDialogPrompt` to plain `Host`. The runtime object was unchanged, so behaviour was identical and every runtime gate stayed green; what was lost was a *compile-time* guarantee, and the loss would only have surfaced later when someone wrote a consumer needing the trait. No conflict appeared and no test failed — `tsc` was the only thing that caught it. **After any reconciliation of a shared assembler, diff the RETURN TYPE, not only the body**, and audit intersection traits by content rather than by member count.

- **A falsifier nobody watched fail is not a falsifier.** A test passing against fixed code is necessary and not sufficient; what makes it a falsifier is that it **fails against the pre-fix code**. A test authored after the fix can pass forever while asserting something the defect would never have violated — the instrument answers a different question than its reader believes. Demonstrate red-before/green-after and **record it**, so the evidence lives in the ledger rather than in the author's memory. And count the arms: where a defect fails through two distinct mechanisms, one test covering both is one falsifier short, and the missing arm is invisible precisely because the surviving test is green.
- **Two ways to prove red-before, and what unites them is that the specimen is real.** Either *transplant* the landed tests onto the detached actual pre-fix tree in old-field-compatible form, or *mutate* — revert the actual fix at one point and confirm the matching test goes red. Mutation isolates a single arm more precisely and is the better choice when a defect has several; transplant proves the whole set at once. What is banned in both is a **synthetic reconstruction** of what the pre-fix code was believed to do: it is built from the same assumptions the fix was written under and will agree with them. When transplanting, count intended against unintended failures. Do not reconstruct what the pre-fix code was believed to do — a reconstruction is built from the same assumptions the fix was written under and will agree with them. Check out the real pre-fix commit, port the new tests back in old-field-compatible form, and run. **The passes are as load-bearing as the failures:** four intended failures with everything else green is evidence the transplant perturbed only the arms under test, where a dozen failures would say the harness changed rather than that the defects were real. Record the specific pre-fix symptom per arm — "zero `blendEquation`/`blendFunc` calls on the derived realization" — not merely that it failed.

- **Measure the marginal test on BOTH sides of the change, or you will blame an innocent commit.** In the instance above the reporter proposed a contention mechanism and then correctly withdrew it after reading its own added test and finding four trivial assertions. What remained was two passes at base, two timeouts at the tip, and no mechanism connecting them — an *unexplained correlation*, not a causal one. On a small sample with a trivial diff, the likelier story is that the test was always marginal and base passing was luck. Measuring only the tip records "this change broke a test" when the honest finding may be that a long-marginal test finally fell the wrong way. Take the completion time at base under the same load, and let the three outcomes separate: both marginal (budget was mis-set from the start, change exonerated), tip-only (a real effect with a still-unidentified mechanism, worth knowing before a bigger constant hides it), or neither completing (a hang). Do not escalate this into a multi-run flakiness study — the budget fix is correct under all three; the both-sides reading is about attributing the finding honestly, not establishing a rate.
