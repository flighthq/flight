...
| Per-**bitmap** `smoothing` flag | ✓ | ✓ | ✓ | ✓ | gl + wgpu honor it via the sprite-batch key (per-bitmap sampler) — see gap #3 |
...
3. **Per-bitmap `smoothing` honored on gl + wgpu.** Both bitmap renderers thread `Bitmap.smoothing` into the sprite-batch key (`prepare*SpriteBatchWrite`'s `smoothing` arg), so a NEAREST bitmap and a LINEAR one flush separately and the flush samples each with its own filter — a distinction the center-only `bitmap-smoothing` never gated. **gl**: `bindGlImageResourceTexture`'s `smoothingOverride` → `applyGlSamplerState` re-applies the min/mag filter every bind (no cache-stick). **wgpu**: the sampler is baked into the group(1) bind group, so `resolveWgpuSmoothingBindGroup` returns a per-smoothing bind-group variant (linear/nearest sampler over the entry's view, cached on the entry, dropped on re-upload). Both fall back to the global `state.allowSmoothing` for a null key (sprites/text/shapes). Proof: `bitmap-perbitmap-smoothing.{webgl,webgpu}.ts` sample the magnified cell boundary — crisp (nearest) pure vs smooth (bilinear) gray. (`bitmap-downscale-smoothing` stays canvas/dom: mip-less GPU bilinear can't box-filter a minified checker, so downscale isn't the GPU vehicle — magnification is.)
