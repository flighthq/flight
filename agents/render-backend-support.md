...
2. **~~Closed stroke rings rasterized on gl + wgpu.~~ RESOLVED.** Solid `lineStyle` spans now remain authored centerlines plus style through `@flighthq/shape` `getShapeStrokeRegions`; both GPU backends send them to the dedicated `@flighthq/path` `tessellateStrokePath` kernel. It shares flattening, dashing, offsets, joins, and caps with `strokePath`, but emits non-overlapping strip triangles directly, so open strokes and hollow closed rings are resolution-independent without making generic `tessellatePath` hole-aware. Self-intersecting or otherwise pathological geometry returns a deliberate `null` sentinel; `explainStrokePathTessellation` supplies a shakeable reason query, and GL/WGPU treat the sentinel as a cached, repeatable Canvas-raster fallback rather than a dropped draw. Gradient/texture strokes also remain raster-backed. `shape-stroke-joints`, `shape-stroke-caps`, `shape-fill-stroke-combined`, and `shape-stroke-ring-fallback` provide GPU coverage; the last scene proves both a hollow closed ring and the pathological fallback across Canvas/WebGL/WebGPU.
