...
| Stroke joins (miter/bevel/round) | ✓ | ✓ | ✓ | ✓ | gl + wgpu tessellate open-stroke outlines, including shapes that also fill; closed rings still raster — see gap #2 |
...
2. **Open strokes tessellate on gl + wgpu; closed (ring) strokes still raster.** Solid `lineStyle` spans resolve to fillable outlines via `@flighthq/shape` `getShapeStrokeRegions` → `@flighthq/path` `strokePath` (real miter/bevel/round joins, butt/round/square caps, dashing), and both `@flighthq/scene2d-gl` `drawGlShape` and `@flighthq/scene2d-wgpu` `drawWgpuShape` compose those outlines after any solid fill regions. The result is resolution-independent with no offscreen raster for either stroke-only shapes or combined solid-fill + open-stroke shapes. `shape-stroke-joints`, `shape-stroke-caps`, and `shape-fill-stroke-combined` carry GPU proofs (the combined scene has identical Canvas/WebGL/WebGPU fingerprints). Closed strokes still take the Canvas-2D raster fallback: a self-closing rectangle/ellipse/circle/round-rect primitive, or a return-to-start polyline, offsets into a hollow ring the direct-fill tessellator cannot hole-subtract (`getShapeStrokeRegions` detects closure geometrically and returns null). Gradient/texture styles also stay on the raster path. Ring tessellation requires a hole-aware stroke-strip or equivalent route. (Earlier the GL path had no join handling and a symmetric-corner miter collapsed onto the centerline vertex producing an untessellatable self-intersecting outline — fixed in `strokePath` by intersecting the offset lines along the segment tangent rather than the normal.)
