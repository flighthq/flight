...
| BitmapText | ✓ | ✗ | ✓ | ✓ | `defaultCanvasBitmapTextRenderer` / `defaultGlBitmapTextRenderer` / `defaultWgpuBitmapTextRenderer`. **DOM is an open gap, not a decision** — `scene2d-dom` has no BitmapText renderer of any kind. Filling it needs a scoped proposal for the DOM glyph path first (see below); do not implement one off a bug report. |
...
DOM support is measured by renderer identity, not by matching another backend's leaf count: **the browser is the renderer**, and one scene node maps to one native element. `scene2d-dom` therefore carries `Sprite`, `NativeText`, `RichText`, `TextLabel`, `Shape`, and `Scale9Shape`, plus the browser-specific `HtmlView` and `TextInput` paths. That is the intended leaf set. Batch kinds (`QuadBatch`, `Tilemap`, and `ParticleEmitter2D`) are declared DOM gaps, not a roadmap for bespoke DOM batch renderers. A general canvas-island renderer assembly is demoted to "probably never," not planned work.

`BitmapText` is deliberately **not** in that list any more, and the distinction is the point. Grouping it with the batch kinds read as a decision — "DOM does not do batches" — and closed the question. It is an unfilled gap: DOM already carries three text leaves (`NativeText`, `RichText`, `TextLabel`), so text on DOM is squarely in the intended leaf set, and a bitmap-font string is not a batch in the sense `Tilemap` is. What actually blocks it is a design call nobody has made — whether a DOM glyph path is one positioned element per glyph, a single sprite-sheet-backed element per run, or a CSS `@font-face` conversion — each with different cost, selectability, and text-node identity. That call needs a scoped proposal before any implementation; it is not work to pick up from a bug report.
