...
Everything else missing on canvas is unwritten rather than blocked, and splits two ways. **Moderate:** `BevelEffect`, `GradientBevelEffect`, `GradientGlowEffect` — blur plus offset plus composite, with `createLinearGradient` for the ramp. **Only via `getImageData` per-pixel JS, correct but slow:** `BarrelDistortionEffect` and `PanniniProjectionEffect` (radial per-pixel remap).
