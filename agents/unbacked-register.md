...
| L3 | Six `conformance/*-formats` suites (`awd2`, `gltf`, `md2`, `md5`, `obj`, `threeDs`) each throw a `beforeAll` hook timeout at 10000ms, zero assertion failures, in the full resource-loader test run — **timeout-class, not a correctness defect as reported.** Builder3's Stage 2 commit was suspected first; the control clears it. | builder3 ran the affected suite on the identical tree with `5973f3df6` reverted, **twice**: both runs produced the same 6 timeouts, 21 files / 105 tests passed, 9 skipped, 0 assertions — survives removal of the suspected change. Integration independently reproduced a green→red→green flip on an *unrelated* suite (`conformance/swf`) on a fully unmodified tree within 20 minutes, same signature (hook timeout, no assertion failure) — evidence the whole class is load/contention-sensitive in this environment, not evidence about these six specifically. **Decisive discriminator:** on the CURRENT tree, `5973f3df6` present and unreverted, the identical suite run with only `--hookTimeout=60000` (default is 10000) passed **27/27 files, 114/114 tests, 0 failures.** Setup latency exceeding the default hook timeout under load, not an assertion-level defect and not caused by Stage 2 either way. | **FIXABLE — cause identified (undersized default hook timeout for this suite's current setup cost), delivery not yet decided.** Not builder3's or this commission's to fix; whoever owns `conformance/*-formats` test infra should raise the hook timeout (or reduce setup cost) for these six suites. |
