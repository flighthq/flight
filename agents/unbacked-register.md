...
| L3 | Six `conformance/*-formats` suites (`awd2`, `gltf`, `md2`, `md5`, `obj`, `threeDs`) — **the entire scene3d format-importer family and nothing else** — each throw a `beforeAll` hook timeout at 10000ms, zero assertion failures, in the full resource-loader test run. **Timeout-class, not a correctness defect as reported.** Builder3's Stage 2 commit was suspected first; the control clears it. | builder3 ran the affected suite on the identical tree with `5973f3df6` reverted, **twice**: both runs produced the same 6 timeouts, 21 files / 105 tests passed, 9 skipped, 0 assertions — survives removal of the suspected change. On the CURRENT tree, `5973f3df6` present and unreverted, the identical suite run with only `--hookTimeout=60000` (default 10000) passed **27/27 files, 114/114 tests, 0 failures** — setup latency exceeding the default hook timeout, not an assertion-level defect, and Stage 2 is cleared either way this is sliced. Integration independently reproduced a green→red→green flip on an *unrelated* suite (`conformance/swf`) on a fully unmodified tree within 20 minutes, same signature (hook timeout, no assertion failure) — evidence of environment-dependence, not evidence about these six specifically. **`environment-dependent, cause undiagnosed` is the accurate label — not `load-sensitive`: nobody varied load and watched a threshold move, and a mechanism-word survives in a reader's head after the surrounding caveat gets skimmed.** | **FIXABLE — cause undiagnosed, delivery not yet decided.** Not builder3's or this commission's to fix. **Unconfirmed hypothesis, not a finding** (not checked against today's tree, no `beforeAll` visible from the checker's own base): this repo fetches corpus assets on demand by design and vendors none — a fetch inside a shared `beforeAll` would time out at 10000ms deterministically on a cold cache and pass invisibly on a warm one or with no corpus configured, which fits both the family-confined shape and the cross-machine disagreement better than generic load does. Three discriminators for whoever picks this up, cheapest first: (1) raise the timeout — passes → timing, still fails → a hang and 10000ms is a red herring; (2) inspect what the six share in `beforeAll` — one family, one hook, likely one fixture path; (3) check whether that shared setup fetches anything. |
