...
| 14 | `Host.protocol` traits | `ProtocolHandler` owns an exact open-provider unsubscribe via `detachProtocolHandler`/`disposeProtocolHandler`. Registration/default operations take an explicit Host witness on every call; core retains no selected or replacement provider. Registered/default schemes are durable OS configuration, not a live core-owned handle. | **Owned by the explicit caller/provider.** A caller that later unregisters/removes-default passes the intended Host again; core cannot reroute work through a successor because no ambient successor exists. Event-subscription cleanup is origin-pinned. |
...
notification, and status-bar cleanup all exist by name yet lack enough provider
...
| 8 | Permissions facade/projector | Explicit-Host query/request delegates to capability owners. Notification and MIDI queries are owned solely by their capability Host traits; MIDI acquisition exists only at `Host.midi.access` and generic request is `no-request-route`. Media-track and wake-lock prompt acquisitions are bounded by attempt-all cleanup in `finally`, with cleanup failure reported separately from denial. The six-row interim native-holdings ledger names every future claiming domain. No replacement backend, subscription, owner map, or slot map exists. |
...
Six interim native holdings remain visible in a structural ledger: media, geolocation, persistence, wake lock, clipboard, and push. MIDI has drained to the split `Host.midi.permission` query and `Host.midi.access` acquisition owner. Media-track and wake-lock acquisitions are bounded calls whose releases run attempt-all in `finally`; a post-grant cleanup failure is an operational failure rather than denial. Each row names the domain that must claim it and drains in the same slice. No generic subscription, replacement backend, owner map, or slot map exists to create an unearned lifecycle.
