...
### `displayObject.scrollRect` — the clip-and-offset property

- **What an agent looks for:** a `displayObject.scrollRect = new Rectangle(...)` property that both clips a node to a window and scrolls its content by that window's origin — the Flash/AS3 staple, reached for when porting content or building a scrolling viewport.
- **Will not build:** the `scrollRect` property. It bundles two independent things — a hard geometric clip and a content offset — into one assignment, and it is strictly less capable than the clip Flight already has: `scrollRect` is axis-aligned-rectangle-only, while a `ClipRegion` is a rectangle *or* arbitrary contour geometry. Adding it would mean a second, weaker way to say what clipping already says.
- **Do instead:** set the window with a `ClipRegion` — `setNode2DClip(node, createClipRegionFromRectangle(rect))` (rectangle form, realized as a scissor) or `createClipRegionFromContours`/`FromPath` for a non-rectangular window — and scroll by moving the content, which is an ordinary transform on the child you were going to offset anyway. The clip applies to the node and its subtree, which is the "minus the scroll" half of `scrollRect` (see `Scene2DClipHooks`). Two explicit primitives, each usable without the other.
- **On SWF import:** `scrollRect` is an AS3 *runtime* property with no tag in the SWF format, so nothing is lost at import — there is nothing to read.
