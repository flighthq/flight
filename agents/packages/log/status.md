...
Every item below was re-checked against `packages/log/src/` and `packages/types/src/` on 2026-08-08.

- **All 62 exports live in one `log.ts` (792 lines).** Sinks, level gates, formatters, combinators,
  rate limiting, timers, groups, spans, redaction, serializers, and the transport backend are one
...
- **`disposeFileLogSink` ignores its handle** (`log.ts:299-304`): the parameter is `_handle` and the
  body flushes and disposes the process-global `_transportBackend`. Any file sink disposes the
  transport for every other one. Consistent with the one-transport-per-process design, but the
  handle-shaped signature says otherwise.
- **`enableLogSignals` is one-way and changes the emit gate for the process.** There is no
  ...
  test in the file.
- **Eleven module-level mutable values** (`log.ts:659-670`): `_channelLevels`, `_onceKeys`,
  `_redactionPaths`, `_serializers`, `_sinks`, `_spanStack`, `_consoleLevel`, `_groupDepth`,
  `_level`, `_logSignals`, `_transportBackend`. All but `_onceKeys` and `_logSignals` have an
...
- **No allocation benchmark.** The suppressed-`logVerbose` fast path gates before constructing
  ...
  in `packages/`.
- **No `createHttpLogSink`.** The `LogTransportBackend` seam plus `createBufferedLogSink` is the
...
- **2026-06-25** — Rewrote the stale `createWebLogTransportBackend` docstring, which still described
  a deferred batched-POST HTTP sink instead of the shipped no-op web transport.
