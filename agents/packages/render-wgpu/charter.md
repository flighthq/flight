...
1. **Subject-agnostic GPU plumbing, nothing more.** This package is named by its _technology_ (WebGPU), not its subject. It should compose with any subject's leaf renderer without ever importing a subject package. A runtime dependency on `scene2d` (or any node/graph package) is a layering inversion: the leaves depend on the core, never the reverse.
...
5. **The conformance/readback path is first-class.** Frame capture into a `@flighthq/bitmap` buffer is the screenshot/conformance instrument, and the Rust `flighthq-render-wgpu` crate is the parity target. Surfaces, formats, and the captured pixel layout should map cleanly across the TS↔Rust seam.
...
7. **Should `@flighthq/scene2d` move to `devDependencies`?** It appears only in _test_ files yet sits in `dependencies` — a layering inversion for a subject-agnostic core. Confirm the dependency is genuinely test-only and demote it (a candidate `packages:check`/manifest fix).
