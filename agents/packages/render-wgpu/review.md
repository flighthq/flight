...
- **Separable/destination-read blend modes are predicate-only.** Difference, Hardlight, Invert, Overlay are `null` in `BLEND_MODES` and silently fall back to Normal; only the _detection_ (`requiresWgpuBlendReadback`) ships. No WGSL read-back pass exists. This is a real feature-completeness gap vs. the full blend-mode target and vs. Canvas's `globalCompositeOperation`.
