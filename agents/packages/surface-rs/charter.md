...
> **Spun out (2026-07-10); rename pending.** `@flighthq/surface-rs` and its `flighthq-surface-wasm` crate are **built and maintained in the separate `flight-rs` repository**, not this monorepo — `packages/surface-rs/` and `crates/` no longer exist here. They will become `@flighthq/bitmap-rs` / `flighthq-bitmap-wasm` in a separately coordinated `flight-rs` change; this monorepo changes no build wiring. This cell is kept for reference (the design/north-star below still describes the package's contract), but it is **not local build work**: do not scaffold it here, and it is excluded from the TODO's chartered-unbuilt list via the `spunOut` front-matter marker. The same applies to any future Rust/wasm `-rs` acceleration siblings — they live in `flight-rs`. The review and status logs below predate the spin-out and describe its in-repo history.

## What it is

Wasm-backed drop-in for `@flighthq/bitmap` — a binding/acceleration layer, not a standalone image-processing library. Re-exports the entire `@flighthq/bitmap` API and selectively shadows bulk per-pixel operations with Rust/wasm implementations compiled from the currently named `flighthq-surface-wasm` crate. A consumer swaps `import from '@flighthq/bitmap'` for the accelerated package and gets wasm-accelerated pixel operations with zero API change; its `@flighthq/surface-rs` → `@flighthq/bitmap-rs` rename is pending in `flight-rs`.
...
1. **Pure drop-in shadow, never leading.** Signatures are byte-and-shape-identical to the `@flighthq/bitmap` that actually ships. surface-rs never adds, removes, or changes a parameter ahead of upstream. If upstream changes a signature, surface-rs updates in the same merge — not before, not after.
...
5. **Side-effect-free, tree-shakable.** The barrel re-exports `@flighthq/bitmap` and shadows only the wasm-backed functions. Unused shadows tree-shake out. No top-level wasm instantiation — the module loads lazily on first call.

## Boundaries

**In scope**

- Wasm bindings for bulk per-pixel bitmap operations where Rust provides meaningful speedup: blur, convolution, morphological, color-matrix, composite, geometric transforms, fill/generate, channel/format ops.
...
- Wasm-only capabilities that `@flighthq/bitmap` does not expose. surface-rs is a shadow, not a superset.
...
- Quality/testing utilities (`compareBitmap`, `getBitmapMismatch`, `createBitmapFingerprint`) — these are not hot-path; JS fallback is acceptable unless profiling shows otherwise.
...
- **2026-07-03 — Pure shadow posture confirmed.** surface-rs tracks shipped `@flighthq/bitmap` signatures, never leads. No wasm-only capabilities.
...
- **2026-07-03 — Hidden-state removal is upstream-first.** `floodFillBitmap`, `scrollBitmap`, and `medianBitmap` carry module-level mutable buffers in `@flighthq/bitmap`. The fix (caller-provided scratch) must originate in `@flighthq/bitmap` and only then be mirrored here in the same merge.
