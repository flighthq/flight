...
**Touches:** `displayobject-formats` is built for static SVG; `lottie-formats`/`rive-formats` remain future cells. They consume `shape`/`path`/`animation`/`mesh`/`scene`; the lenient rich-text runs inside these are the second consumer that triggers `markup-tokenizer` extraction (fork-adjacent). **Charters:** `packages/{displayobject-formats,lottie-formats,rive-formats}/charter.md`. The constraint-layer design remains the user's.
