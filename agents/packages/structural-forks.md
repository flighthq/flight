...
- **`svg-formats` — SVG, _to a point_.** Static vector import: paths (path-data already lands via `path-formats`), shapes, fills/strokes/gradients, groups/transforms/`use`, and text, parsed into Flight `shape` commands and a display subtree. **The "to a point" line is the scope:** SVG *as a static vector graphic* is in scope; SVG *as a live document* — SMIL/CSS animation, `<filter>` effect pipelines, scripting, foreignObject — is out (animation belongs to `lottie`/`rive`, effects to `@flighthq/effects`, interactivity to app code). Bedrock check passes: real upstream demand, a well-homed output (shape/display), honest name.
- **`lottie-formats` — Lottie/Bodymovin JSON.** Vector **keyframe animation** (After Effects export): shape layers + transforms + keyframes parsed into Flight `shape` + `@flighthq/animation` tracks. The animated sibling of `svg-formats` (static) — same vector output subject, plus the animation graph.
...
**Touches (when built):** new `svg-formats`/`lottie-formats`/`rive-formats` cells; consumes `shape`/`path`/`animation`/`mesh`/`scene`; the lenient rich-text runs inside these are the second consumer that triggers `markup-tokenizer` extraction (fork-adjacent). **Charters:** `packages/{svg-formats,lottie-formats,rive-formats}/charter.md` (draft). **Not built** — chartered as candidates per the register's recommended set; the plurality is real (three distinct formats), the homes exist, the bless-to-build call and the constraint-layer design are the user's.
