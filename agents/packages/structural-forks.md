...
**The premise.** UI and rich vector content should be **authored in a visual tool** and **imported**, not hand-built from a code-side layout DSL. Visual things are authored visually; the artifacts a designer produces — **SVG**, **Rive** `.riv`, **Lottie** (Bodymovin JSON) — are the interchange. Flight's job is to **import** them into its existing primitives (shape/display graph, animation, mesh), not to grow a competing box-model/flexbox DSL in code. This is the SDK's answer to "where does layout fit": in the design tool, carried by the artifact.

**The shape — each is a `-formats` importer into an existing subject home, never a new runtime:**

- **`scene2d-formats` — SVG, _to a point_.** Static vector import: paths (path-data already lands via `path-formats`), shapes, fills/strokes/gradients, groups/transforms/`use`, and text, parsed into Flight `shape` commands and a display subtree. **The "to a point" line is the scope:** SVG *as a static vector graphic* is in scope; SVG *as a live document* — SMIL/CSS animation, `<filter>` effect pipelines, scripting, foreignObject — is out (animation belongs to `lottie`/`rive`, effects to `@flighthq/effects`, interactivity to app code). Bedrock check passes: real upstream demand, a well-homed output (shape/display), honest name. This supersedes the never-built `svg-formats` reservation by user direction.
- **`lottie-formats` — Lottie/Bodymovin JSON.** Vector **keyframe animation** (After Effects export): shape layers + transforms + keyframes parsed into Flight `shape` + `@flighthq/animation` tracks. The animated sibling of `scene2d-formats`' static SVG importer — same vector output subject, plus the animation graph.
- **`rive-formats` — Rive `.riv`.** The richest: meshes, bones, and **state machines** on top of vector + animation. The parse layer (`.riv` → mesh/animation/scene data) is a `-formats` cell; the **state-machine _runtime_** (interactive inputs driving state) is a *separate* interpretation concern, not part of the format parse — flag it as its own future cell rather than letting the importer absorb a runtime (the same node/sim split as `particles`/`particleemitter`).

**The still-open half — the constraint/anchor layer _(open, NOT greenlit)_.** Imported artifacts are authored at a fixed size; fitting them to a live, resizing viewport needs a **responsive constraint/anchor** layer (anchor points, alignment, fit/scale modes — the "safe-area + pins" model, à la design-tool constraints). This is a genuine design fork, deliberately **left open**: it is the one place a small amount of *layout* logic is warranted, and its shape (a data descriptor over display nodes vs. a solver) is undecided. Distinct from the importers above — those are in scope and chartered; this layer is a direction to settle before building. It is **not** a general code-layout DSL; it is the minimal fit-to-viewport seam an imported visual needs.

**Touches:** `scene2d-formats` is built for static SVG; `lottie-formats`/`rive-formats` remain future cells. They consume `shape`/`path`/`animation`/`mesh`/`scene`; the lenient rich-text runs inside these are the second consumer that triggers `markup-tokenizer` extraction (fork-adjacent). **Charters:** `packages/{scene2d-formats,lottie-formats,rive-formats}/charter.md`. The constraint-layer design remains the user's.
