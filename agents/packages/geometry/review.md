...
- **Rectangle** remains the OpenFL-grade 2D type (contains/intersects/encloses, union, inflate, every edge/corner accessor); **infrastructure** (typed-array `reserve*` helpers, per-type `acquire*`/`release*` pools) is unchanged and clean.
