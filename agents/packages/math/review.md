...
- **Package Map addition** — confirmed: `base/agents/index.md` had zero `@flighthq/math` matches; the bundle `head` adds a full descriptive entry. **This is scope expansion** — flag to the user per the ingest rule (a new Package Map line + a charter authoring pass, both still pending — the charter is a seeded stub).
