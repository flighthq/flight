...
10. **Package Map listing.** `agents/index.md` references this package via the `particles-formats` entry but has no line of its own. Add a Package Map entry mirroring the `particles-formats` one.
