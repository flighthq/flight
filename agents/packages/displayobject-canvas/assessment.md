...
- **Rust-parity posture documentation** — _cross-doc, not this package._ Record in `agents/rust/conformance.md` that `displayobject-canvas` is intentionally TS/host-web only and that `rust:skia ~ ts:canvas` is the reference pairing. No `flighthq-displayobject-canvas` crate is ever built; this is a conformance-map entry owned elsewhere. (roadmap Gold; review.md confirms `crate: null` is correct.)
