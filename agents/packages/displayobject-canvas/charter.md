...
9. **Package Map staleness.** The live `agents/index.md` Package Map has no `displayobject-canvas` entry (nor its `displayobject-<backend>` siblings or `@flighthq/clip`); the bundle's evolved map does. Whether to refresh the live map is the user's doc gate, surfaced here.
