...
4. **`enableCollisionGuards` + `explainCollisionTest2D`** — guard module warning on non-convex polygons and degenerate shapes; a shakeable `explain*` query for `testCollision2D`'s silent-`false` sentinel (segment/point/unknown kinds carry no manifold). Pure diagnostics-inversion work.
...
- **`testCollision2D` union → registry** — fork B trigger fires when phase-3 shapes grow the family; premature now (tight closed loop is the charter-sanctioned exception).
