...
- **Types** (`packages/types/src/Collision.ts`): `CollisionShapeKind2D` open string union, six shape interfaces (`CollisionCircle2D`/`CollisionAabb2D`/`CollisionObb2D`/`CollisionPolygon2D`/`CollisionSegment2D`/`CollisionPoint2D`), discriminated `CollisionShape2D`, and the flat `CollisionManifold2D {overlapping, normalX, normalY, depth}` — exactly the 2026-07-10 decisions.
- **Manifold bracket** — `createCollisionManifold2D` / `clearCollisionManifold2D` (`manifold.ts`); every miss path clears, so a reused `out` never carries a stale normal.
...
- **Generic dispatcher** — `testCollision2D` (`testCollision2D.ts`): kind-ranked canonical ordering collapses the 4×4 matrix to ten branches, negates the normal on swapped input; segment/point/unknown kinds return `false` with a cleared manifold.
- **Queries** — `getCollisionShapeContainsPoint2D` (`pointContainment.ts`, boundary-inclusive, per-kind including on-segment/point-coincidence) and five boolean `testSegment*Collision` queries (`segmentCollision.ts`: Liang–Barsky slab clip for boxes, closest-point for circle, local-frame for OBB, endpoint-inside + edge-crossing for polygon, collinear-aware segment-segment).
...
- **No guard layer** — a non-convex polygon or unknown kind silently yields an undefined/false result; per the diagnostics inversion rule this wants `enableCollisionGuards` + an `explain*` query for the `testCollision2D` sentinel (status.md flags the same).
...
- Registry vs union for `testCollision2D` when phase-3 shapes land (fork B trigger).
