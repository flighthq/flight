...
- Realize new Silver arms in `host-electron`; capability-flags-vs-method-presence canonicity; realize `IpcError`/`IpcErrorCode`; Gold tier (`IpcPort`, `IpcTransferable`, swappable `IpcSerializer`); the Rust `flighthq-ipc` crate; Package Map line refresh in `agents/index.md` — all cross-package, cross-worktree, shared-doc, or gated on an Open-direction decision.
