...
- **`hasUv1` material-time vs. `uv1` geometry-time mismatch.** `hasGlMeshGeometryUv1` now exists but is not yet wired into `glStandardPbrMeshMaterialRenderer.bind()` (geometry is not available at `bind()`). Safe today (an unbound attribute reads zero) but the define key and bound attributes can disagree. Resolve where the geometry signal reaches the material renderer.
