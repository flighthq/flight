...
Strictly within `@flighthq/scene2d-canvas` — no upstream type change, no cross-package coupling, no open design decision. Safe to bless as a set.
...
- **Quad-batch / tilemap per-quad tint / inline node color adjustments** — _recorded capability absence, not a fallback task._ Canvas2D cannot do arbitrary per-vertex color and has no inline color-adjustment registrar. A caller may flatten a cached subtree to a surface and apply an Effect to that surface, but that is a post-flattening group operation with different semantics from element-by-element inherited adjustments; it is not a Canvas inline fallback. (roadmap Gold.)
...
- **Rust-parity posture documentation** — _cross-doc, not this package._ Record in `agents/rust/conformance.md` that `scene2d-canvas` is intentionally TS/host-web only and that `rust:skia ~ ts:canvas` is the reference pairing. No `flighthq-scene2d-canvas` crate is ever built; this is a conformance-map entry owned elsewhere. (roadmap Gold; review.md confirms `crate: null` is correct.)
- **Package-level supported-feature doc + Package Map entry** — _outside the package._ The roadmap's Gold "Documentation" item (supported vs. unsupported blend/scale modes, mask/filter/cache flows) and the review's catch that the **live** `agents/index.md` Package Map has no `scene2d-canvas` / `scene2d-<backend>` / `@flighthq/clip` entries are doc edits the user gates, not within-package code. (review.md Candidate doc revisions; roadmap Gold.)
