...
# Status: @flighthq/scene2d-canvas
...
File: `packages/scene2d-canvas/src/canvasRegistration.ts`
...
File: `packages/scene2d-canvas/src/canvasMaterials.ts`
...
File: `packages/scene2d-canvas/src/canvasRichText.ts`
...
File: `packages/scene2d-canvas/src/canvasRenderTarget.ts`
...
File: `packages/scene2d-canvas/src/canvasCache.ts`
...
Files: `packages/scene2d-canvas/src/canvasShapeCommands.ts`, `packages/scene2d-canvas/src/canvasShape.ts`; type surface: `packages/types/src/CanvasShapeDrawState.ts`
...
File: `packages/scene2d-canvas/src/canvasShape.ts`
...
- `npm run test --workspace=packages/scene2d-canvas`: 217/217 pass
- `npm run exports:check`: no `scene2d-canvas` gaps (pre-existing gaps in `render-gl` and `particles-formats` are unrelated)
...
`scene2d-gl` already has `registerGlDisplayObjectRenderers` + `glDisplayObjectRendererEntries` (found in `glDisplayObjectRegistration.ts`). This was incorrectly listed as deferred in the first pass. No action needed.
...
`scene2d-canvas` is intentionally TS/host-web-only. The Canvas2D substrate does not exist in the Rust box; software-render parity is provided by `scene2d-skia`. The conformance map (`agents/rust/conformance.md`) should record: `scene2d-canvas` has no Rust crate; `rust:skia ~ ts:canvas` is the reference pairing; the canvas functional scenes serve as the TS side of that cross-impl comparison. Documentation decision for whoever next owns the Rust conformance map.
