...
`@flighthq/clip` entry added to `agents/index.md` Package Map between `@flighthq/filters-gl` (which describes per-backend GPU leaf shaders) and `@flighthq/filters`, listing constructors, composition, queries, transform, utilities, and pool bracket.
