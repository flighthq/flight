...
1. **~~Build `ApplicationRenderView` as the explicit 95% assembly.~~** — assembly retired 2026-09-07. `createApplicationRenderView` / `attachApplicationRenderView` / `synchronizeApplicationRenderView` are built and tested; `createGlApplicationRenderView` / `destroyGlApplicationRenderView` exist in `@flighthq/application-gl`. The remaining work is **loop composition and example adoption** — getting from "I have a page" to a main loop + render surface in one step. That entry-point shape remains an open fork (Open direction 5 in the charter).
