...
| interaction | dispatched | 2026-07-02 | 6 decisions blessed: gestures separate package, DisplayObject typing for overlap, `*Handler` suffix intentional, clip-aware picking in scope, shapeFlag stays with honest fallback, broadphase is interaction's opt-in. Prior builder work lost — tiers 1–3 re-queued: registerDefaultHitTests, spatial queries, overlap, HitTestResult + detailed hit, gating, hitArea, suppressTouchHover, shape-accurate picking, tilemap/quad-batch sub-index, clip-aware picking. |
...
| Package | State | Last visited | Note |
| --- | --- | --- | --- |
| adjustments | direction | 2026-07-11 | NEW. The pointwise value-remap tier from fork H ([effect-adjustment-architecture.md](../effect-adjustment-architecture.md)). Built: package + `Adjustment`/`AdjustmentKind` types + color-matrix fuse primitives; color transform folded into the sprite/quad batch as a `HasColorTransform` trait (materials CT kinds removed). Open: re-sort the pointwise effects (colorGrade/hueSaturation/…) out of `effects` into here so a stack fuses to one matrix/LUT (migration Phase 3); LUT baker; per-instance vs per-batch LUT; the `(kind,backend)` realization seam. |
| ~~filters~~ | **dissolved** | 2026-07-11 | Dissolved into `adjustments` (pointwise) + `effects` (spatial/composite) per fork H. All 7 `filters*` packages deleted; composites + blur + convolution + median rehomed to `effects`, color-matrix fuse math to `adjustments`. Prior 2026-07-02 direction superseded. |
| effects | direction | 2026-07-11 | Now the **spatial/composite tier** (fork H): gained the 7 composites (drop-shadow/glow/bevel), plus blur, convolution, median, and its own blur math — all rehomed from the deleted `filters`; `effects-*` no longer import `filters*`. Earlier (2026-07-02): 8 decisions blessed (effects owns intents+math, per-kind handler registration, interpolation via field-role metadata, TS-leads). Open: re-sort the pointwise color-grade ops out to `adjustments`; functional-scene coverage for the rehomed composites/blur/convolution/median (10 gaps, see status); registration migration scope. |
