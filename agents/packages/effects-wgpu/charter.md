...
- **The 8 missing WebGPU effects.** The agnostic layer exports 53 descriptors and this backend implements 45. Missing here: `AutoExposure`, `BarrelDistortion`, `CompositeEffect`, `ContactShadows`, `CustomShaderEffect`, `FilmEmulation`, `PanniniProjection`, and `VolumetricLight`. GL already realizes `CompositeEffect` and `CustomShaderEffect`; the remaining six lack either GPU realization. Are those six in scope for the backends, or agnostic-descriptor-only for now? A descriptor with no backend silently no-ops in a user's chain.
