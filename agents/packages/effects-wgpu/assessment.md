...
1. **The 8 missing effect runners** (`AutoExposure`, `BarrelDistortion`, `ContactShadows`, `CustomShader`, `FilmEmulation`, `PanniniProjection`, `VolumetricLight` — `ColorBlindSimulation` moved to the Adjustment family and is not a missing runner) — review gap 1. _Parked:_ the agnostic `effects` descriptors raced ahead of both backends, so this is a coordinated `effects-gl` + `effects-wgpu` build (lockstep is a stated value), and `scope` is undecided — whether these are backend recipes at all is an Open direction. `CustomShader` is additionally an API-shape fork (a user-supplied-WGSL seam), not a concrete recipe. Acting in `effects-wgpu` alone would break the sibling-parity invariant.
