...
- **Also built:** the envelope check, as `npm run docs:check` (`scripts/docs.ts`, gated inside `npm run check`) rather than the separately-named `docs:packages:check` this section once planned — it validates front matter, file presence, the charter body sections, and the provenance stamp, touching only the envelope and never the prose.
- **Still deferred:** the append-only ledger assertion — the git-diff check that `charter.md › Decisions` and `assessment.md › Approved` are only ever added to. Until it exists, append-only is a convention the skills follow, not a guarantee.

**A derived value is not validated, it is derived.** `status.md`'s `updated:` drifted behind its own entries on 48 of 116 cells because it duplicates what the dated `##` headings already say; the checker now warns on the mismatch, but the reason it stopped mattering is that `todo.mjs` reads the headings. Prefer computing a field the body already carries over adding a rule that asks an agent to keep two copies in step.

**And a self-reported signal measures the reporting, not the thing.** Deriving the status date fixed the arithmetic but not the source: the log records only work somebody wrote down. Ranking re-review by it named 37 cells when 115 had newer source, and put the heaviest rewrites last — `scene3d-gl` (22k lines changed) and `types` (21k) were missing outright, while `scene2d-wgpu` (15k) sat at the bottom as "1 day" of drift. Liveness now ranks re-review on git churn under `packages/<name>/` since the review date, because git cannot be forgotten. The status log stays as continuity prose for the next agent, which is what it is good at; it is no longer asked to be a metric.

**Then count in the unit the thing actually goes stale in.** Git was the right source but lines were the wrong currency: a survey goes stale per _distinct piece of work_ landed on top of it, and one generated-file rewrite is 22k lines it can still describe in a sentence while forty focused commits are forty decisions it never saw. Re-review ranks on commits, with lines kept as the magnitude tiebreak and status entries shown alongside — a row reading `+50 commits · 7.8k lines · +0 status` says both "re-survey this" and "nobody wrote down why".

**A heuristic that is merely correlated will misfile the cases you most care about.** Counting every commit put the whole platform-integration suite on the re-review list at three commits each, none of which was work on those packages — they were repo-wide sweeps. But excluding wide commits outright was equally wrong, and the tidy story that justified it (a bimodal gap, focused work under ~14 packages and sweeps above 23) does not survive the log: `refactor(texture): flatten texture source model` spans 24 packages and is real design work. Breadth correlates with mechanical-ness; it does not determine it. What separates them is _inside_ the commit — the owning package takes hundreds of lines while consumers take a call-site edit apiece — so a wide commit counts only for packages taking a real share of it. Both halves are needed: breadth alone discards the texture refactor, share alone lets the sweeps back in. When a proxy misfiles a case, prefer finding the signal that distinguishes it over tuning the proxy's threshold.
