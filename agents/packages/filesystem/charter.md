...
Host file system access: read/write/append text and binary files, directory CRUD, stat/metadata, recursive traversal and glob, ranged and streaming I/O, atomic write, symlink/permissions seam, disk-usage introspection, pure path utilities, and a file-watch callback — each delegating to a swappable `FileSystemBackend`. A fully implemented OPFS web backend is installed explicitly via `enableHostWebFileSystem()` from `@flighthq/host-web` (custom > host > sentinel); uncovered seams return sentinels (`null`/`[]`/`false`) and `explain*`, never silently substitute web. The only platform-suite package that imports a sibling (`@flighthq/dialog`) — for 4 bridge functions (`read/writeDialogHandle*File`) that consume dialog's `FileDialogHandle`. Six pure path utilities (`joinPath`, `getBaseName`, `getDirName`, `getExtName`, `normalizePath`, `isAbsolutePath`) have zero backend dependency.
