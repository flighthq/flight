...
- **Path editing/simplification** — no `simplifyPath` (Douglas–Peucker), no `fitPathCurves` (polyline→bezier), no `dashPath` exposed independently of `strokePath`. (Region offsetting is intentionally not here — it lives in `@flighthq/path-boolean` as `offsetPath`; path offers only stroke-to-outline expansion, a distinct operation.)
