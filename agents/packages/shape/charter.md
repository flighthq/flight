...
- The distinct `MorphShape` node kind: multiple stable sampled paths, in-place solid/gradient/bitmap fill and stroke sampling in the ordinary Shape command stream, and explicit atomic progress mutation with one content invalidation.
...
- **[2026-08-02] `MorphShape` is its own node kind and retains compound prepared geometry and paint; it does not own a clock.** `createMorphShape` establishes a primary compatibility path binding; repeated `appendMorphShapePath` calls can add independently prepared morphs to the same ordinary Shape command stream, so one node can carry multiple styled regions. Morph paint appenders retain stable command values for solid colors and alpha, gradient stops/matrices/focal ratios, bitmap matrices, and stroke width/color/paint. Gradient endpoint arrays must have equal non-zero topology; incompatible append calls return `false`, with detached reasons from `explainMorphShapeGradientEndpoints`. `setMorphShapeProgress` samples all geometry and paint before one content invalidation. Structural style choices and texture identity remain static. `applyAnimationClipToMorphShape` and `applyMorphShapeAnimationSample` bind the target-free animation substrate to that setter, while callers still own advancement. Canvas, DOM, GL, and WebGPU expose explicit default MorphShape renderer aliases over their existing Shape implementations.
