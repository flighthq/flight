...
4. **Make adjustment color domains explicit in the pipeline.** The current chain applies every fused contrast/HSL/LUT adjustment to whichever values the source target holds, then performs the final linear-to-sRGB encode. A scene-linear grade and a display-referred “look” are not interchangeable: contrast around 0.5, saturation, lift/gamma/gain, and a legacy gamma-space crush produce materially different results. Carry or infer a `scene-linear` versus `display-referred` domain and schedule the output transform between them; do not make callers reproduce a legacy look by corrupting material shaders.
5. **Keep shadows outside the effects lane.** Shadow-map production and light-space sampling belong to lighting/scene rendering; effects may consume shadow-derived buffers only when an explicit later recipe calls for them.
