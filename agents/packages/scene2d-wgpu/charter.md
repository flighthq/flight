...
- **Conformance to the TS spec, then Rust parity.** The TS package is the authoritative shape; `flighthq-scene2d-wgpu` conforms to it. (Order of TS-first vs. co-evolution is an open direction below.)
...
- The shape-command IR and command builders → `scene2d-canvas` (re-exported here, not authored here).
- Canvas2D / DOM rendering → `scene2d-canvas` / `scene2d-dom` (host-web only; no wgpu analogue).
...
5. **Rust-crate parity order.** `flighthq-scene2d-wgpu` is named in the conformance map. Is the intended order TS-Gold-then-Rust, or TS/Rust co-evolution? The charter should state which.
...
7. **Docs-fit revisions surfaced by the review** (admin, not package vision):
   - `render-backend-support.md` says "blend modes … wgpu = none"; the sprite path implements **Normal + Add**. Should read "wgpu = Normal + Add" to match the `scene2d-gl` line.
   - The TS Package Map (`index.md`) has no `scene2d-wgpu` / `scene2d-gl` / `scene2d-canvas` / `scene2d-dom` family entry, so a reader cannot locate where wgpu display rendering lives. Candidate: add the `scene2d-<backend>` family to the TS map.
