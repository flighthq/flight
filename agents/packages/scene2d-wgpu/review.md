...
- **`registerWgpuDisplayObjectRenderers` / `registerWgpuSpriteRenderers`** exist in `wgpuRegistration.ts` (+69) with colocated tests (`wgpuRegistration.test.ts` +115). Both call `registerStandardWgpuMaterial` first, then `registerRenderer` per kind. The display-object set registers 11 kinds + render-cache; the sprite set registers the 4 sprite-graph kinds. JSDoc correctly notes tree-shaking is unchanged.
