...
- ⚠ **WITHDRAWN 2026-08-10 — this ✓ describes behavior the package does not have.** The original read: "`createDirectionalLight` now normalizes `direction` and resets a zero-length input to `(0,-1,0)`". Neither half holds. `createDirectionalLight` (`directionalLight.ts:23-35`) clones the supplied direction or defaults to `(0,-1,0)` when none is given — it does not normalize what it is handed. Normalization lives in `setDirectionalLightDirection` (`directionalLight.ts:39-48`), and on a **zero-length input that function leaves the direction unchanged** rather than resetting it, so nothing anywhere resets to `(0,-1,0)`. The "normalized direction" contract therefore holds only for directions set through the setter, not for one passed to the constructor.
- ⚠ **WITHDRAWN 2026-08-10 — `setSpotLightCone` does not swap-clamp.** It writes both cosines straight through (`spotLight.ts:60-63`); the invariant `innerConeCos >= outerConeCos` holds only when the caller passes `innerDegrees <= outerDegrees`, and the function's own comment says so in terms: "callers are responsible for ordering their inputs." The ✓ credited the package with a guard it does not implement.
