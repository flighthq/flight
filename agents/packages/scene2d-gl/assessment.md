...
- **Own the shape command vocabulary / drop the `@flighthq/scene2d-canvas` runtime dep.** Today `index.ts` re-exports thirteen `gl*` shape commands as aliases of `canvas*`. Can only _fully_ land after native gradient + stroke + bitmap fills exist, and whether dropping the canvas dep is in scope is a **Boundary decision** routed to the charter. — roadmap Bronze (sequencing item 4).
...
- **Boundary — shape command ownership.** Is dropping the `@flighthq/scene2d-canvas` runtime dep (owning `glBeginGradientFill` / `glLineStyle` / …) in scope, or is borrowing Canvas's shape commands an accepted permanent boundary?
