...
ingested:
  ...
  - reviews/maturation/depth/scene2d-gl.md
  - source
...
- **New `glTestHelper.ts`** (`b2824e3d8:packages/scene2d-gl/src/glTestHelper.ts`, `new file mode`) — a package-local test helper exporting one function, `makeGlState`, that builds a render state through render-gl's **public** entry point:
...
- **Import migration in ~22 `*.test.ts` files** — every test file flips `import { makeGlState } from '@flighthq/render-gl'` to `import { makeGlState } from './glTestHelper'`. E.g. `b2824e3d8:packages/scene2d-gl/src/glBitmap.test.ts`:
...
- **One assertion tightened in `glRichText.test.ts`** (`b2824e3d8:packages/scene2d-gl/src/glRichText.test.ts`):
