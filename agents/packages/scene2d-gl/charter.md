...
- **Shape command vocabulary — own it or borrow it permanently?** `index.ts` re-exports thirteen `gl*` shape commands as aliases of `canvas*` from `@flighthq/scene2d-canvas`, which remains a runtime `dependency`. Is owning the native fill/stroke command surface (and dropping the canvas dep) in scope, or is "borrow Canvas's shape commands" an accepted permanent boundary? The dep cannot be dropped until native fill/stroke land.
