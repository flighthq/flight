...
1. **Fix `detectConnectivityReachability` fallback** — when the native backend lacks `detectReachability`, the fallback allocates a fresh web backend per call via `createWebConnectivityBackend()`. Should reuse a cached instance or the already-set backend to avoid unnecessary allocation on every reachability probe.
