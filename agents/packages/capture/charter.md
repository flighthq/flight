...
> **CHARTERED; first build directed 2026-07-09 (standalone policy/format layer).** Blessed during the 2026-07-03 render-verification direction session. Today the capability lives in the tooling layer (`scripts/capture-core.ts`, `scripts/compare-render.ts`, `scripts/baseline-store.ts`) plus the fingerprint math in `@flighthq/bitmap`. The approved near-term work in [assessment.md](./assessment.md) executes against that tooling; the package extraction follows.
...
- **Consumes:** `@flighthq/bitmap` for pixel-level operations (`createBitmapFingerprint` and friends are surface ops on an `ImageSource`; see Open direction 6 for the exact seam).
...
- **[2026-07-09] First-build scope = the pure policy/format layer; surface keeps the pixel math (resolves Open direction 6).** The package owns, as plain importable functions with no Playwright/Node I/O: the baseline record shape + serialize/parse (the `Record<column, { fingerprint?, sha256? }>` format, `format*`/`parse*` + field get/set — no `fs`), the tolerance comparison policy (`compareCaptureFingerprints` = parse two fingerprint strings via surface + `compareBitmapFingerprints`; `evaluateCaptureRegression`/`evaluateCaptureParity` applying a tolerance → a pass/diff result), the default tolerances (regression 5, parity 15), and the tier vocabulary type (`regression` | `parity` | `smoke`). It **depends on `@flighthq/bitmap`** for the pixel math (`createBitmapFingerprint`, `compareBitmapFingerprints`, `parseBitmapFingerprint` stay in surface) and does NOT re-export them (keeps `api:check`'s single-export rule). Playwright driving, dev servers, gallery, CI wiring, and the baseline file I/O stay in the tools.
...
1. **WebGL software rendering (SwiftShader pinning) — partially closed.** The CI screenshot-baseline legs were removed after both pre-registered remedies from 4fdfd2a15 were exhausted: scoping to pinned backends (c7ef40d4e restricted to dom/canvas/webgpu, still red) and pinning WebGPU to SwiftShader (six of ten functional diffs were webgpu-only, proving that even a pinned rasterizer does not make a CI-captured golden hash reproduce across machines). The underlying insight: a golden hash compares one blind run to another — it cannot tell a correct render from a reproducible wrong one. Parity and pixel oracles carry the correctness signal; the golden's job is stability, and a stable reference must be captured where it is checked. `capture:*:check` stays a local command. Whether to additionally pin WebGL to SwiftShader for the parity/smoke legs is still open but is now a narrower question about what those environment-independent tiers observe, not about gating on golden hashes.
...
6. **The seam with `@flighthq/bitmap`.** Fingerprint create/compare/format/parse currently live in surface. Decide whether capture re-exports, wraps, or takes ownership of the comparison-policy layer while surface keeps the pixel math.
