...
- **[2026-07-11] ATF (Adobe Texture Format) — `parseAtf` returns an ARRAY; `TextureContainer` stays single-format.** ATF is a feature-target (it's in the flight-reference Starling demo). A "cross-platform" ATF embeds the SAME texture in several GPU encodings (DXT + PVRTC + ETC, sometimes ASTC) so the runtime picks by GPU support. Blessed shape: **`TextureContainer` is unchanged** (single-format descriptor — no `variants` field; keeps the 90% single-texture path allocation-free and the type uniform across every parser). ATF's multiplicity is expressed by the **return cardinality**: `parseAtf(bytes): TextureContainer[] | null` — the peer encodings as an array (a single-format ATF is a 1-element array). Each element is a standard `TextureContainer`. There is **no "primary"** — the encodings are peers; the consumer picks by GPU at upload time via a `selectTextureContainer(containers, supportedFormats): TextureContainer | null` helper. JPEG-XR/JPEG fallback blocks are **identify-only** (locate the byte range; decoding a JPEG-XR original is a full image decode → `flight-rs`/`image-codec`, out of scope here, same boundary as Basis transcode). `TextureContainerFormat` gains the ATF-required members (PVRTC, ETC1).
- **[2026-08-17] ATF scope stands on format coverage.** The retired external comparison harness is no longer a feature-target authority; the array return and peer-encoding model above remain the blessed container design.
