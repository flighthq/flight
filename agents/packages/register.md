...
| `swf`              | display/movieclip · domain            | ✅ named-graph slice landed — standalone domain package; uncompressed recursive first-frame `DefineSprite` named instances + composed transforms + linkage into `Scene2DDocument`. Later frames, visual tags, and compression remain staged; ABC/execution out of scope |
