...
- `createAnimationChannel` / `createAnimationClip` / `getAnimationClipDuration` — channel = track + opaque `targetRef`; clip duration defaults to the latest keyframe across channels with an explicit override. Deliberately target-free: the binding layer (`applyAnimationClipToScene` in `@flighthq/scene3d`, reading `SceneAnimationTarget { node, path }`) interprets `targetRef`.
