...
- **Package Map line is thin.** `agents/index.md` does not list `@flighthq/mesh` in the main Package Map at all (the 3D family — `mesh`/`scene`/`lighting`/`texture`/`camera` — is described only in the Rust map's 3D-pipeline section). With fork G's _"full 3D is in scope"_ decision (2026-06-24), `mesh` warrants a first-class Package Map entry next to `geometry`/`materials`.
