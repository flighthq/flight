...
- **[2026-07-10] Invalid input degrades, does not throw for expected cases.** An unsupported unit/currency code or malformed locale that `Intl` itself throws on is a programmer error (surfaces as the native throw); but `format*` of `NaN`/`Infinity`/invalid `Date` returns `Intl`'s own sentinel output (`"NaN"`, `"Invalid Date"`) rather than adding a wrapper error.
