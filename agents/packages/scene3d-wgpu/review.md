...
**Types-first discipline** (verified in `changes.patch`): `MaxForwardLights = 8`, the `hemisphere?`/`point?`/`spot?` arrays on `Scene3DLights`, and `hemisphereCount`/`pointCount`/`spotCount` on `SceneLightBlock` were added to `@flighthq/types` first; `packSceneLightBlock` in `@flighthq/render` packs them. The backend implements against the header.
