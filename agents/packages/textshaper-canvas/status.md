...
Per the maturation roadmap, `flighthq-textshaper-canvas` does not exist as a Rust crate (Canvas 2D substrate is not in the box). Should be recorded in `agents/rust/conformance.md` as a TS-only backend with no Rust counterpart. The seam (`TextShaperBackend` / `FontMetrics` / `ShapedRun`) is what the Rust shaper conforms to, not the canvas implementation. Deferred — coordinate with whoever owns the conformance map.
...
6. **Record Rust divergence**: add a line to `agents/rust/conformance.md` noting `textshaper-canvas` is TS/`host-web` only — no `flighthq-textshaper-canvas` crate, by the crate existence rule.
