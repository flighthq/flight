...
1. **`CFF `/`CFF2` charstring outlines.** The stated absence above, and what SWF `DefineFont4` needs specifically — its embedded font is an OpenType CFF. Built from the specification, never from a reference implementation.
