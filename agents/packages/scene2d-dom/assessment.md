...
# scene2d-dom — Assessment

Sorts the gaps from `review.md` (the 89/100 post-work survey) and the prior `reviews/maturation/depth/scene2d-dom.md` roadmap into sweep-safe **Recommended** work and parked **Backlog**. Much of the roadmap's Bronze/Silver tier already landed this pass — accessibility (`enableDomAccessibility`), the blend-fidelity table (`getDomBlendModeFidelity`), CSS-filter-equivalence detection (`hasDomCssFilterEquivalent`), the exact SVG color-matrix path, caret-from-format, `drawDomShape` HiDPI, the `escapeHtmlString → escapeDomHtmlString` rename, and `domDisplayObjectRenderer` symmetry are all verified-present in the review. What remains is a small set of within-package fidelity follow-ups (Recommended) and a larger set of cross-package / design-fork / Gold-tier items (Backlog), with the package's defining boundaries routed to the charter's Open directions.
...
Sweep-safe: within `@flighthq/scene2d-dom`, no cross-package coupling, no breaking change, no open design decision. A blanket "do all recommended" can bless this set safely.

- **~~HiDPI follow-up for `drawDomBitmap`.~~** — retired 2026-08-05. The renamed unified sprite path's `renderSpriteAsCanvas` sizes cropped-source backing canvases by `state.pixelRatio`, constrains their CSS dimensions to logical pixels, scales the context before drawing, and preserves sampler-driven smoothing; `domSprite.test.ts` covers the physical/CSS sizing path.
