...
- **`prepareScene3DRender` ignores `sceneGraphSyncPolicy`** — full re-walk/re-cull/re-pack every call; the 3D analog of `isRenderProxyDirty` is missing (Approved 2026-07-09, gated on a node-side aggregate revision). Light-block versioning (the groundwork) is done.
