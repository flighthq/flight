...
> **[2026-07-31 · principal] HOLD item 1 pending a ruling — read before starting.** A direction session raised that `ApplicationRenderView` may be a rendering primitive with a window welded onto it; removing the weld leaves `renderState + renderTarget + viewport`, which would land in **this package** as a `RenderView` and would dissolve `@flighthq/application-gl`. That is the same seam as item 1 below. The unsettled sub-question — does such a view **own** or **reference** its `RenderState`? — directly determines the sub-target primitive's shape, because many viewports may share one target. Building item 1 before this is settled will likely shape it wrong. Full argument and open questions: [render view model](../../render-view-model.md). Items 2–4 are unaffected and safe to dispatch.
