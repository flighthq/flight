...
0. **Does `RenderView` belong here? — proposal awaiting ruling; blocks the sub-target Directed item.** `ApplicationRenderView` is arguably a rendering primitive with a window welded onto it. Remove the weld and what remains is `renderState + renderTarget + viewport` — no application dependency, so it would land in this package, and `@flighthq/application-gl` would dissolve. The load-bearing sub-question is whether such a view **owns** or **references** its `RenderState`: many viewports may share one target, so ownership recreates one layer down the same 1:1 trap the window weld created. Full argument, evidence, and open questions in [render view model](../../render-view-model.md). **This is the same seam as the Directed item "Make `RenderTarget` + device-pixel `Viewport` the allocation-free sub-target primitive" — settle it before that item is dispatched, or the primitive will likely be shaped wrong.**

1. **Retained queue as the future driver's input.** `RenderQueue` exists (`buildRenderQueue`/`sortRenderQueue`), but the chartered `drawRenderProxy` driver has not landed. When it does, decide whether its first implementation consumes the sorted queue directly or whether a distinct `drawRenderQueue(state, queue)` variant is warranted. Either shape should decouple sort order from graph traversal.
