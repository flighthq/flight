...
6. **`CollisionObb2D` abbreviated fields.** `halfW`/`halfH` — the only abbreviated field names found in sampling; the codebase spells out `halfExtents`/`width` elsewhere.
