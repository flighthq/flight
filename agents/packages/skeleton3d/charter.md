...
- **Explicit deform, no magic:** extract the example's hand-coded interleave/deform into
  ...
  `applyAnimationClipToScene`. (Placing `updateMeshSkin` in `@flighthq/scene3d` was the initial lean, but
...
- **[2026-07-17] `updateMeshSkin` lives in `@flighthq/skeleton3d` (with a `mesh` dep), NOT `@flighthq/scene3d`** — reverses the earlier lean. Placing it in `scene` created a project-reference dependency cycle; `skeleton3d` breaks it and is closer to the deform feature anyway. `Skin = { skeleton, skeletonRoot? }`; bind pose on `MeshGeometryRuntime.skinBindPose`; float32x4 joints0/weights0 encoding; >4 influences → top-4 renormalized; MD5 normals regenerated, glTF normals parsed. User-approved (delivered parcel builder2-631cd71e; re-recorded after a sync dropped the original commit).
- **[2026-07-17] GPU skin palette is computed centrally in `prepareScene3DRender`; the joint-matrix variant is geometry-driven via a render-state `activeSkinnedRun` flag folded into each prelude's program-cache key — so the ~15 material renderers are untouched and all 5 gl families (classic/unlit/toon/pbr/shaded) skin on GPU.** Consequence (footgun to guard later): on a GPU backend the app must NOT also call `updateMeshSkin`, or the mesh is skinned twice (CPU over GPU).
- **[2026-07-17] The "shared skin-emit path" is realized as `packSkinInfluences` in `scene-formats/shared.ts` (top-4-by-weight + renormalize), consumed by ALL three skeletal importers — MD5, glTF, and AWD** (delivered parcel builder-3a5169aa; MD5's duplicate influence logic dropped onto it). AWD parity added: `createScene3DFromAwd` emits `joints0`/`weights0`, parses the skeleton, sets `mesh.skin`, and `parseAwdSkeletonAnimation` is MD5-symmetric (breaking intra-package signature). **Open correctness risk (host-gate only):** AWD joint matrices use the existing local-transform interpretation — static bind pose is correct; animated local-vs-inverse-bind is unverifiable in-sandbox.
