...
- **Refresh the stale `@flighthq/shell` Package Map line** — cross-boundary: the Package Map lives in `agents/index.md` (the codebase map / project CLAUDE.md), not under `agents/packages/shell/`; the assessment also flags it as gated on the user's exact wording.
