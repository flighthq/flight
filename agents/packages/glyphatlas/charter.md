...
- **[2026-07-10] The `GlyphSource` seam is page-aware: `GlyphEntry.page` + `getGlyphAtlasImage(page?)`.** The seam this cell owns now pairs its geometry with its pixels — each `GlyphEntry` carries the atlas `page` its rect samples, and `getGlyphAtlasImage(page = 0): ImageResource | null` returns that page's backing image (null for a non-existent page), so a renderer needs nothing beyond the source. The dynamic atlas is a single growing page: `getGlyphAtlasEntry` stamps `page: 0` and `createGlyphSourceFromGlyphAtlas` returns the atlas bitmap for page 0, null otherwise. This is the seam-side enabler for the multi-page cache (open direction 1's LRU/dirty-region decision already anticipated `page`); populating several page *bitmaps* remains that deepening.
- **[2026-08-01] Rasterizer backends may bind per atlas.** `GlyphAtlasOptions.rasterizerBackend`
  captures one backend for that atlas; omitting it preserves the process-wide default. Parser-produced
  fonts can therefore coexist without mutating global state, and the adapter stays an explicit,
  tree-shakable caller choice. User-directed 2026-08-01.
