...
- **Parse core depends on `@flighthq/types` + `@flighthq/xml`** (TMX/TSX are XML — reuse the sanctioned XML dep, as `spritesheet-formats`/`textureatlas-formats` do); **projection adds `@flighthq/tilemap`** (`createTilemapData`, `Tilemap`). A caller-supplied `inflate` callback decodes gzip/zlib layer data (no bundled zlib). No DOM, no renderer. External tilesets and atlas/layout data resolve via caller-supplied resolvers (the reference-then-resolve pattern of `shape-formats`).
...
- **[2026-09-01] Parse failures are structured `ImportDiagnostic`s, not dropped warnings.** This supersedes one clause of the 2026-07-10 Tiled decision — the "dropped-with-warning if a compressed layer appears without one" wording — and nothing else in it. Both parse paths, `tiledJsonParse` and `tiledXmlParse`, take an optional `diagnostics` array and report through `reportImportDiagnostic` from `@flighthq/importdiagnostics`, with six codes: `tiled.json-malformed`, `tiled.xml-malformed`, `tiled.root-unexpected`, `tiled.required-field-missing`, `tiled.layer-encoding-invalid`, and `tiled.layer-inflate-unavailable`.

  The compressed-layer-without-`inflate` case keeps its shape-preserving behaviour: the layer is still produced as an all-zero grid of the correct dimensions, so a caller's indexing does not shift, and the fact that it is empty because no decompressor was supplied is now reported as `tiled.layer-inflate-unavailable` instead of vanishing into a log line. Sentinel `null` on malformed input is unchanged.

  **Why:** "Dropped with a warning" put the explanation somewhere the caller could not read it programmatically, in a codec whose whole job is to hand back a faithful document. A structured diagnostic makes the same fact queryable, and routing it through the shared `importdiagnostics` seam means a Tiled import reports the way every other `*-formats` importer does rather than inventing a local convention. Landed in `2082b3c7a`.
