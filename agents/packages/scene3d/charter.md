...
> Durable vision and core values for `@flighthq/scene3d`. You author this (via an agent transcribing your direction); it is the rubric `review.md` and `assessment.md` are judged against. No status here — that lives in the review. See ../CONTRACT.md.

## What it is

`@flighthq/scene3d` is the **3D scene graph** — the spatial node hierarchy of "what exists" in a 3D world: a scene root, transform-only group nodes, and renderable mesh leaf nodes, plus the node taxonomy (`Group`/`Mesh`/`InstancedMesh`/`LodMesh`/`Billboard`), the runtime/signal/world-matrix plumbing those nodes need to participate in a render walk, and the **spatial queries** over that graph (traversal, world AABB + bounding sphere, frustum culling, raycasting).
...
2. **Plain data, free functions, explicit allocation.** Nodes are plain entities with a paired opaque runtime (`Node3DRuntime` holding the `worldMatrix` slot); operations are free functions with full unabbreviated names (`cullSceneNodeByFrustum`, `getSceneNodeWorldBoundingSphere`). Queries write into out-arrays / out-params and are alias-safe; `create*`/`clone*` are the only allocators. Hot-path queries (cull, raycast) must not allocate per element — use the geometry constructors and scratch state, never object-literal casts. (See Open direction 8 on raycast-hit allocation policy.)

3. **Types-first, homed in `@flighthq/types`.** Every cross-package shape (`Node3D`, `Mesh`, `Group`, `Billboard`, `InstancedMesh`, `LodMesh`, `Ray3D`, `SceneRaycastHit`, `SceneRaycastOptions`, `Node3DVisitor`) is defined in `types` and re-exported — the header layer is the design surface, not inline package types.
...
2. **Boundaries: `Scene` vs `Group` vs bare `Node3D`.** Three near-identical transform-only node forms exist (`Scene = Node3D`; `Group` a distinct kind). Bless the intended distinction (root identity vs. named container vs. anonymous transform) or collapse it.
3. **Cull/render-walk contract ownership.** The build chose a caller-driven cull list (acyclic: scene builds → render consumes). Is that the blessed seam, or should `prepareScene3DRender` own culling? This is the one cross-package design decision the build made implicitly; it should be ratified.
...
14. **Scene3DLights draw/prepare seam: widen to `Scene3DLightsLike`?** The 78-scene functional-typecheck break (2026-07-30) was fixed by instantiating `createScene3DLights()` at each call site, per an explicit ruling to use the owning create* constructor rather than hand-write `[EntityRuntimeKey]`. An alternative surfaced during that fix: `Scene3DLightsLike` (`EntityWithoutRuntime<Scene3DLights>`) already exists in `@flighthq/types`, is already used in 73 places across `lighting`/`scene3d-gl` tests, and its own doc comment states the intent — callers that already own a plain descriptor should be able to pass it without manufacturing identity solely for one draw call. The draw/prepare entry point that scenes call never widened to accept it. Widening that one signature (instead of the ~40-scene createScene3DLights pattern) would be more idiomatic per AGENTS' "structural literals only for `*Like` inputs" rule, but it is a design decision on the draw seam's accepted type, not a mechanical cleanup — parked here rather than redone against the already-shipped, already-verified fix.
