...
- **Recorded `getEntityRuntime` asserting-vs-Option divergence in `agents/rust/conformance.md`** — intentional TS/Rust seam divergence documented under "Intentional value-type seam divergences". TS keeps the non-null asserting form (paired with `hasEntityRuntime`); Rust uses `Option<&EntityRuntime>`.
- **Recorded `get_entity_runtime_mut` intentional TS omission in `agents/rust/conformance.md`** — Rust exposes a mutable-borrow accessor for borrow-checker ergonomics; TS mutates the slot object directly and has no need for a separate mutable accessor. Recorded so the conformance checker does not report it as a coverage gap.
...
- The divergence is documented in `agents/rust/conformance.md` and does not affect behavior.
