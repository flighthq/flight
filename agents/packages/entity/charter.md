...
- **[2026-09-03] SUPERSEDING: every exported production `create*` returns transitively Entity. There is no collision carve-out, no `build*` rename escape, no baseline, and no second taxonomy.** This supersedes the collision/value clause of the [2026-09-03] doctrine entry and lifts the hold recorded beside it. `EntityRuntime` is the only documented second root; `clone*` remains excluded from the rule rather than exempted by it. User-directed.

  **The rule is enforced, not asserted.** `npm run entity-contracts:check` reports **709 of 807 object-returning create symbols resolving to Entity or EntityRuntime, 0 violations across 0 packages**, over 2921 production files. Scope is tracked and untracked `packages/*/src` production TypeScript, with declarations, tests, and `tool-*` packages excluded by role rather than by a site roster — so the gate cannot be satisfied by editing a list of blessed sites.

  **What "no second taxonomy" means mechanically, and why it holds.** A `create*` may be excluded from the rule only through `NonEntityCreateResult<Type, Kind>`, whose `Kind` is a **closed three-value union**: `'descriptor' | 'options' | 'type-only'`. Those are exactly the Not-Entity categories the explicit dependency model already names. **Query results and intermediates are deliberately not a markable kind**, which is the whole point — that was the carve-out the collision clause leaned on, and it no longer exists as an option. A contact manifold cannot be marked out of the rule; it had to become an Entity, and it did: `createCollisionManifold2D` and `createCollisionRaycastHit2D` both compose `createEntity` today.

  **Why:** An exception list grows until it is the rule, and the earlier clause would have started one with 357 entries. Closing the marker's kind union to the three categories the architecture already blesses means a future author cannot invent a fourth reason to opt out — the type system refuses it — so the doctrine stays enforceable without anyone policing a roster. The 25 exported `create*Runtime` results are the documented second root and are reported by name rather than hidden in a count.

- **[2026-09-04] SUPERSEDING: Entity is decided by shape, not by semantic category. `NonEntityCreateResult`'s kind union no longer buys an exemption.** This supersedes the closed-three-value-union mechanic of the [2026-09-03] entry above. Every object Flight defines and allocates is an Entity. The only things outside are those that structurally cannot be one — arrays and tuples, callables, native/external handles, and primitive results — which the gate already derives automatically rather than from a marking. `descriptor`, `options`, and `type-only` stop being opt-outs. `EntityRuntime` remains the documented second root and `clone*` remains excluded from the rule rather than exempted by it. User-directed.

  **Why closing the union was not enough.** It stopped a fourth reason from being invented, but it left the three in place as judgment calls, and judgment is where drift lives. Whether a type is "just a descriptor" is a property of how callers use it, not of the type: the `adjustments/*` descriptors are change-time only because `enableColorAdjustments` fuses the stack into `resolvedColorScaleBias` on set, and moving that cache would put the identical types in the per-frame path with nothing at the declaration to say so. Shape, unlike usage, cannot be invalidated by an unrelated change elsewhere — so it is a rule an author can apply while reading one file.

  **The consequence the marker was hiding.** In the Haxe/C++ port an Entity becomes a statically typed direct reference and everything else becomes dynamic field access. `NonEntityCreateResult` therefore chose a field-access representation while appearing to annotate semantics only, letting a documentation-level decision set performance — and it had already done so on some of the hottest reads in the SDK. `SpriteData` is marked `descriptor` and `.data.texture` is read once per sprite per frame in all four backends; `ParticleEmitterConfig` is marked `options` and its fields are read inside the per-particle loop in `updateParticleObjects.ts`. Neither marking was wrong under the old test; both were costly under the real one.

  **Outstanding migration — the tree does not yet satisfy this entry.** Recorded so a reader who greps is not misled. 50 source-marked exclusions remain (40 `descriptor`, 8 `options`, 2 `type-only`), and two base aliases defeat the rule structurally rather than by marking: `NodeData = object` (`types/src/Node.ts`) and `MaterialData = object` (`types/src/Material.ts`), both read on per-instance paths. `npm run entity-contracts:check` reports 0 violations regardless, because it audits exported `create*` returns and a type that only ever appears as a field type is invisible to it. **A clean gate is not evidence of compliance with this entry**, and extending the gate to hot field types is the first piece of the migration, not an afterthought to it.
