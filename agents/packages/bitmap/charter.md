...
`@flighthq/bitmap` is the **CPU pixel-manipulation library** — read, generate, transform, and analyze raw RGBA pixel buffers (`Bitmap` over `Uint8ClampedArray`). This is the `BitmapData` lineage plus the wider feature set of a software raster library (BitmapData + a slice of ImageMagick/Pixman/tiny-skia's raster ops). 103 exported functions across 43 source files. Dependencies: `entity`, `image`, `types`.
...
It is also the SDK's primary **wasm-mixing beachhead** — value-in/value-out over flat typed arrays, the ideal near-zero-copy JS↔wasm boundary. The Rust `flighthq-surface` crate and `surface-rs` package are pending a separately coordinated rename to `flighthq-bitmap` / `bitmap-rs` in `flight-rs`. This is already established in the Rust port docs and influences API shape: functions stay pure, allocation stays explicit, no hidden state.
...
2. **Unified sampling contract.** Any operation that samples at non-integer coordinates accepts explicit `BitmapEdgeMode` (clamp/mirror/transparent/wrap) and `BitmapResizeMode` (nearest/bilinear/bicubic). No implicit border handling, no per-op edge vocabularies.
...
- Bitmap lifecycle: create, clone, dispose, encode (PNG/JPEG), create from canvas/image/ImageResource.
- Pixel access: get/set individual pixels, channel access, luminance, bulk extract/write.
- Region ops: sub-rectangular views (`BitmapRegion`).
...
- **[2026-07-02] Unify `BitmapEdgeMode` as the single edge-mode type.** Collapse `BitmapConvolutionEdge` (`'clamp' | 'fill' | 'wrap'`) into the canonical `BitmapEdgeMode` (`'clamp' | 'mirror' | 'transparent' | 'wrap'`). `'fill'` and `'transparent'` are the same intent under two names — consolidate on `'transparent'`. Convolution gains `'mirror'` support. All geometric and sampling ops use `BitmapEdgeMode`.
...
- **[2026-07-02] Unified sampling contract for all geometric/sampling ops.** Every function that samples at non-integer coordinates accepts explicit `BitmapEdgeMode` and `BitmapResizeMode` (nearest/bilinear/bicubic). No implicit border handling. This applies to: `resizeBitmap`, `rotateBitmap`, `transformBitmap`, `warpBitmap`, `warpBitmapQuad`, `displaceBitmap`, and convolution. Golden API — get the parameter contract right now.
...
- **[2026-07-02] Room for both CPU pixel ops and GPU-parity software rendering.** Bitmap serves both user-facing CPU pixel manipulation and SDK infrastructure (fingerprinting, test baselines, the Rust tiny-skia software-render path reads Bitmap buffers). Neither role is secondary.

  **Why:** The Rust port's `scene2d-skia` renders into `flighthq-surface` buffers. Bitmap is simultaneously a user API and an infrastructure seam. Both uses inform the API shape.

- **[2026-07-02] Wasm-mixing awareness is standing context.** API shape decisions across the SDK bear in mind that bitmap is a wasm-mixing target. This is already established in the Rust port docs and doesn't change the TS API — it reinforces existing constraints (pure functions, explicit allocation, no hidden state).

  **Why:** The near-zero-copy typed-array boundary is bitmap's strongest wasm-mixing advantage. API decisions that introduce hidden state or non-value semantics would compromise this.
...
1. **Sampling contract migration scope.** Which functions need `BitmapEdgeMode`/`BitmapResizeMode` added? The clear set: `resizeBitmap` (already has `BitmapResizeMode` but not `BitmapEdgeMode`), `rotateBitmap`, `transformBitmap`, `displaceBitmap`, convolution. `warpBitmap`/`warpBitmapQuad` already have both. Settle the full list and parameter order.

2. **Convolution `'fill'` → `'transparent'` migration.** `BitmapConvolutionEdge` has `'fill'` where `BitmapEdgeMode` has `'transparent'`. The type unification removes `BitmapConvolutionEdge` entirely. Convolution's `edge` parameter becomes `BitmapEdgeMode`. The `'fill'` value is removed (callers use `'transparent'`).
...
4. **Wide-gamut / higher bit depth.** Gold-tier maturation item. `Bitmap` is currently `Uint8ClampedArray` (8-bit RGBA). Float32 surfaces for HDR workflows and wide-gamut color spaces are a potential future extension.

5. **`bitmap-formats` neighbor.** A neighbor package for format-specific encode/decode (TIFF, EXR, HDR, TGA, etc.) beyond the browser-native PNG/JPEG.
