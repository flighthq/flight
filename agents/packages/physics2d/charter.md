...
- **[2026-08-02] P2 partial: registry complete, six of nine joint kinds solved.** `Distance`, `Revolute`, `Prismatic`, `Weld`, `Rope`, and `Mouse` ship with solvers. `Wheel`, `Pulley`, and `Gear` have neither types nor solvers — **this is unfinished work, not a design choice**. The registry keeps the rest additive: each remaining kind is a `Physics2DJointSolver` registered under a name, with no change to the world, the step, or the solve loop. Wheel composes over the prismatic constraint; pulley and gear couple two constraints rather than two bodies and want the registry's `prepare`/`solve` split more than the others do. User-directed.
