...
**Notification R5/R6 census — 2026-08-30 append.** The derived top-level population is exactly eleven: `permission`, `delivery`, `close`, `activeList`, `scheduling`, `lifecycle`, `action`, `click`, `dismiss`, `reply`, and `received`. Provider vectors are page 7, Service Worker 8, Electron common 6/macOS 8, Tauri 3, and Capacitor 6 as enumerated in the notification charter. Commands and events do not merge under R16. Notification and ScheduledNotification are provider-created Entity resources; their private origin binding removes every per-item ambient lookup, and subscription Entities carry Signals plus async attach/release. The deleted ambient/default/update/capability/channel/launch and fake scheduling paths are the exact R3 removal population. `Host.notification.permission` is the sole native notification-permission owner; generic Permissions delegates and does not add a parallel native seam. Dependency census: notification core imports entity, signals, and types; all four host packages import notification contract helpers, entity, and types; host-web additionally owns the injected DOM/SW facades. Those dependencies are used and retained.

**R19 — Native coverage is Host × injected OS profile wherever the vector varies.** A host kind alone
is not always a complete construction fact: Electron and Tauri expose different real Tray operations on
Linux, macOS, and Windows. Their constructors therefore take `DesktopOsProfile` explicitly and build
only the slots for that pair. They never read `process.platform` during construction or later calls;
the application shell maps its environment to the profile before calling the constructor. Coverage
tables, tests, and returned Host types state both axes whenever either axis changes the shape.

**Log zero-provider ruling — 2026-08-30 append.** `LogTransportBackend` had no concrete package or platform provider, so R7's zero-provider result is deletion, not a new Host lane: remove `HostAppCapabilities.logTransport`, `HasAppLogTransport`, ambient get/set/explain/has/destroy selection, its operation key, and optional operations. A caller now supplies one configured `LogTransport` Entity directly to `createFileLogSink`, which exclusively owns and pins it. `write` is a synchronous, method-tight FIFO-admission decision for one complete line; `accepted` means entry into that transport's FIFO in call order and says nothing about persistence. Awaited `flush` settles every line accepted before its call and names the achieved boundary. Awaited teardown unregisters the sink before waiting, attempts terminal transport destroy even after flush failure, preserves both outcomes, and is idempotent. There is no borrowed mode or stale successor. This does not broaden the global-core R4 exception: the SDK logger and `LogSignals` remain core under R18, while only the external destination is explicit.
