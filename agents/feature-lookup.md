...
| SWF named-graph import | `swf` + `scene2d-resources` | parser | Uncompressed `FWS` root-timeline named instances, affine transforms, and `SymbolClass`/`ExportAssets` linkage into `Scene2DDocument`; nested sprites, visuals/extents, and compression are staged |
