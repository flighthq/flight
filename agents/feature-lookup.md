...
| SWF named-graph import | `swf` + `scene2d-resources` | parser | Uncompressed `FWS` `PlaceObject`–`PlaceObject4` / `RemoveObject`–`RemoveObject2` first-frame graphs, including fresh/move/replacement state, names, affine transforms, linkage, and stage/shape/text/morph/embedded-image/lossless-bitmap/video/recursive-sprite extents across recursive `DefineSprite` containers into `Scene2DDocument`; a revision-pinned external Ruffle named-shape file verifies the boundary and zero-bit RECT compatibility; later frames, visual bodies, and compression are staged |
