...
| SWF named-graph import | `swf` + `scene2d-resources` | parser | Uncompressed `FWS` named instances, affine transforms, linkage, and stage/shape/text/morph/recursive-sprite extents across recursive `DefineSprite` first-frame graphs into `Scene2DDocument`; later frames, visual bodies, and compression are staged |
