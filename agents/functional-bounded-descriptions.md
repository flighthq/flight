...
| --- | --- | --- | --- |
| `swf-color-transform` | **undecided design — INELIGIBLE** | the square is white or green, never red or blue | One file covers all four backends. WebGL and WebGPU fold the imported RGB transform into their tessellated solid-shape path; Canvas and DOM leave it unapplied. Whether that transform belongs in backend-neutral scene data or in the GPU fold is **under design**, so there is no fact of the matter yet about which is right and blessing either would settle it by accident. The three cross-backend-equal alpha cases now live separately in `swf-alpha-transform`. |
