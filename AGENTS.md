...
- [draw order model](agents/draw-order-model.md) — **unratified.** Before adding an ordering field to a node, giving a format importer its own child-reordering pass, or deciding where a draw-order timeline binds. Child order is the only order; ordering is a caller-owned `NodeOrderList`, never node state.
