...
- [explicit dependency model](agents/explicit-dependency-model.md) — **unratified.** Before adding module-scoped global state, `register*` functions that mutate shared state, `set*Backend` singletons, or any ambient dependency a function reaches for without taking as an argument. Replaces ambient state with explicit value-based dependency threading.
