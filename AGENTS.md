...
- Keep _transient_ notes about the **work** out of the code. `TODO`, "half-done", "revisit after X", and known-incomplete threads rot inline. Their home is the `## Open` section of the package's `status.md` — present tense, rewritten in place, under 6,000 characters, with session narration left in git where the diff already is (see [packages](agents/packages/index.md)). Code carries meaning that survives; work-in-progress state lives in status. Caller-facing warning comments ("must call X first", "do not release twice") are likewise banned inline — they become guard-layer runtime warnings (see [diagnostics](agents/conventions/diagnostics.md)).
