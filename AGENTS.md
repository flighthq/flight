...
- **Before handoff** — run `npm run check <package>` and `npm run test <package>` for every affected package on the final tree. Run the bare whole-repo `npm run check` when a change crosses package boundaries or affects shared contracts, manifests, exports, build structure, or repository-wide tooling. Run the bare whole-repo `npm run test` only for broadly cross-cutting behavior or when explicitly requested; integration/CI runs it once for the combined tree. The full render matrix is also CI's job; see [commands](agents/commands.md#checkpoints-in-detail).
...
- [effect recipe model](agents/effect-recipe-model.md) — **unratified.** Before adding a field to an effect descriptor, changing how a chain sequences passes, or adding an effect runner. Who turns an effect intent into passes, the `strength` definition, and the accepted-then-silently-dropped field defect.
...
- [timeline cue model](agents/timeline-cue-model.md) — before adding anything that fires on frame entry, giving an importer a `FrameScript`, or touching `swfFrameAction.ts`. Authored cues are plain kind-dispatched data on the source; `constructFrame` is idempotent content and cannot hold them, and importers emit zero closures.
- [document audio resources](agents/document-audio-resources.md) — before adding a non-pixel byte payload to a document, or deciding where an imported sound goes. The bytes are a document sidecar on the image lane's terms; the playback never is.
- [skeleton2d animation model](agents/skeleton2d-animation-model.md) — before adding a non-bone timeline, placing a constraint solver, or adding a vertex-offset (deform) timeline. Targets dispatch by `kind` through a registry; attachment swap is a Step index track; the track-vs-cue test.
...
- [draw order model](agents/draw-order-model.md) — before adding an ordering field to a node, giving a format importer its own child-reordering pass, or deciding where a draw-order timeline binds. Child order is the only order; ordering is a caller-owned `NodeOrderList`, never node state.
- [texture color space](agents/texture-color-space-model.md) — **unratified.** Before touching `Texture.colorSpace`, the `resolveGl*/resolveWgpu*Texture` resolvers, or adding a color-space-aware op to the 2D path. Where Flight decodes and where it encodes: the 2D path samples linear and draws to an sRGB canvas with no encode stage.
