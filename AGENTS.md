...
- `dispose*` and `destroy*` are distinct teardown verbs. `dispose*` detaches listeners and clears registries so the entity becomes GC-eligible — nothing to free. `destroy*` immediately frees a non-GC resource (GPU textures, native handles), leaving the entity invalid. Choose by what teardown does: detach-and-release-to-GC → `dispose*`; free-a-resource-now → `destroy*`. (`release*` stays reserved for pool/cache `acquire`/`capture` brackets.)
- `attach*` / `detach*` wire a host capability to an app-layer object; `begin*` / `end*` bracket a stateful pass; `clear*` resets a container to empty without destroying it; `remove*` is the inverse of `add*`.
...
- Dependencies are explicit: every function takes what it needs as arguments. No `set*Backend` singletons, no module-scoped mutable state that functions reach for, no `Object.defineProperty` indirection for shared state. The host is a value you pass; a missing capability is a compile-time type error. Entity is the base type for domain objects Flight defines and allocates — `create*` always returns Entity. Dispatch infrastructure (renderers, registry tables, pipelines, host capabilities) is plain data, not Entity — formed as literals with no `allocateEntity`/`finishEntity`. Only arrays, callables, native handles, and primitives also fall outside Entity. See [explicit dependency model](agents/explicit-dependency-model.md) and [entity boundary](agents/conventions/entity-boundary.md).
