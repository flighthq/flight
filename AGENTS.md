...
- Dependencies are explicit: every function takes what it needs as arguments. No `set*Backend` singletons, no module-scoped mutable state that functions reach for, no `Object.defineProperty` indirection for shared state. The host is a value you pass; a missing capability is a compile-time type error. Entity is the base type for every object Flight defines and allocates — `create*` always returns Entity, with no descriptor or options opt-out; only arrays, callables, native handles, and primitives fall outside. See [explicit dependency model](agents/explicit-dependency-model.md).
