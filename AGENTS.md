...
- [explicit dependency model](agents/explicit-dependency-model.md) — **unratified.** Before adding module-scoped global state, `register*` functions that mutate shared state, `set*Backend` singletons, `Object.defineProperty` indirection for shared state, or any ambient dependency a function reaches for without taking as an argument. Const objects over factories, trait constraints on functions, two-level Host capability groups, three-layer graphics context (acquisition → context state → render state).
