...
- **After adding or renaming an exported `register*` function** — `npm run backend-prefix:check`. The render backend token prefixes the type: `register` + `Gl`/`Wgpu`/`Canvas`/`Dom` + the target type's name, so `registerGlToonMaterial` for `ToonMaterial`. Words that are not part of the type name — `Standard`, `Default`, `BuiltIn` — stay in front of that whole unit, which is why `registerBuiltInGlModifierSnippets` is already right. The rule does **not** mean inserting a backend token where there is none; a shared, backend-independent registrar (the effect padding resolvers) carries no backend in its name at all.
