// DOM backend of the pixelate-parity test.
//
// Pixelate has NO CSS-filter equivalent, so there is no native DOM pixelate path: app.ts draws the CPU
// pixelate result as BOTH the reference tile and the native tile, and parity holds by construction. This
// backend just renders the scene — drawNativePixelate is omitted (a no-op).
//
