...
//   - Gl: the native inner glow is a multi-pass shader (invert-tint → box-blur → clip → composite)
//     over offscreen render targets, composited onto the screen after the scene. `drawNativeGlow(spec)`
//     runs that GPU pass; it is a no-op on DOM/Canvas. Gl is the meaningful comparison.
