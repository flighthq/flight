// Wgpu backend of the filter-median-parity test. Mechanical port of render.webgl.ts to the Wgpu offscreen-filter
// flow (see filter-blur-parity/render.webgl.ts for the annotated reference): render the source into an
// offscreen target with a baked vertical flip, run applyMedianFilterToWgpu over offscreen targets, then composite the
...
function renderFrame(state: WgpuRenderState, root: DisplayObject, specs: readonly NativeMedianSpec[]): void {
...
function compositeNative(
  ...
  applyMedianFilterToWgpu(state, sourceTarget, destTarget, { radius: spec.radius });
