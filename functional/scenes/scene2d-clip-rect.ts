// scene2d-clip-rect — validates rectangular clipping: a large filled shape is clipped to a SMALLER axis-aligned
...
export function assertRender(frame: Readonly<Bitmap>): void {
  ...
  if (!isShapeColor(inside)) {
    throw new Error(`[scene2d-clip-rect] inside clip window not orange — got #${hex(inside)}`);
  }
  ...
  if (!isBackground(clipped)) {
    throw new Error(
      `[scene2d-clip-rect] shape pixel outside clip window not clipped to background — got #${hex(clipped)}`,
    );
  }
  ...
  if (!isBackground(outside)) {
    throw new Error(`[scene2d-clip-rect] point outside everything not background — got #${hex(outside)}`);
  }
}
