...
// Two CCW triangles share only their center vertex. Both UV determinants are positive, but their U
// axes point in opposing model-space directions. One indexed tangent cannot represent both lobes:
// Flight must split the center by direction even though tangent.w remains +1 on each side.
const vertices = new Float32Array(5 * 12);
setVertex(vertices, 0, 0, 0, 0, 0);
setVertex(vertices, 1, 1.5, -1, 1, 0);
setVertex(vertices, 2, 1.5, 1, 1, 1);
setVertex(vertices, 3, -1.5, 1, 1, 0);
setVertex(vertices, 4, -1.5, -1, 1, 1);
...
if (geometry.vertices.length !== 6 * 12 || geometry.indices![0] === geometry.indices![3]) {
  throw new Error('[mesh-tangent-directional-seam] incompatible same-handed corners still share one tangent record');
}
...
export function assertRender(bitmap: Readonly<Bitmap>): void {
  ...
  const offset = Math.floor(bitmap.width * 0.035);
  ...
  if (right <= 50) throw new Error(`[mesh-tangent-directional-seam] lit lobe is blank (luminance ${right})`);
  if (right <= left + 50) {
    throw new Error(
      `[mesh-tangent-directional-seam] directional split did not separate lighting (left ${left}, right ${right})`,
    );
  }
}
