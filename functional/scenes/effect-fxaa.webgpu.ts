...
registerWgpuFxaaEffect(state);

const pipeline = createWgpuEffectState(state, { sampleCount: 4, format: 'rgba8' });
...
export function render(root: Node2D): void {
  ...
  endWgpuEffectState(scenePass, pipeline, [createFxaaEffect({ edgeThreshold: 0.0312, subpixel: 0.75 })]);
