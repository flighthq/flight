...
declareExpectedImageDescription(
  'An 800x600 field on a near-black background with a single sphere centred in it, about 245 px across — D = ' +
    ...
    'right side is clearly brighter than the left. Its surface carries a faint SHIFTING COLOUR CAST that varies ' +
    'across the sphere — a soap-bubble or oil-film tint, so the lit region is not one neutral grey but picks up ' +
    'different hues at different angles. A completely neutral grey sphere is the failure here, as is one lit ' +
    'uniformly or brighter on the left. The background stays near-black.',
);
...
export function assertRender(bitmap: Readonly<Bitmap>): void {
  ...
  if (litLuminance <= 24) {
    throw new Error(`[material-iridescence] lit side is blank (luminance ${litLuminance}) — mesh did not render`);
  }
  if (litLuminance <= shadowLuminance + 24) {
    throw new Error(
      `[material-iridescence] no directional shading: lit side (${litLuminance}) is not clearly brighter than shadow side (${shadowLuminance})`,
    );
  }
}
