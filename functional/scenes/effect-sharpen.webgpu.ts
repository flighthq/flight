...
// ORACLE-BLOCK
// Sharpening (amount 1.5) amplifies edge contrast, raising high-frequency energy above the
// unprocessed scene's level. The 18 rotated shapes provide many edges for the filter to amplify.
// Without the effect, HF energy stays at the base scene level (~2); with sharpening it exceeds 3
// from the amplified edge transitions.
export function assertRender(frame: Readonly<Bitmap>): void {
  const hf = measureHighFrequency(frame);
  if (hf < 3) {
    throw new Error(
      `[effect-sharpen] high-frequency energy is ${hf.toFixed(2)} (expected >= 3) — ` + `sharpening not applied`,
    );
  }
}
