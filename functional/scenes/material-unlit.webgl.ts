...
// material-unlit — proves an UnlitMaterial mesh renders as a FLAT, UNSHADED surface on the Gl and Wgpu
// forward renderers, independent of scene lighting. A single unlit-blue sphere sits at the origin under
// the SAME directional + ambient rig as material-standard-pbr — but because Unlit is lighting-
// independent, the surface must read as a flat, bright, roughly UNIFORM blue with no light/dark gradient
// across it. That uniformity (despite a strong angled directional light) is the signature that separates
// an unshaded material from a shaded one: a PBR sphere under this rig has a clear bright/shadow split
// (see material-standard-pbr), while Unlit does not.
//
...
// cleanly — the point is that Unlit shows none.
const geometry = createSphereMeshGeometry(0.5, 48, 32);

// A flat blue base color. Unlit emits this color directly to the rgba16f scene target, ignoring all
// lights and surface normals.
const material = createUnlitMaterial({ baseColor: 0x40a0e0ff });
...
// The same strong angled sun + ambient fill as material-standard-pbr. Unlit ignores both, so this
...
// Assertion: the surface is bright (the base color emits directly) AND uniform (lighting-independent).
...
export function assertRender(bitmap: Readonly<Bitmap>): void {
  ...
  if (center <= 24) {
    throw new Error(`[material-unlit] bitmap is blank (center luminance ${center}) — mesh did not render`);
  }
  ...
  if (Math.abs(right - left) > 24) {
    throw new Error(
      `[material-unlit] unlit bitmap is not uniform: left (${left}) vs right (${right}) differ — it appears to be responding to the directional light`,
    );
  }
}
