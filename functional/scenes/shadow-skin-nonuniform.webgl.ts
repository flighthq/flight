...
import { registerGlMeshSkinning, renderGlScene3D, renderGlScene3DShadowMap } from '@flighthq/scene3d-gl';
...
// layouts), so nothing declared in one place breaks loudly if they drift apart, and the covector work
// added a second palette that a shadow pass could now bind by mistake.
//
...
// would draw its skinned mesh at the bind pose — including its shadow caster, which gates on the same capability.
