...
declareExpectedImageDescription(
  'A centered sphere (radius 0.5, 48×32 segments) on a dark background (0x0a0c10) rendered with toon material (mid-gray 0x808080, 4 cel-shading steps). A white directional light from upper-right illuminates the screen-right hemisphere; the N·L gradient is quantized into 4 discrete stepped bands rather than a smooth gradient, creating a cartoon/cel-shaded look. Dim cool ambient fill (0x6070a0, intensity 0.15). Camera at (0,0,3) looking at origin.',
);
...
// begin/end (Toon writes linear HDR into the rgba16f scene target, depth-tested), and
...
// material-toon — proves a 3D Toon mesh renders WITH directional lighting on the Gl and Wgpu
...
export function assertRender(bitmap: Readonly<Bitmap>): void {
  ...
  if (litLuminance <= 24) {
    throw new Error(`[material-toon] lit side is blank (luminance ${litLuminance}) — mesh did not render`);
  }
  if (litLuminance <= shadowLuminance + 24) {
    throw new Error(
      `[material-toon] no directional shading: lit side (${litLuminance}) is not clearly brighter than shadow side (${shadowLuminance})`,
    );
  }
}
