// scene2d-clip-contour — validates polygon (contour) clipping: a large filled shape is clipped to a TRIANGULAR
...
export function assertRender(frame: Readonly<Bitmap>): void {
  ...
  if (!isShapeColor(inside)) {
    throw new Error(`[scene2d-clip-contour] triangle interior not cyan — got #${hex(inside)}`);
  }
  ...
  if (!isBackground(corner)) {
    throw new Error(
      `[scene2d-clip-contour] bbox corner outside polygon not clipped to background — got #${hex(corner)}`,
    );
  }
}
