...
// scene-skin-nonuniform-normals — a CPU/GPU AGREEMENT check on the skinned-normal covector path.
//
// A normal is a covector: under a non-uniformly scaled joint it transforms by the palette matrix's
// INVERSE-TRANSPOSE, not by the palette matrix itself. The two coincide exactly for a rigid joint, so
// every rigid skinning scene in this suite passes either way and none of them can see the difference.
// This scene makes them differ: one joint carries scale (3, 1, 1), so its normal matrix is
// diag(1/3, 1, 1) while its pose matrix is diag(3, 1, 1).
//
// Two facets are fully weighted to that joint and carry authored shading normals — (1, 0, 5) and
// (5, 0, 1) — chosen so the scale REVERSES their brightness order rather than merely rescaling it: a
// diagonal scale cannot flip a sign, so ordering is the one property that distinguishes the two
// matrices. Under the correct normal palette facet B is the brighter; under the pose matrix used as a
// normal matrix (the defect the covector fix removed) facet A is. `assertRender` predicts the ordering
// on the CPU through the real `skinVertices` path, reading the SAME palette the GPU uploaded, and
// requires the rendered pixels to agree — so this fails on any GPU-side palette drift, not just on a
// missing inverse-transpose.
//
// The reversal itself is re-derived at assert time and required to hold. That keeps the scene from
// silently decaying into a non-trial: if a future edit to the geometry or the joint made the two
