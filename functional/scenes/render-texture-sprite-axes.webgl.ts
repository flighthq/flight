...
// Reproduce the pooled-slab failure: the old view selects a 100x80 region of a 720x480 physical
// source. Reacquisition must expose the new 100x80 target as a full view, not retain 100/720 and
// shrink the Sprite to roughly 14 pixels.
const pool = createGlRenderTexturePool();
const slabView = acquireGlRenderTexture(state, pool, { height: 480, width: 720 });
...
renderIntoGlRenderTexture(state, renderTexture, (captureState) => {
  setGlRenderTransform2D(captureState, createMatrix());
  prepareScene2DRender(captureState, producer);
  renderGlScene2D(captureState, producer);
});
