import { applyInnerGlowFilterToWebGL } from './innerGlowFilter';
import { makeFilterState, makeRenderTarget, makeScratch } from './testHelper';

describe('applyInnerGlowFilterToWebGL', () => {
  it('applies a default inner glow without throwing', () => {
    const { state } = makeFilterState();
    expect(() =>
      applyInnerGlowFilterToWebGL(state, makeRenderTarget(), makeRenderTarget(), makeScratch(), {}),
    ).not.toThrow();
  });
