import { applyInnerShadowFilterToWebGL } from './innerShadowFilter';
import { makeFilterState, makeRenderTarget, makeScratch } from './testHelper';

describe('applyInnerShadowFilterToWebGL', () => {
  it('applies a default inner shadow without throwing', () => {
    const { state } = makeFilterState();
    expect(() =>
      applyInnerShadowFilterToWebGL(state, makeRenderTarget(), makeRenderTarget(), makeScratch(), {}),
    ).not.toThrow();
  });
