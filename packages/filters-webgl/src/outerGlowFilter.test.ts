import { applyOuterGlowFilterToWebGL } from './outerGlowFilter';
import { makeFilterState, makeRenderTarget, makeScratch } from './testHelper';

describe('applyOuterGlowFilterToWebGL', () => {
  it('applies a default outer glow without throwing', () => {
    const { state } = makeFilterState();
    expect(() =>
      applyOuterGlowFilterToWebGL(state, makeRenderTarget(), makeRenderTarget(), makeScratch(), {}),
    ).not.toThrow();
  });
