import { applyGradientGlowFilterToWebGL } from './gradientGlowFilter';
...
describe('applyGradientGlowFilterToWebGL', () => {
  it('applies without throwing', () => {
    const { state } = makeFilterState();
    expect(() =>
      applyGradientGlowFilterToWebGL(state, makeRenderTarget(), makeRenderTarget(), makeScratch(), {
        colors: RAMP_COLORS,
        alphas: RAMP_ALPHAS,
        ratios: RAMP_RATIOS,
      }),
