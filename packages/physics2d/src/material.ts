...
export function mixPhysics2DFriction(first: number, second: number): number {
  return Math.sqrt(first) * Math.sqrt(second);
}

// A contact uses the bouncier surface's restitution. Kept beside friction mixing so callers that
// author or inspect contacts can predict both combined values without duplicating step internals.
export function mixPhysics2DRestitution(first: number, second: number): number {
