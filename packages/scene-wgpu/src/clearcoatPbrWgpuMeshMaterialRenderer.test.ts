...
import { createClearcoatPbrMaterial } from '@flighthq/materials';
...
} from './clearcoatPbrWgpuMeshMaterialRenderer';
...
function makeProxy(): SceneRenderProxy {
  ...
  return {
    material: createClearcoatPbrMaterial({ clearcoat: 1 }),
...
describe('clearcoatPbrWgpuMeshMaterialRenderer', () => {
  it('bind selects a pipeline and binds frame + material groups + uploads uniforms', () => {
    const { fake, state } = makeWgpuSceneState();
    clearcoatPbrWgpuMeshMaterialRenderer.bind(
      state,
      createClearcoatPbrMaterial({ clearcoat: 1 }),
      makeLights(),
      makeCamera(),
    );
  ...
  it('bind compiles the CLEARCOAT extension variant', () => {
    const { fake, state } = makeWgpuSceneState();
    clearcoatPbrWgpuMeshMaterialRenderer.bind(state, createClearcoatPbrMaterial(), makeLights(), makeCamera());
    const moduleCall = fake.calls.find((c) => c.name === 'createShaderModule');
    const code = (moduleCall!.args[0] as { code: string }).code;
    expect(code).toContain('const CLEARCOAT : bool = true;');
  });

  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    const { fake, state } = makeWgpuSceneState();
    const proxy = makeProxy();
    clearcoatPbrWgpuMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    clearcoatPbrWgpuMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind has not selected a pipeline', () => {
    const { fake, state } = makeWgpuSceneState();
    clearcoatPbrWgpuMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
    expect(fake.calls.some((c) => c.name === 'drawIndexed')).toBe(false);
  });
});

describe('registerClearcoatPbrWgpuMaterial', () => {
  it('installs the renderer for ClearcoatPbrMaterialKind', () => {
    const { state } = makeWgpuSceneState();
    registerClearcoatPbrWgpuMaterial(state);
    expect(getWgpuMeshMaterialRenderer(state, ClearcoatPbrMaterialKind)).toBe(clearcoatPbrWgpuMeshMaterialRenderer);
  });
});
