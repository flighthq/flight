...
export function areWgpuSceneCustomShaderGuardsEnabled(state: WgpuRenderState): boolean {
  return getWgpuSceneRuntime(state).customShaderGuard != null;
}

// Enables actionable, warn-once diagnostics for the fixed custom-material WGSL binding ABI.
export function enableWgpuSceneCustomShaderGuards(state: WgpuRenderState): void {
  getWgpuSceneRuntime(state).customShaderGuard = runWgpuCustomShaderGuards;
}
...
export function runWgpuCustomShaderGuards(
  ...
  if (getWgpuSceneRuntime(state).customShaderGuard == null) return;
