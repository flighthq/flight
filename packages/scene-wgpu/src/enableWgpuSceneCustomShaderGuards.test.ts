...
import {
  areWgpuSceneCustomShaderGuardsEnabled,
  enableWgpuSceneCustomShaderGuards,
  runWgpuCustomShaderGuards,
} from './enableWgpuSceneCustomShaderGuards';
import { makeWgpuSceneState } from './wgpuSceneTestHelper';

describe('areWgpuSceneCustomShaderGuardsEnabled', () => {
  it('reports installation state', () => {
    const { state } = makeWgpuSceneState();
    expect(areWgpuSceneCustomShaderGuardsEnabled(state)).toBe(false);
    enableWgpuSceneCustomShaderGuards(state);
    expect(areWgpuSceneCustomShaderGuardsEnabled(state)).toBe(true);
  });
});

describe('enableWgpuSceneCustomShaderGuards', () => {
  it('is idempotent', () => {
    const { state } = makeWgpuSceneState();
    enableWgpuSceneCustomShaderGuards(state);
    enableWgpuSceneCustomShaderGuards(state);
