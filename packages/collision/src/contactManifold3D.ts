...
import type { CollisionContactManifold3D, EntityConstruction } from '@flighthq/types/contract';
...
export function createCollisionContactManifold3D(): CollisionContactManifold3D {
  const out = allocateEntity<CollisionContactManifold3D>();
  initializeCollisionContactManifold3D(out);
...
export function initializeCollisionContactManifold3D(out: EntityConstruction<CollisionContactManifold3D>): void {
  const points = [];
