...
import type { WellKnownMenuItemRoleValue } from './WellKnownMenuItemRole.ts';
...
export interface HostAppMenuCapability {
  destroy?(): void;
...
// subscription only.
export interface HostMenuSelectCapability {
  subscribe(listener: (id: string) => void): () => void;
}
