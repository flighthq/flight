...
// the fixed-function `BlendMode` enum cannot express. The backdrop is a per-state registered GPU resource
// referenced by `backdropKey` (populated with registerGlBlendEffectBackdrop or
// registerWgpuBlendEffectBackdrop), mirroring how CustomShaderEffect references a registered fragment
// source — a live backend texture/target cannot live in plain serializable data, so the intent carries
