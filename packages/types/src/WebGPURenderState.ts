...
import type { WebGPUMaterialRenderer } from './WebGPUMaterialRenderer';
...
// Package-private GPU state for a WebGPURenderState entity. Lives in the runtime tier (not on the
// entity) so the public WebGPURenderState surface stays minimal; the render path resolves it each
// frame via getWebGPURenderStateRuntime. Defined in @flighthq/types — the header layer — so
// out-of-package custom renderers can reach the same state.
export interface WebGPURenderStateRuntime extends RenderStateRuntime {
  // Active blend mode tracked to avoid redundant pipeline rebinds. Internal — formerly public on the
  // WebGPURenderState entity.
...
// A bound WebGPU bitmap shader: a render pipeline plus a bind hook that writes its per-draw uniforms.
// The default shader is registered on the render state runtime; custom shaders are installed via
// registerWebGPUBitmapShader / setWebGPUShader.
...
// One active contour clip's GPU resources. Kept on a stack so popWebGPUClipContours can redraw the
...
// WebGPURenderState runtime's renderTargetStack.
...
// WebGPURenderState runtime's textureCache.
