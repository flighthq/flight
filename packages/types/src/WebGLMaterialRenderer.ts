...
// Per-backend behavior for a material kind on WebGL, registered against the kind on the
// render state via registerWebGLMaterialRenderer. The renderer owns its shader (program and
...
export interface WebGLMaterialRenderer {
  ...
  packInstance?(state: WebGLRenderState, materialData: MaterialData | null, out: Float32Array, offset: number): void;
}
