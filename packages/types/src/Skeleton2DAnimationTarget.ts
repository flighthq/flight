...
// SceneAnimationTarget (which is `{ node, path }` over a SceneNode's TRS). Stored as the channel's opaque
// `targetRef`; a binder — `applyAnimationClipToSkeleton2D` in @flighthq/skeleton2d, mirroring
// `applyAnimationClipToScene` — casts `targetRef` to this, samples the channel's track, writes the value
