...
// stream connection on the same swappable HostSocketCapability; it never masquerades as framed Socket state.
...
// opt-in signals. Modeled on HostConnectivityChangeCapability.subscribe's listener, but split per event kind so the
...
export interface HostSocketCapability {
