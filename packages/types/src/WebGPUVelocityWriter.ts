...
// Per-draw velocity production, the WebGPU mirror of WebGLVelocityWriter. Velocity is tied to the draw:
// only the thing that draws a renderable knows its coverage and per-instance breakdown (a QuadBatch's
// per-instance velocities live nowhere else). A WebGPUVelocityWriter is registered per renderable Kind
// (registerWebGPUVelocityWriter) and draws that kind's velocity into the bound velocity target during the
...
export interface WebGPUVelocityContext {
...
export type WebGPUVelocityWriter = (ctx: Readonly<WebGPUVelocityContext>, node: object) => void;
