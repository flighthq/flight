...
// dispatches an agnostic Effect[] through the registry, so one intent list drives every backend.
// Effect is an open base contract: a new effect is added by defining its interface (extending
// Effect with a literal `kind`) and registering a runner — no central union to edit here.
