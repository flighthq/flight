...
// `@capacitor/device` — device identity. All async, so the sync HostDeviceCapability getInfo/getId are served
...
// Capacitor host advertises message and prompt dialog capabilities but omits every file-dialog slot.
...
// check/requestPermissions. `watchPosition` resolves a string callback id, whereas HostGeolocationCapability
...
// HostHapticsCapability triggers fire-and-forget and report `true`. `style`/`type` are Capacitor's uppercase
...
// sync HostSoftKeyboardInfoCapability.getInfo reads a local mirror kept fresh by the plugin's will/did show/hide
