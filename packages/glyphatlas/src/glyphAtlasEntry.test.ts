...
describe('getGlyphAtlasEntry', () => {
  ...
  it('returns null when the rasterizer produces nothing', () => {
    const backend: HostGlyphRasterizerCapability = { rasterize: () => null };
    const atlas = createGlyphAtlas({
      fontFamily: 'mock',
      fontSize: 16,
      height: 256,
...
): { backend: HostGlyphRasterizerCapability; calls: number[] } {
  const calls: number[] = [];
  const backend: HostGlyphRasterizerCapability = {
...
describe('setGlyphAtlasEntryGuard', () => {
  ...
  it('stops reporting once cleared with null', () => {
    ...
    const nullBackend: HostGlyphRasterizerCapability = { rasterize: () => null };
    const atlas = createGlyphAtlas({
      fontFamily: 'm',
      fontSize: 16,
      height: 64,
