...
function geolocationProvider(outcome: GeolocationAccessOutcome, calls?: { count: number }): HostGeolocationProvider {
  return {
    ...
    getCurrentPositionResult: async () => ({ position: null, reason: 'unavailable' }),
    isAvailable: () => true,
    promptForAccess: () => {
...
describe('requestPermission', () => {
  it('projects a granted geolocation prompt to granted', async () => {
    expect(
      await requestPermission(
        permissionsProvider(),
        undefined,
        geolocationProvider({ reason: 'granted' }),
        'geolocation',
      ),
    ).toEqual({
      reason: 'granted',
  ...
  it('projects a denied geolocation prompt to denied', async () => {
    expect(
      await requestPermission(
        permissionsProvider(),
        undefined,
        geolocationProvider({ reason: 'denied' }),
        'geolocation',
      ),
    ).toEqual({
      reason: 'denied',
  ...
  it('projects a dismissed geolocation prompt to prompt', async () => {
    expect(
      await requestPermission(
        permissionsProvider(),
        undefined,
        geolocationProvider({ reason: 'dismissed' }),
        'geolocation',
      ),
    ).toEqual({
      reason: 'dismissed',
  ...
  it('keeps the granted decision when cleanup fails', async () => {
    expect(
      await requestPermission(
        permissionsProvider(),
        undefined,
        geolocationProvider({ reason: 'cleanup-failed' }),
        'geolocation',
      ),
    ).toEqual({
  ...
  it('asks the geolocation provider exactly once per request', async () => {
    ...
    await requestPermission(
      permissionsProvider(),
      undefined,
      geolocationProvider({ reason: 'granted' }, calls),
      'geolocation',
    );
  ...
  it('reports runtime-unavailable when no geolocation provider is installed', async () => {
    const outcome = await requestPermission(permissionsProvider(), undefined, undefined, 'geolocation');
...
function permissionsProvider(): HostPermissionsProvider {
  return {
    ...
    queryPermission: async () => ({ reason: 'unsupported' }),
