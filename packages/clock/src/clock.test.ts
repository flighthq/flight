...
describe('advanceClock', () => {
  it('sets deltaTime to the real delta and accumulates elapsed on a realtime root', () => {
    ...
    advanceClock(clock, 500);
    expect(clock.deltaTime).toBe(500);
    expect(clock.elapsed).toBe(500);
    advanceClock(clock, 250);
    expect(clock.deltaTime).toBe(250);
  ...
  it('applies the local scale', () => {
    ...
    expect(clock.deltaTime).toBe(2000);
  ...
  it('composes scale down a 3-level hierarchy — grandparent to parent to child', () => {
    ...
    advanceClock(grandparent, 1000);
    expect(grandparent.deltaTime).toBe(2000); // 1000 * 2
    expect(parent.deltaTime).toBe(6000); // 2000 * 3
    expect(child.deltaTime).toBe(3000); // 6000 * 0.5
    expect(child.elapsed).toBe(3000);
  });

  it('composes scale down the hierarchy — a child rate is the product of the chain', () => {
    const parent = createClock({ scale: 2 });
    const child = createChildClock(parent, { scale: 0.5 });
    advanceClock(parent, 1000);
    expect(parent.deltaTime).toBe(2000);
    expect(child.deltaTime).toBe(1000); // 1000 * 2 * 0.5
    expect(child.elapsed).toBe(1000);
  });
  ...
  it('emits onTick with the scaled delta when signals are enabled', () => {
    ...
    advanceClock(clock, 500);
    expect(received).toEqual([1000]); // 500 * 2
  });
  ...
  it('applies a negative scale, reversing the effective delta direction', () => {
    ...
    advanceClock(clock, 1000);
    expect(clock.deltaTime).toBe(-1000);
    expect(clock.elapsed).toBe(-1000);
  });

  it('does not emit or throw when signals are not enabled', () => {
    const clock = createClock();
    expect(() => advanceClock(clock, 1000)).not.toThrow();
