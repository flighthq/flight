...
import { applyParticleCollisions } from './applyParticleCollisions';
...
describe('applyParticleCollisions — circle', () => {
  it('exclude mode keeps a particle outside the disc', () => {
    ...
    applyParticleCollisions(emitter, state, [
      { type: 'circle', x: 0, y: 0, radius: 10, mode: 'exclude', restitution: 1 },
    ]);
  ...
  it('contain mode keeps a particle inside the boundary', () => {
    ...
    applyParticleCollisions(emitter, state, [{ type: 'circle', x: 0, y: 0, radius: 10, mode: 'contain' }]);
    const d = Math.hypot(emitter.data.transforms[0], emitter.data.transforms[1]);
    expect(d).toBeCloseTo(10);
  });
});

describe('applyParticleCollisions — integration', () => {
  it('settles a falling particle onto a floor over multiple frames', () => {
    ...
    const floor = [{ type: 'plane' as const, nx: 0, ny: -1, distance: -500 }];
    for (let i = 0; i < 60; i++) {
...
describe('applyParticleCollisions — plane (floor)', () => {
  it('pushes a penetrating particle back to the surface and stops it (restitution 0)', () => {
    ...
    applyParticleCollisions(emitter, state, [{ type: 'plane', nx: 0, ny: -1, distance: -500 }]);
  ...
  it('bounces with restitution', () => {
    const { emitter, state } = place(0, 520, 0, 100);
    applyParticleCollisions(emitter, state, [{ type: 'plane', nx: 0, ny: -1, distance: -500, restitution: 0.5 }]);
  ...
  it('applies friction to the tangential component', () => {
    ...
    applyParticleCollisions(emitter, state, [{ type: 'plane', nx: 0, ny: -1, distance: -500, friction: 0.25 }]);
...
describe('applyParticleCollisions — rect', () => {
  it('contain mode clamps particles to the bounds and reflects velocity', () => {
    ...
    applyParticleCollisions(emitter, state, [
      { type: 'rect', x: 0, y: 0, width: 100, height: 100, mode: 'contain', restitution: 1 },
    ]);
  ...
  it('exclude mode pushes a particle out of a solid box along the shallowest axis', () => {
    ...
    applyParticleCollisions(emitter, state, [{ type: 'rect', x: 0, y: 0, width: 100, height: 100, mode: 'exclude' }]);
