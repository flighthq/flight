...
export const defaultWebGPUInputTextRenderer: DisplayObjectRenderer = {
  ...
  submit: drawWebGPUInputText,
};

// Renderer-agnostic caret/selection overlay drawn onto the rich text offscreen canvas. Identical to
// the WebGL InputText overlay — both backends rasterize the field to a 2D canvas before uploading,
// so the caret/selection chrome is plain Canvas2D drawing with no backend-specific calls.
function drawWebGPUInputTextOverlay(
