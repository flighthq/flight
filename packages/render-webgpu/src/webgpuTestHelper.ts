...
import { createWebGPURenderState } from './webgpuRenderState';

// WebGPU flag constants are only type-level in @webgpu/types; install runtime values for JSDOM.
...
// Minimal WebGPU mock for JSDOM test environments.
...
export async function createWebGPURenderStateForTest(): Promise<WebGPURenderState> {
  const canvas = document.createElement('canvas');
  canvas.width = 800;
  canvas.height = 600;
  return createWebGPURenderState(canvas);
}
