...
import type {
  ...
  WebGPURenderState,
  WebGPURenderTarget,
  WebGPUVelocityContext,
  WebGPUVelocityWriter,
} from '@flighthq/types';
import { EntityRuntimeKey } from '@flighthq/types';
import { getVelocity } from '@flighthq/velocity';

import { getWebGPURenderStateRuntime } from './webgpuRenderState';
import { createWebGPURenderTarget } from './webgpuRenderTarget';

// WebGPU velocity-buffer production, the mirror of render-webgl's webglVelocity. Velocity is tied to the
// draw, so production is per-kind: the velocity pass walks the scene and dispatches a registered
// WebGPUVelocityWriter for each node's kind, which draws that kind's velocity into the bound rgba16float
...
// top-left origin, y-down; drawWebGPUVelocityQuad maps a rect into clip space, flipping y.

/** Allocates an rgba16float render target sized to hold a signed, sub-pixel screen-space velocity buffer. */
export function createWebGPUVelocityTarget(
  state: WebGPURenderState,
  width: number,
  height: number,
): WebGPURenderTarget {
  return createWebGPURenderTarget(state, width, height, 'rgba16float');
}
...
export const defaultWebGPUDisplayObjectVelocityWriter: WebGPUVelocityWriter = (ctx, node) => {
  ...
  drawWebGPUVelocityQuad(ctx, bounds.x, bounds.y, bounds.width, bounds.height, _scratchVelocity.x, _scratchVelocity.y);
};
...
// Velocity stays in node units; drawWebGPUVelocityQuad applies pixelRatio (matching the other writers).
...
export const defaultWebGPUParticleEmitterVelocityWriter: WebGPUVelocityWriter = (ctx, node) => {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    drawWebGPUVelocityQuad(ctx, minX, minY, maxX - minX, maxY - minY, velocityX, velocityY);
  }
};
...
// bounds). Velocity stays in node units; drawWebGPUVelocityQuad applies pixelRatio. Fallback: when no
// per-instance array is present, cover the batch's world bounds with one coarse field velocity, like the
// DisplayObject writer. Identical math to defaultWebGLQuadBatchVelocityWriter.
export const defaultWebGPUQuadBatchVelocityWriter: WebGPUVelocityWriter = (ctx, node) => {
 ...
 * node units (scaled by pixelRatio here). The velocity pass must be active — renderWebGPUVelocity sets it
 ...
 */
export function drawWebGPUVelocityQuad(
  ctx: Readonly<WebGPUVelocityContext>,
  x: number,
  y: number,
  width: number,
  height: number,
...
export function getWebGPUVelocityWriter(state: WebGPURenderState, kind: symbol): WebGPUVelocityWriter | null {
  return _velocityWriters.get(state)?.get(kind) ?? null;
}

export function registerWebGPUVelocityWriter(
  state: WebGPURenderState,
  kind: symbol,
  writer: WebGPUVelocityWriter,
): void {
 ...
 * registered WebGPUVelocityWriter for each node's kind. Nodes whose kind has no writer are skipped; the
 * cleared (0,0,0,0) background means zero velocity. Runs inside the frame's command encoder (open one with
 * renderWebGPUBackground first), as its own render pass; ends any pass currently open and leaves none open.
 */
export function renderWebGPUVelocity<Traits extends object>(
  state: WebGPURenderState,
  ...
  if (runtime.commandEncoder === null) {
    throw new Error('No active command encoder — call renderWebGPUBackground before renderWebGPUVelocity.');
  }
  ...
  const pipeline = ensureWebGPUVelocityPipeline(state);
  pipeline.cursor = 0;
  ...
  const ctx: WebGPUVelocityContext = {
    state,
    field,
    width: target.width,
  ...
  visitWebGPUVelocity(ctx, root);

  pass.end();
  _activeVelocityPasses.delete(state);
}

function ensureWebGPUVelocityPipeline(state: WebGPURenderState): WebGPUVelocityPipeline {
  ...
  const entry: WebGPUVelocityPipeline = {
    pipeline,
    uniformBuffer,
    bindGroup,
    cursor: 0,
...
function visitWebGPUVelocity<Traits extends object>(
  ctx: Readonly<WebGPUVelocityContext>,
  node: Readonly<Transform2DNode<Traits>>,
): void {
  const writer = getWebGPUVelocityWriter(ctx.state, node.kind);
  if (writer !== null) writer(ctx, node);

  const count = getNodeChildCount(node);
  for (let i = 0; i < count; i++) {
    const child = getNodeChildAt(node, i);
    if (child !== null) visitWebGPUVelocity(ctx, child as unknown as Readonly<Transform2DNode<Traits>>);
  }
}

interface WebGPUVelocityPipeline {
...
// velocity pass (set for the duration of renderWebGPUVelocity so drawWebGPUVelocityQuad can reach it from
// just the public context). WeakMaps so all release when the state is GC'd.
const _velocityPipelines = new WeakMap<WebGPURenderState, WebGPUVelocityPipeline>();
const _velocityWriters = new WeakMap<WebGPURenderState, Map<symbol, WebGPUVelocityWriter>>();
const _activeVelocityPasses = new WeakMap<
  WebGPURenderState,
  { pass: GPURenderPassEncoder; pipeline: WebGPUVelocityPipeline }
>();
