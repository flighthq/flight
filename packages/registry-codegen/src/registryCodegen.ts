...
export function createRegistryCodegenPlan(
  ...
  const out = allocateEntity<RegistryCodegenPlan & Entity>();
  initializeRegistryCodegenPlan(out, catalog, requirements, backend);
...
export function initializeRegistryCodegenPlan(
  out: EntityConstruction<RegistryCodegenPlan & Entity>,
  catalog: Readonly<RegistryCatalog>,
  requirements: Readonly<RequirementSet>,
  backend: string,
): void {
