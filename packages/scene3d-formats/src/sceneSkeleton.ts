...
// this internally; for MULTIPLE clips against one mesh, `createScene3DFromMd5Mesh` produces a scene with no
// animation, and this finds the joints each `parseMd5Anim` binds to:
//
//   const scene = createScene3DFromMd5Mesh(meshSource);
...
export function findScene3DSkeletonJoints(root: Readonly<Node3D>): readonly Node3D[] | null {
  const stack: Node3D[] = [...getNodeChildren(root)];
