...
describe('createTweenTimer', () => {
  it('fires onComplete after the specified duration', () => {
    ...
    const timer = createTweenTimer(manager, 1000);
    let fired = 0;
  ...
  it('respects the delay option', () => {
    const manager = createTweenManager();
    const timer = createTweenTimer(manager, 1000, { delay: 500 });
    let fired = 0;
    connectSignal(timer.onComplete, () => fired++);
    updateTweens(manager, 1000);
    expect(fired).toBe(0);
  ...
  it('each call produces an independent sentinel target', () => {
    const manager = createTweenManager();
    const t1 = createTweenTimer(manager, 1000);
    const t2 = createTweenTimer(manager, 1000);
  ...
  it('fires onRepeat and keeps running when repeat: -1', () => {
    const manager = createTweenManager();
    const timer = createTweenTimer(manager, 1000, { repeat: -1 });
    let ticks = 0;
