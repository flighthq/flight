...
// texture.
//
// NESTING MODEL — Wgpu cannot clear the stencil mid render pass (the pass clears it once at start,
...
export function pushWgpuClipContours(
...
// Expands each contour's triangle fan (origin, i, i+1) into a triangle-list vertex buffer — Wgpu has
...
function ensureClipContourPipelines(state: WgpuRenderState): WgpuClipContourPipelines {
  const runtime = getWgpuRenderStateRuntime(state);
  ...
  const pipelines: WgpuClipContourPipelines = {
