...
export function createCapacitorMessageDialogBackend(capacitor: CapacitorApi): HostMessageDialogProvider {
  const out = allocateEntity<HostMessageDialogProvider>();
  initializeCapacitorMessageDialogBackend(out, capacitor);
  return finishEntity(out);
}

export function createCapacitorPromptDialogBackend(capacitor: CapacitorApi): HostPromptDialogProvider {
