...
afterEach(async () => {
  ...
  await webHostPowerKeepAwake.release();
  vi.unstubAllGlobals();
...
describe('webHostPowerKeepAwake', () => {
  it('reports unavailable when the Wake Lock API is absent', async () => {
    setNavigator(undefined);
    await expect(webHostPowerKeepAwake.acquire('PreventDisplaySleep')).resolves.toEqual({ reason: 'unavailable' });
  });

  it('reports unavailable for a mode the web cannot honour', async () => {
    setNavigator({ request: async () => sentinel() });
    await expect(webHostPowerKeepAwake.acquire('PreventAppSuspension')).resolves.toEqual({ reason: 'unavailable' });
  });

  it('reports ok and becomes active only after the request resolves', async () => {
    setNavigator({ request: async () => sentinel() });
    await expect(webHostPowerKeepAwake.acquire('PreventDisplaySleep')).resolves.toEqual({ reason: 'ok' });
    expect(webHostPowerKeepAwake.isActive()).toBe(true);
  });
  ...
  it('reports a platform refusal as denied and does not become active', async () => {
    ...
    await expect(webHostPowerKeepAwake.acquire('PreventDisplaySleep')).resolves.toEqual({ reason: 'denied' });
    expect(webHostPowerKeepAwake.isActive()).toBe(false);
  });
  ...
  it('separates an operation failure from a policy denial', async () => {
    ...
    await expect(webHostPowerKeepAwake.acquire('PreventDisplaySleep')).resolves.toEqual({ reason: 'failed' });
  });
  ...
  it('clears state immediately when the OS releases the lock', async () => {
    ...
    await webHostPowerKeepAwake.acquire('PreventDisplaySleep');
    expect(webHostPowerKeepAwake.isActive()).toBe(true);
    onRelease!();
    expect(webHostPowerKeepAwake.isActive()).toBe(false);
  });
  ...
  it('reports inactive rather than ok when nothing is held', async () => {
    setNavigator({ request: async () => sentinel() });
    await expect(webHostPowerKeepAwake.release()).resolves.toEqual({ reason: 'inactive' });
  });
  ...
  it('reports a failed release and keeps reporting the lock as held', async () => {
    ...
    await webHostPowerKeepAwake.acquire('PreventDisplaySleep');
    await expect(webHostPowerKeepAwake.release()).resolves.toEqual({ reason: 'failed' });
    expect(webHostPowerKeepAwake.isActive()).toBe(true);
  });
  ...
  it('reports ok and becomes inactive after a successful release', async () => {
    setNavigator({ request: async () => sentinel() });
    await webHostPowerKeepAwake.acquire('PreventDisplaySleep');
    await expect(webHostPowerKeepAwake.release()).resolves.toEqual({ reason: 'ok' });
    expect(webHostPowerKeepAwake.isActive()).toBe(false);
  });
});

describe('webHostPowerSuspension', () => {
  it('registers the Page Lifecycle pair and removes both on unsubscribe', () => {
    ...
    const stopSuspend = webHostPowerSuspension.subscribeSuspend(() => {});
    const stopResume = webHostPowerSuspension.subscribeResume(() => {});
