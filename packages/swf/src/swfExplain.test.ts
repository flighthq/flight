...
describe('explainSwfContent', () => {
  it('returns null for empty input', () => {
    expect(explainSwfContent(new Uint8Array(), DECOMPRESS_DEFLATE, DECOMPRESS_LZMA)).toBeNull();
  });

  it('returns null for input too small to be a valid SWF', () => {
    expect(explainSwfContent(new Uint8Array([0x46, 0x57, 0x53]), DECOMPRESS_DEFLATE, DECOMPRESS_LZMA)).toBeNull();
  });
  ...
  it('returns a manifest for a minimal valid SWF', () => {
    ...
    const manifest = explainSwfContent(swf, DECOMPRESS_DEFLATE, DECOMPRESS_LZMA);
  ...
  it('preserves a fractional 8.8 frame rate', () => {
    const manifest = explainSwfContent(createSwf([createTag(TAG_END)], 23.5), DECOMPRESS_DEFLATE, DECOMPRESS_LZMA)!;
  ...
  it('uses the declared file length as the tag-stream boundary', () => {
    ...
    const manifest = explainSwfContent(withTrailingTag, DECOMPRESS_DEFLATE, DECOMPRESS_LZMA)!;
  ...
  it('rejects invalid declared lengths and truncated tag records', () => {
    ...
    expect(explainSwfContent(tooLong, DECOMPRESS_DEFLATE, DECOMPRESS_LZMA)).toBeNull();

    const tooShort = createSwf([createTag(TAG_END)]);
    tooShort.set(uint32(11), 4);
    expect(explainSwfContent(tooShort, DECOMPRESS_DEFLATE, DECOMPRESS_LZMA)).toBeNull();

    const zeroVersion = createSwf([createTag(TAG_END)]);
    zeroVersion[3] = 0;
    expect(explainSwfContent(zeroVersion, DECOMPRESS_DEFLATE, DECOMPRESS_LZMA)).toBeNull();
  ...
  it('returns the same manifest for registered compressed input', () => {
    ...
    expect(explainSwfContent(compressed, null, null)).toBeNull();
    const reversing = {
    ...
    expect(explainSwfContent(compressed, reversing, null)).toEqual(
      explainSwfContent(uncompressed, DECOMPRESS_DEFLATE, DECOMPRESS_LZMA),
    );
  });
});
