...
describe('explainSwfContent', () => {
  ...
  it('marks unhandled tags correctly', () => {
    const swf = createSwf([createTag(TAG_PROTECT, new Uint8Array([0])), createTag(TAG_SHOW_FRAME), createTag(TAG_END)]);
    const manifest = explainSwfContent(swf)!;
    const protectEntry = manifest.entries.find((e) => e.code === TAG_PROTECT);
    expect(protectEntry).toBeDefined();
    expect(protectEntry!.handled).toBe(false);
  });
