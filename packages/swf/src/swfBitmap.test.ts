...
describe('createSwfJpegAlphaBitmap', () => {
  it('replaces the decoded alpha plane with real deflate output while preserving straight RGB', () => {
    ...
    const bitmap = createSwfJpegAlphaBitmap(decoded, jpegAlphaPayload(3, 1, alpha), sdkHostDecompressDeflate);
  ...
  it('rejects decoded dimensions that differ from the retained SWF dimensions', () => {
    ...
    expect(createSwfJpegAlphaBitmap(decodedImage(1, 1), payload, sdkHostDecompressDeflate)).toBeNull();
    expect(createSwfJpegAlphaBitmap(decodedImage(2, 2), payload, sdkHostDecompressDeflate)).toBeNull();
  });

  it('rejects short and long decompressed alpha planes', () => {
    const decoded = decodedImage(2, 1);

    expect(createSwfJpegAlphaBitmap(decoded, jpegAlphaPayload(2, 1, [0]), sdkHostDecompressDeflate)).toBeNull();
    expect(
      createSwfJpegAlphaBitmap(decoded, jpegAlphaPayload(2, 1, [0, 0x80, 0xff]), sdkHostDecompressDeflate),
    ).toBeNull();
  });
});
...
describe('createSwfLosslessBitmap', () => {
  it('unpacks a 24-bit image, discarding the padding byte each pixel carries', () => {
    ...
    const bitmap = createSwfLosslessBitmap(
      payload(5, 2, 1, stored([0, 0x11, 0x22, 0x33, 0, 0x44, 0x55, 0x66])),
      false,
  ...
  it('reads the alpha form as premultiplied, which is how the format stores it', () => {
    const bitmap = createSwfLosslessBitmap(
      payload(5, 1, 1, stored([0x80, 0x40, 0x20, 0x10])),
      true,
      sdkHostDecompressDeflate,
    );
  ...
  it('resolves colour-mapped pixels through the table that precedes them', () => {
    ...
    const bitmap = createSwfLosslessBitmap(payloadWithTable(3, 2, 1, 2, stored(body)), false, sdkHostDecompressDeflate);
  ...
  it('scales fifteen-bit channels up rather than leaving them dark', () => {
    ...
    const bitmap = createSwfLosslessBitmap(
      payload(4, 1, 1, stored([0x7f, 0xff, 0, 0])),
      false,
      sdkHostDecompressDeflate,
    );
  ...
  it('reports nothing for a payload whose pixels run short', () => {
    expect(
      createSwfLosslessBitmap(payload(5, 64, 64, stored([0, 1, 2, 3])), false, sdkHostDecompressDeflate),
    ).toBeNull();
    expect(createSwfLosslessBitmap(new Uint8Array([5, 1]), false, sdkHostDecompressDeflate)).toBeNull();
  });

  it('bounds lossless expansion to the exact row layout and requests zlib framing', () => {
    const spy: HostDecompressDeflateCapability = {
      decompress: (_compressed, uncompressedLength, framing) => {
    ...
    expect(createSwfLosslessBitmap(payload(5, 3, 1, [0]), false, spy)).not.toBeNull();
  });
});
