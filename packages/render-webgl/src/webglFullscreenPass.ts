...
export function clearWebGLRenderTarget(state: WebGLRenderState, target: WebGLRenderTarget): void {
...
export function compileWebGLFullscreenProgram(
  gl: WebGL2RenderingContext,
  fragmentSource: string,
): WebGLFullscreenProgram {
  const vs = compileWebGLShader(gl, gl.VERTEX_SHADER, FULLSCREEN_VERTEX_SRC);
 ...
 */
export function drawWebGLFullscreenPass(
  state: WebGLRenderState,
  program: Readonly<WebGLFullscreenProgram>,
  inputs: ReadonlyArray<WebGLTexture>,
  dest: Readonly<WebGLRenderTarget> | null,
  setUniforms: (gl: WebGL2RenderingContext, program: Readonly<WebGLFullscreenProgram>) => void,
): void {
  const runtime = getWebGLRenderStateRuntime(state);
  const gl = state.gl;
  ...
  drawWebGLFullscreenQuad(state, program);
}

function compileWebGLShader(gl: WebGL2RenderingContext, type: number, src: string): WebGLShader {
...
function drawWebGLFullscreenQuad(state: WebGLRenderState, program: Readonly<WebGLFullscreenProgram>): void {
  const runtime = getWebGLRenderStateRuntime(state);
  const gl = state.gl;
