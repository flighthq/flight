...
export function prepareWebGLDisplayObjectRender(state: WebGLRenderState, source: DisplayObject): boolean {
...
export function prepareWebGLSpriteRender(state: WebGLRenderState, source: SpriteNode): boolean {
  beginFrame(state);
  const pool = state.commandPool;
  resetRenderCommandPool(pool);
...
export function renderWebGL(state: WebGLRenderState): void {
  useWebGLProgram(state as WebGLRenderStateInternal);
