...
// clip when ANDing is needed (documented in popWebGLClipContours).
//
// Verified via the `mask` functional capture (canvas/dom/webgl agree). The clip program must keep
// state.currentProgram in sync (see pushWebGLClipContours) so content draws re-bind their own program.
...
export function pushWebGLClipContours(
  ...
  // disturb the coverage. The gate persists through the content draws until popWebGLClipContours.
