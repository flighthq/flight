import { getTextShaperBackend, shapeText } from '@flighthq/textshaper';
...
// backend from @flighthq/textshaper-canvas, or a future HarfBuzz one), this returns shapeText, which
// is itself a TextMeasureFunction. An explicitly set provider (setTextLayoutMeasureProvider) still
...
export function getTextLayoutMeasureProvider(): TextMeasureFunction | null {
  ...
  if (getTextShaperBackend() !== null) return shapeText;
  return null;
}
