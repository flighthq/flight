...
describe('glExtendedPbrMeshMaterialRenderer', () => {
  it('binds the lean no-extension variant', () => {
    const { state, gl } = makeGlScene3DState();
    glExtendedPbrMeshMaterialRenderer.bind(state, createExtendedPbrMaterial(), makeLights(), makeCamera());
...
describe('registerGlExtendedPbrMaterial', () => {
  it('registers the one generic material renderer', () => {
    ...
    expect(getGlMeshMaterialRenderer(state, ExtendedPbrMaterialKind)).toBe(glExtendedPbrMeshMaterialRenderer);
  });
});
