...
import {
  ...
  glSpecularGlossinessPbrMeshMaterialRenderer,
} from './glSpecularGlossinessPbrMeshMaterialRenderer.ts';
...
describe('glSpecularGlossinessPbrMeshMaterialRenderer', () => {
  it('bind converts spec-gloss to a standard block and uploads it through the base PBR program', () => {
    const { state, gl } = makeGlScene3DState();
    glSpecularGlossinessPbrMeshMaterialRenderer.bind(
      state,
  ...
  it('draw issues an indexed draw after bind', () => {
    ...
    glSpecularGlossinessPbrMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    glSpecularGlossinessPbrMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
    expect(gl.calls.some((c) => c.name === 'drawElements')).toBe(true);
  });

  it('draw is a no-op before bind', () => {
    const { state, gl } = makeGlScene3DState();
    glSpecularGlossinessPbrMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
