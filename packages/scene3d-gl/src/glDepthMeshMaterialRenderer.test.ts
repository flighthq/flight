...
import { glDepthMeshMaterialRenderer, registerGlDepthMaterial } from './glDepthMeshMaterialRenderer.ts';
...
describe('glDepthMeshMaterialRenderer', () => {
  it('bind selects a program and uploads depth state, view matrices, and the near/far range', () => {
    const { state, gl } = makeGlScene3DState();
    const camera = makeCamera();
    glDepthMeshMaterialRenderer.bind(state, createDepthMaterial(), NO_LIGHTS, camera);
  ...
  it('draw issues an indexed draw over the subset after bind', () => {
    ...
    glDepthMeshMaterialRenderer.bind(state, proxy.material, NO_LIGHTS, makeCamera());
    glDepthMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind has not selected a program', () => {
    const { state, gl } = makeGlScene3DState();
    glDepthMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
...
describe('registerGlDepthMaterial', () => {
  it('installs the renderer for DepthMaterialKind', () => {
    ...
    expect(getGlMeshMaterialRenderer(state, DepthMaterialKind)).toBe(glDepthMeshMaterialRenderer);
  });
});
