...
import { createLambertMaterial } from '@flighthq/materials/contract';
...
import { glLambertMeshMaterialRenderer, registerGlLambertMaterial } from './glLambertMeshMaterialRenderer.ts';
...
describe('glLambertMeshMaterialRenderer', () => {
  it('bind selects a program, sets depth/cull, and uploads light block + diffuse color', () => {
    const { state, gl } = makeGlScene3DState();
    glLambertMeshMaterialRenderer.bind(state, createLambertMaterial(), makeLights(), makeCamera());
  ...
  it('bind disables back-face culling for a double-sided material', () => {
    ...
    glLambertMeshMaterialRenderer.bind(state, material, makeLights(), makeCamera());
  ...
  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    ...
    glLambertMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    glLambertMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind has not selected a program', () => {
    const { state, gl } = makeGlScene3DState();
    glLambertMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
...
describe('registerGlLambertMaterial', () => {
  it('installs the renderer for LambertMaterialKind', () => {
    ...
    expect(getGlMeshMaterialRenderer(state, LambertMaterialKind)).toBe(glLambertMeshMaterialRenderer);
  });
});
