...
describe('glNormalMeshMaterialRenderer', () => {
  it('bind selects a program, sets depth/cull, view-projection, and the normal scale uniform', () => {
    const { state, gl } = makeGlScene3DState();
    glNormalMeshMaterialRenderer.bind(state, createNormalMaterial(), NO_LIGHTS, makeCamera());
  ...
  it('draw issues an indexed draw over the subset after bind', () => {
    ...
    glNormalMeshMaterialRenderer.bind(state, proxy.material, NO_LIGHTS, makeCamera());
    glNormalMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind has not selected a program', () => {
    const { state, gl } = makeGlScene3DState();
    glNormalMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
...
describe('registerGlNormalMaterial', () => {
  it('installs the renderer for NormalMaterialKind', () => {
    ...
    expect(getGlMeshMaterialRenderer(state, NormalMaterialKind)).toBe(glNormalMeshMaterialRenderer);
  });
});
