...
describe('glShadedMeshMaterialRenderer', () => {
  it('bind selects a program and uploads camera + light block + base colors + time', () => {
    const { state, gl } = makeGlScene3DState();
    glShadedMeshMaterialRenderer.bind(state, createShadedMaterial(), makeLights(), makeCamera());
  ...
  it('binds modifier uniforms for a material carrying a modifier stack', () => {
    ...
    glShadedMeshMaterialRenderer.bind(state, material, makeLights(), makeCamera());
  ...
  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    ...
    glShadedMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    glShadedMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind has not selected a program', () => {
    const { state, gl } = makeGlScene3DState();
    glShadedMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
...
describe('registerGlShadedMaterial', () => {
  it('installs the renderer for ShadedMaterialKind', () => {
    ...
    expect(getGlMeshMaterialRenderer(state, ShadedMaterialKind)).toBe(glShadedMeshMaterialRenderer);
  });
});
