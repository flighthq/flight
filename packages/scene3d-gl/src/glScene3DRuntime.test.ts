...
describe('ensureGlInstancePalette', () => {
  it('creates the instance palette lazily and reuses it after', () => {
    const { state } = makeGlScene3DState();
    expect(getGlScene3DRuntime(state).instancePalette).toBeNull();

    const first = ensureGlInstancePalette(state);
    expect(first).toBe(getGlScene3DRuntime(state).instancePalette);
    expect(ensureGlInstancePalette(state)).toBe(first);
  });
});
...
describe('getGlScene3DRuntime', () => {
  ...
  it('surfaces its upload cache without replacing persistent material dispatch policy', () => {
    ...
    const modifierSnippetRevision = stateRuntime.modifierSnippetRevision;
    const pbrExtensions = stateRuntime.registries.pbrExtensions;
    const pbrExtensionRevision = stateRuntime.pbrExtensionRevision;
    const scene = getGlScene3DRuntime(state);
    ...
    expect(stateRuntime.modifierSnippetRevision).toBe(modifierSnippetRevision);
    expect(stateRuntime.registries.pbrExtensions).toBe(pbrExtensions);
    expect(stateRuntime.pbrExtensionRevision).toBe(pbrExtensionRevision);
