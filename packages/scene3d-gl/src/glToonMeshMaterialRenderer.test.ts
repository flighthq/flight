...
describe('glToonMeshMaterialRenderer', () => {
  it('bind selects a program, sets depth/cull state, and uploads camera + light block + base color', () => {
    const { state, gl } = makeGlScene3DState();
    glToonMeshMaterialRenderer.bind(state, createToonMaterial(), makeLights(), makeCamera());
  ...
  it('bind disables back-face culling for a double-sided material', () => {
    ...
    glToonMeshMaterialRenderer.bind(state, material, makeLights(), makeCamera());
  ...
  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    ...
    const geometry = createBoxMeshGeometry();
    glToonMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    glToonMeshMaterialRenderer.draw(state, proxy, geometry);
  ...
  it('draw is a no-op when bind has not selected a program', () => {
    const { state, gl } = makeGlScene3DState();
    glToonMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
...
describe('registerGlToonMaterial', () => {
  it('installs the renderer for ToonMaterialKind', () => {
    ...
    expect(getGlMeshMaterialRenderer(state, ToonMaterialKind)).toBe(glToonMeshMaterialRenderer);
  });
});
