...
import { glEmissiveMeshMaterialRenderer, registerGlEmissiveMaterial } from './glEmissiveMeshMaterialRenderer.ts';
...
describe('glEmissiveMeshMaterialRenderer', () => {
  it('bind uploads the emissive color scaled by emissiveStrength via the intensity uniform', () => {
    ...
    glEmissiveMeshMaterialRenderer.bind(state, material, NO_LIGHTS, makeCamera());
  ...
  it('draw issues an indexed draw over the subset after bind', () => {
    ...
    glEmissiveMeshMaterialRenderer.bind(state, proxy.material, NO_LIGHTS, makeCamera());
    glEmissiveMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
...
describe('registerGlEmissiveMaterial', () => {
  it('installs the renderer for EmissiveMaterialKind', () => {
    ...
    expect(getGlMeshMaterialRenderer(state, EmissiveMaterialKind)).toBe(glEmissiveMeshMaterialRenderer);
  });
});
