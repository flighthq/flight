...
describe('glCustomShaderMeshMaterialRenderer', () => {
  it('bind selects a program and uploads view-projection and camera position when shader is registered', () => {
    ...
    glCustomShaderMeshMaterialRenderer.bind(
      state,
  ...
  it('bind is a no-op when shaderKey is empty', () => {
    const { state, gl } = makeGlScene3DState();
    glCustomShaderMeshMaterialRenderer.bind(state, createCustomShaderMaterial(), NO_LIGHTS, makeCamera());
    expect(gl.calls.some((c) => c.name === 'useProgram')).toBe(false);
  });
  ...
  it('draw issues an indexed draw over the subset after bind', () => {
    ...
    glCustomShaderMeshMaterialRenderer.bind(state, proxy.material, NO_LIGHTS, makeCamera());
    glCustomShaderMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind skipped due to missing shader key', () => {
    ...
    glCustomShaderMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
...
describe('registerGlCustomShaderMaterial', () => {
  it('installs the renderer for CustomShaderMaterialKind', () => {
    ...
    expect(getGlMeshMaterialRenderer(state, CustomShaderMaterialKind)).toBe(glCustomShaderMeshMaterialRenderer);
  });
});
