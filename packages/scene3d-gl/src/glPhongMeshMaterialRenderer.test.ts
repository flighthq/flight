...
describe('glPhongMeshMaterialRenderer', () => {
  it('bind selects a program, uploads camera position + light block + diffuse/specular colors', () => {
    const { state, gl } = makeGlScene3DState();
    glPhongMeshMaterialRenderer.bind(state, createPhongMaterial(), makeLights(), makeCamera());
  ...
  it('bind disables back-face culling for a double-sided material', () => {
    ...
    glPhongMeshMaterialRenderer.bind(state, material, makeLights(), makeCamera());
  ...
  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    ...
    glPhongMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    glPhongMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind has not selected a program', () => {
    const { state, gl } = makeGlScene3DState();
    glPhongMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
...
describe('registerGlPhongMaterial', () => {
  it('installs the renderer for PhongMaterialKind', () => {
    ...
    expect(getGlMeshMaterialRenderer(state, PhongMaterialKind)).toBe(glPhongMeshMaterialRenderer);
  });
});
