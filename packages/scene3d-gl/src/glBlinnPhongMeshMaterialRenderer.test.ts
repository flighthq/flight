...
import { glBlinnPhongMeshMaterialRenderer, registerGlBlinnPhongMaterial } from './glBlinnPhongMeshMaterialRenderer.ts';
...
describe('glBlinnPhongMeshMaterialRenderer', () => {
  it('bind selects a program, uploads camera position + light block + diffuse/specular colors', () => {
    const { state, gl } = makeGlScene3DState();
    glBlinnPhongMeshMaterialRenderer.bind(state, createBlinnPhongMaterial(), makeLights(), makeCamera());
  ...
  it('bind disables back-face culling for a double-sided material', () => {
    ...
    glBlinnPhongMeshMaterialRenderer.bind(state, material, makeLights(), makeCamera());
  ...
  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    ...
    glBlinnPhongMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    glBlinnPhongMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind has not selected a program', () => {
    const { state, gl } = makeGlScene3DState();
    glBlinnPhongMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
...
describe('registerGlBlinnPhongMaterial', () => {
  it('installs the renderer for BlinnPhongMaterialKind', () => {
    ...
    expect(getGlMeshMaterialRenderer(state, BlinnPhongMaterialKind)).toBe(glBlinnPhongMeshMaterialRenderer);
  });
});
