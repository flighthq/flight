...
describe('glWireframeMeshMaterialRenderer', () => {
  it('bind selects the program, disables culling, and uploads the line color', () => {
    const { state, gl } = makeGlScene3DState();
    glWireframeMeshMaterialRenderer.bind(state, createWireframeMaterial(), NO_LIGHTS, makeCamera());
  ...
  it('bind selects the masked program and uploads alphaCutoff', () => {
    ...
    glWireframeMeshMaterialRenderer.bind(state, material, NO_LIGHTS, makeCamera());
  ...
  it('draw is a no-op when bind has not selected a program', () => {
    const { state, gl } = makeGlScene3DState();
    glWireframeMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
...
describe('registerGlWireframeMaterial', () => {
  it('installs the renderer for WireframeMaterialKind', () => {
    ...
    expect(getGlMeshMaterialRenderer(state, WireframeMaterialKind)).toBe(glWireframeMeshMaterialRenderer);
  });
});
