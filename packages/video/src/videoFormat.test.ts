...
describe('canPlayVideoType', () => {
  ...
  it('accepts only primitive true from a backend', () => {
    expect(canPlayVideoType({ canPlayType: () => 'probably' } as unknown as HostVideoCapability, 'video/mp4')).toBe(
      false,
    );
  });

  it('normalizes backend exceptions to false', () => {
    const backend: HostVideoCapability = {
      canPlayType(): boolean {
...
describe('selectVideoResourceUrl', () => {
  ...
  it('picks the first source whose inferred type is playable', () => {
    const backend: HostVideoCapability = { canPlayType: (type) => type === 'video/webm' };
  ...
  it('honours an explicit type over the URL extension', () => {
    const backend: HostVideoCapability = { canPlayType: (type) => type === 'video/mp4' };
  ...
  it('continues after invalid truthy results and backend exceptions', () => {
    ...
    const selected = selectVideoResourceUrl({ canPlayType } as HostVideoCapability, [
      { url: 'first.mp4' },
      { url: 'second.webm' },
      { url: 'third.ogv' },
    ]);
