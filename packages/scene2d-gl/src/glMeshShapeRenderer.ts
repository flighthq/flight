...
import { createGlShapeData, destroyGlShapeData, getGlShapeData } from './glShapeData.ts';
import { drawGlShapeMeshes } from './glShapeMesh.ts';
...
export function drawGlMeshShape(state: GlRenderState, renderProxy: RenderProxy2D): boolean {
  ...
  const tessellatorEntry = getGlRenderStateRuntime(state).registries.strokeTessellator;
  const strokePathTessellator = tessellatorEntry ?? null;
