...
import { applyWgpuBlitPass } from './tintShader';
...
const boxBlurPipelines = new WeakMap<WgpuRenderState, WgpuFilterPipeline>();
const gaussianBlurPipelines = new WeakMap<WgpuRenderState, WgpuFilterPipeline>();

function getBoxBlurPipeline(state: WgpuRenderState): WgpuFilterPipeline {
  let p = boxBlurPipelines.get(state);
  if (p === undefined) {
    p = createWgpuFilterPipeline(state, BOX_BLUR_WGSL, 'replace');
    boxBlurPipelines.set(state, p);
  }
  return p;
}

function getGaussianBlurPipeline(state: WgpuRenderState): WgpuFilterPipeline {
  let p = gaussianBlurPipelines.get(state);
  if (p === undefined) {
    p = createWgpuFilterPipeline(state, GAUSSIAN_BLUR_WGSL, 'replace');
 ...
 * `applyGaussianBlurFilterToWgpu`. `temp` is a caller-provided ping-pong scratch
 * target distinct from both `source` and `dest`.
 */
export function applyBoxBlurFilterToWgpu(
  state: WgpuRenderState,
  source: WgpuRenderTarget,
  dest: WgpuRenderTarget,
  ...
  if (read !== dest) {
    applyWgpuBlitPass(state, read, dest);
  }
}
 ...
 * `source` and `dest`.
 */
export function applyGaussianBlurFilterToWgpu(
  state: WgpuRenderState,
  source: WgpuRenderTarget,
  dest: WgpuRenderTarget,
  temp: WgpuRenderTarget,
