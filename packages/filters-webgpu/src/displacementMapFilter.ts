...
//
// Note: scaleY is negated versus the WebGL implementation because WebGPU UV y=0
// is the top of the texture (matches screen Y-down), while WebGL UV y=0 is the bottom.
//
...
function getPipeline(state: WebGPURenderState): WebGPUDualSourcePipeline {
  ...
  if (p === undefined) {
    p = createWebGPUDualSourcePipeline(state, DISPLACEMENT_MAP_WGSL, 'replace');
    shaders.set(state, p);
  }
  return p;
}
 ...
 * targets needed.
 *
 * Note: the Y displacement is internally negated versus the WebGL implementation
 * to account for WebGPU's top-left UV origin.
 */
export function applyDisplacementMapFilterToWebGPU(
  state: WebGPURenderState,
  source: WebGPURenderTarget,
  map: WebGPURenderTarget,
  ...
  drawWebGPUDualSourcePass(state, source, map, dest, pipeline, (f32, i32) => {
    f32[0] = 1 / source.width;
    f32[1] = 1 / source.height;
    f32[2] = filter.scaleX ?? 0;
    // Negate Y scale: WebGPU UV y=0 is top (Y-down matches screen), WebGL UV y=0 is bottom.
