...
import { installWebGPUMock, makeFilterState } from './testHelper';

installWebGPUMock();

describe('createWebGPUGradientRampTexture', () => {
  it('creates a 256×1 texture from a two-stop gradient', async () => {
    ...
    const tex = createWebGPUGradientRampTexture(state, [0x000000, 0xffffff], [1, 1], [0, 255]);
  ...
  it('returns a texture even for an empty gradient', async () => {
    const state = await makeFilterState();
    const tex = createWebGPUGradientRampTexture(state, [], [], []);
    expect(tex).toBeDefined();
  });

  it('creates a texture for a multi-stop gradient', async () => {
    const state = await makeFilterState();
    const tex = createWebGPUGradientRampTexture(state, [0xff0000, 0x00ff00, 0x0000ff], [1, 1, 1], [0, 128, 255]);
    expect(tex).toBeDefined();
  });
});
