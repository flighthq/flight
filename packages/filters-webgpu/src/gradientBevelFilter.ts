...
//
// Note: The Y offset component is negated versus the WebGL implementation so
// that the bevel direction matches across backends (WebGPU UV y=0 is top-left).
//
...
const encodePipelines = new WeakMap<WebGPURenderState, WebGPUFilterPipeline>();
const applyPipelines = new WeakMap<WebGPURenderState, WebGPUFilterPipeline>();

function getEncodePipeline(state: WebGPURenderState): WebGPUFilterPipeline {
...
function getApplyPipeline(state: WebGPURenderState): WebGPUFilterPipeline {
 ...
 *
 * Note: The Y bevel offset is negated versus the WebGL implementation to
 * account for WebGPU's top-left UV origin.
 */
export function applyGradientBevelFilterToWebGPU(
  ...
  // Negate Y: WebGPU UV y=0 is top (Y-down matches screen), WebGL UV y=0 is bottom.
  ...
  const rampTexture = createWebGPUGradientRampTexture(state, filter.colors, filter.alphas, filter.ratios);
  ...
  applyWebGPUBlitPass(state, s1, dest);
}
