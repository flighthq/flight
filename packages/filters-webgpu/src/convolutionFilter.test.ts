...
import { applyConvolutionFilterToWebGPU } from './convolutionFilter';
import { installWebGPUMock, makeFilterState, makeRenderTarget } from './testHelper';

installWebGPUMock();

describe('applyConvolutionFilterToWebGPU', () => {
  it('applies a 3×3 identity kernel without error', async () => {
    ...
    expect(() =>
      applyConvolutionFilterToWebGPU(state, source, dest, {
  ...
  it('applies a 7×7 kernel (maximum supported)', async () => {
    ...
    expect(() => applyConvolutionFilterToWebGPU(state, source, dest, { matrix, matrixX: 7, matrixY: 7 })).not.toThrow();
  });
