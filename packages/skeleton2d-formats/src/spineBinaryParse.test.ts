...
describe('parseSpineSkeletonBinary', () => {
  it('parses the header, bones, and slots of a 4.x file', () => {
    ...
    expect(slots[0]).toMatchObject({ attachment: null, boneIndex: 1, color: 0x80c0ffff, name: 'body' });
  ...
  it('reports how many bytes of the file it did not parse', () => {
    ...
    // The fixture ends exactly after its slots, so nothing is left over — the crumb still fires to record
    // that the importer stopped rather than finished, which is the honest signal while sections are missing.
...
function buildSpineBinary(
  options: { version?: string; nonessential?: boolean; transformMode?: number; darkColor?: number } = {},
): Uint8Array {
  ...
  writeVarint(out, 1); // string table
  ...
  writeVarint(out, 0); // setup attachment: none
