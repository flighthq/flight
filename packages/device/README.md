...
Device identity, hardware, and safe-area insets through explicit web/native providers.

`device` is a host-identity leaf in the platform-integration suite. Every query takes a `HostDeviceProvider` directly. `@flighthq/host-web` exports the ready-made `webHostDevice`; Electron, Tauri, Capacitor, and native shells pass their own provider. There is no module-global provider selection. Snapshot reads fill an `out` value and return it. Unknown or unavailable fields resolve to sentinels (`''` / `-1` / `false`), never throwing.
...
| `getDeviceCapabilities(provider, out)` | Fill `out` with input/hardware capability flags. |
| `getDeviceDisplayMetrics(provider, out)` | Fill `out` with the built-in display metrics. |
| `getDeviceId(provider)` | Return a stable install identifier (resettable, not a hardware serial). |
| `getDeviceInfo(provider, out)` | Fill `out` with the device's identity snapshot. |
| `getSafeAreaInsets(provider, out)` | Fill `out` with safe-area insets, in CSS pixels. |
| `refreshDeviceInfo(provider)` | Invalidate the supplied provider's snapshot cache so the next read is fresh. No-op when it has no cache. |

`@flighthq/host-web` also exports `createWebDeviceBackend()` for a fresh provider and `enableWebSafeAreaInsets()` to mount the opt-in CSS `env(safe-area-inset-*)` probe.

## `DeviceInfo` fields

| Field | Type | Unit | Sentinel | Web provider |
| --- | --- | --- | --- | --- |
...
| Field | Type | Sentinel | Web provider |
| --- | --- | --- | --- |
...
Edge insets keep content clear of notches, rounded corners, and system bars. The Web provider returns zeros until `enableWebSafeAreaInsets()` mounts a live CSS-var probe (one hidden element, no polling).

## `getDeviceId`

The Web provider generates a `crypto.randomUUID()` and persists it to `localStorage` as a stable install id. It returns `''` when no stable id can be formed (SSR, blocked storage, privacy mode). This is an **install** id — it resets when storage is cleared — not a hardware serial.

## Usage

```ts
import { createDeviceInfo, getDeviceInfo, getDeviceId } from '@flighthq/device';
import { webHostDevice } from '@flighthq/host-web';

const info = getDeviceInfo(webHostDevice, createDeviceInfo());
console.log(info.formFactor, info.osName, info.cpuCores);

const installId = getDeviceId(webHostDevice);
```
