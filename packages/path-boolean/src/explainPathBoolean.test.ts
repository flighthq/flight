...
describe('explainOffsetPath', () => {
  it('distinguishes empty, invalid, collapsed, and viable results', () => {
    expect(explainOffsetPath(kernel, createPath(), 1)?.reason).toBe('empty-input');
    ...
    expect(explainOffsetPath(kernel, square, Number.NaN)?.reason).toBe('non-finite-delta');
    expect(explainOffsetPath(kernel, square, -1)?.reason).toBe('collapsed-or-degenerate');
    expect(explainOffsetPath(kernel, square, 1)).toBeNull();
  });
});

describe('explainSimplifyPath', () => {
  it('distinguishes empty input from a viable region', () => {
    expect(explainSimplifyPath(kernel, createPath())?.reason).toBe('empty-input');
    ...
    expect(explainSimplifyPath(kernel, triangle)).toBeNull();
  });
});
