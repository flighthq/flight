...
export function createTauriMenuBackend(tauri: TauriApi): MenuBackend {
  ...
  return {
    ...
    // A fire-and-forget async clear races with the replacement backend's setApplicationMenu: the
    // outgoing destroy's empty-menu promise can settle after the successor installs its real menu,
    // overwriting it with an empty one. Destroy therefore releases JS-owned state only (listener
    // and guard flag); the native menu stays until the replacement backend installs its own.
    destroy() {
      if (destroyed) return;
      destroyed = true;
      selectListener = null;
