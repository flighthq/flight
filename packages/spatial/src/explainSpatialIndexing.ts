...
// `declined` means its bounds were non-finite and it is deliberately not held, `overflow` means it is
// held but too large to bucket, and `cells` reports the bucket count the per-object bound caps.
//
...
export function explainSpatialIndexing(index: Readonly<SpatialIndex>, id: SpatialObjectId): SpatialIndexingExplanation {
