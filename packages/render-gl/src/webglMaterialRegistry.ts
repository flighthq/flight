import type { GlMaterialRenderer, GlRenderState, Material } from '@flighthq/types';
import { DefaultMaterialKind } from '@flighthq/types';

import { getGlRenderStateRuntime } from './webglRenderState';

export function getGlMaterialRenderer(state: GlRenderState, kind: symbol): GlMaterialRenderer | null {
  return getGlRenderStateRuntime(state).materialRendererMap?.get(kind) ?? null;
}

export function registerGlMaterialRenderer(state: GlRenderState, kind: symbol, renderer: GlMaterialRenderer): void {
