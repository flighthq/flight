...
// GPU-origin produced backing. Unlike CPU-origin images this is keyed by the Texture in
// glRenderTexture's hidden target cache; resolving returns its attachment without an upload.
export const glProducedTextureBackingKind: GlTextureBackingKind = (storage: Readonly<TextureStorage>): boolean =>
  storage.target !== undefined;
...
// Installs the produced-target realization. Register after CPU-origin resolvers so a target
// descriptor wins even if a future transitional storage record also carries an image.
