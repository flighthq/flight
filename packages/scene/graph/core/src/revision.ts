...
export function getLocalTransformID<K extends symbol>(target: SceneNode<K> & HasTransform2D<K>): number {
  return getRuntime(target).localTransformID;
}

export function getWorldTransformID<K extends symbol>(target: SceneNode<K> & HasTransform2D<K>): number {
 ...
 */
export function invalidateLocalTransform<K extends symbol>(target: SceneNode<K> & HasTransform2D<K>): void {
 ...
 */
export function invalidateWorldTransformParent<K extends symbol>(target: SceneNode<K> & HasTransform2D<K>): void {
