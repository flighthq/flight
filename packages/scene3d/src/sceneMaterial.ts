...
export function findScene3DMaterialByName(root: Readonly<Node3D>, name: string): Material | null {
  const rootMatch = getNamedNodeMaterial(root, name);
  if (rootMatch !== null) return rootMatch;
  let found: Material | null = null;
  findNode(root, (node) => {
    const match = getNamedNodeMaterial(node as Readonly<Node3D>, name);
...
export function getScene3DMaterials(root: Readonly<Node3D>, out: Material[]): void {
  collectNodeMaterials(root, out);
  forEachNodeDescendant(root, (node) => collectNodeMaterials(node as Readonly<Node3D>, out));
}
...
function collectNodeMaterials(node: Readonly<Node3D>, out: Material[]): void {
...
function getNamedNodeMaterial(node: Readonly<Node3D>, name: string): Material | null {
  const materials = (node as Readonly<Partial<Mesh>>).materials;
