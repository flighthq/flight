...
export function buildScene3DFrustum(out: FrustumLike, viewProjection: Readonly<Matrix4Like>): void {
...
function _cullNode(out: Node3D[], node: Readonly<Node3D>, frustum: Readonly<FrustumLike>): void {
  ...
  if (children !== null) {
    for (let i = 0; i < children.length; i++) {
      _cullNode(out, children[i] as Node3D, frustum);
    }
  }
}
