...
describe('webTextSegmenterBackend', () => {
  ...
  it('segments a ZWJ family emoji as a single grapheme', () => {
    const segments = webTextSegmenterBackend.segment('a👨‍👩‍👧b', 'grapheme');
    expect(segments.map((s: TextSegment) => s.text)).toEqual(['a', '👨‍👩‍👧', 'b']);
  });

  it('reports isWordLike for word granularity and omits it otherwise', () => {
    const words = webTextSegmenterBackend.segment('Hi there', 'word');
    expect(words.every((s: TextSegment) => typeof s.isWordLike === 'boolean')).toBe(true);
    const graphemes = webTextSegmenterBackend.segment('Hi', 'grapheme');
    expect(graphemes.every((s: TextSegment) => s.isWordLike === undefined)).toBe(true);
  });

  it('produces gap-free start/end offsets covering the whole string', () => {
    const segments = webTextSegmenterBackend.segment('abc', 'grapheme');
