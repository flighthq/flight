...
describe('getCamera3DFrustumCorners', () => {
  it('writes 8 world-space corners and returns true', () => {
    const camera = makeCamera();
    const corners = makeCorners();
    const result = getCamera3DFrustumCorners(corners, camera, 1);
    expect(result).toBe(true);
    // Near corners (index 0-3) should be closer to camera than far corners (4-7).
    // Camera3D is at z=10 looking toward z=0, so smaller z values are farther away.
    for (let i = 0; i < 4; i++) {
  ...
  it('near corners and far corners have opposite symmetry in xy', () => {
    const camera = makeCamera();
    const corners = makeCorners();
    getCamera3DFrustumCorners(corners, camera, 1);
