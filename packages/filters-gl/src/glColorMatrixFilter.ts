...
type ColorMatrixShaderLocations = GlFullscreenProgram & {
...
const shaders = new WeakMap<GlRenderState, ColorMatrixShaderLocations>();
 ...
 */
export function applyColorMatrixFilterToGl(
  state: GlRenderState,
  source: GlRenderTarget,
  dest: GlRenderTarget,
  filter: Readonly<Omit<ColorMatrixFilter, 'kind'>>,
): void {
  const { matrix } = filter;
...
function getShader(state: GlRenderState): ColorMatrixShaderLocations {
  let loc = shaders.get(state);
  if (loc === undefined) {
    const gl = state.gl;
    const base = compileGlFullscreenProgram(gl, COLOR_MATRIX_FRAGMENT_SRC);
    loc = {
      ...base,
