...
import type { GlFullscreenProgram, GlRenderState, InnerGlowFilter } from '@flighthq/types';

import { applyBoxBlurFilterToGl } from './blurFilter';
import { applyGlBlitPass, applyGlInvertTintPass } from './tintShader';
 ...
 */
export function applyInnerGlowFilterToGl(
  state: GlRenderState,
  ...
  applyGlInnerClipPass(state, s1, source, s0);
...
function applyGlInnerClipPass(
  ...
  drawGlFullscreenPass(state, loc, [glow.texture, source.texture], dest, () => {});
}
