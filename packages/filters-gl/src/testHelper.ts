import type { GlRenderTarget, GlShaderLocations } from '@flighthq/render-gl';
import type { GlRenderState, GlRenderStateRuntime } from '@flighthq/types';
import { EntityRuntimeKey } from '@flighthq/types';

export function makeFilterState(): { state: GlRenderState; gl: WebGL2RenderingContext } {
  const canvas = document.createElement('canvas');
  canvas.width = 64;
  canvas.height = 64;
  const gl = canvas.getContext('webgl2') as WebGL2RenderingContext;
  const shaderLoc: GlShaderLocations = {
  ...
  } as unknown as GlRenderStateRuntime;
  const state = {
    gl,
    canvas,
    allowSmoothing: true,
...
export function makeRenderTarget(width = 64, height = 64): GlRenderTarget {
...
export function makeScratch(count = 3, width = 64, height = 64): GlRenderTarget[] {
