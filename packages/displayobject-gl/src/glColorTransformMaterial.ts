...
import type { GlColorTransformInstancedShader } from '@flighthq/types';
import { ColorTransformMaterialKind } from '@flighthq/types';

import { bindGlQuadBatchBaseAttributes, setGlQuadBatchWorldAndTexture, useGlQuadBatchProgram } from './glSpriteBatch';
...
export function registerGlColorTransformMaterial(state: GlRenderState): void {
  registerGlMaterialRenderer(state, ColorTransformMaterialKind, colorTransformGlMaterialRenderer);
}
...
export const colorTransformGlMaterialRenderer: GlMaterialRenderer = {
  ...
  packInstance(_state: GlRenderState, materialData: MaterialData | null, out: Float32Array, offset: number): void {
