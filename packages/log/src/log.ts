import type { LogData, LogEntry, LogSink } from '@flighthq/types';
import { LogLevel } from '@flighthq/types';

// Logging is split into two faces of one contract so each consumer tree-shakes its half:
//
//   Emit side  — log and the log* wrappers. Featherweight: each forwards an entry to the
...
// logs.jsonl. Entries at or above the console threshold (setLogConsoleLevel) are ALSO printed
// as a human-readable line via the matching console method. Install with setLogSink.
export function createConsoleCaptureSink(): LogSink {
...
export function log(level: LogLevel, data: LogData, channel: string | null = null): void {
  _sink?.({ level, channel, data });
}

// Emit side. Severity-named convenience wrappers over log (mirrors console.debug/info/warn/error).
export function logDebug(data: LogData, channel: string | null = null): void {
  _sink?.({ level: LogLevel.Debug, channel, data });
}

export function logError(data: LogData, channel: string | null = null): void {
  _sink?.({ level: LogLevel.Error, channel, data });
}

export function logInfo(data: LogData, channel: string | null = null): void {
  _sink?.({ level: LogLevel.Info, channel, data });
}

export function logVerbose(data: LogData, channel: string | null = null): void {
  _sink?.({ level: LogLevel.Verbose, channel, data });
}

export function logWarn(data: LogData, channel: string | null = null): void {
...
function writeConsoleCaptureEntry(entry: Readonly<LogEntry>): void {
