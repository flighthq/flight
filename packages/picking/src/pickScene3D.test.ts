...
function sceneWithCenteredBox(): { scene: Node3D; mesh: Mesh } {
...
describe('pickScene3D', () => {
  it('fills barycentric weights that sum to one', () => {
    ...
    const out = createScene3DHit();

    const hit = pickScene3D(scene, camera, 0, 0, out);
  ...
  it('returns null for a scene containing only non-mesh nodes', () => {
    const camera = makeCamera();
    const scene = createNode3D(Node3DKind);
    // A plain Node3D is not a Mesh; isMesh() returns false for it.
    const node = createNode3D();
    addNodeChild(scene, node);
    const out = createScene3DHit();

    expect(pickScene3D(scene, camera, 0, 0, out)).toBeNull();
  });
  ...
  it('returns null when the ray misses every mesh', () => {
    ...
    expect(pickScene3D(scene, camera, 0.99, 0.99, out)).toBeNull();
  });
  ...
  it('excludes a mesh rejected by the predicate option', () => {
    ...
    const hit = pickScene3D(scene, camera, 0, 0, out, { predicate: (m) => m === meshFar });
  ...
  it('rejects hits beyond maxDistance', () => {
    ...
    expect(pickScene3D(scene, camera, 0, 0, out, { maxDistance: 3 })).toBeNull();
    expect(pickScene3D(scene, camera, 0, 0, out, { maxDistance: 4.5 })).not.toBeNull();
  });
  ...
  it('maps a non-square orthographic viewport without an aspect mismap', () => {
    ...
    const hit = pickScene3D(scene, camera, 0.75, 0, out);

    expect(hit?.node).toBe(mesh);
    expect(hit?.pointX).toBeCloseTo(3, 3);
    expect(hit?.pointZ).toBeCloseTo(1, 3);
    // A pick past the horizontal extent misses.
    expect(pickScene3D(scene, camera, 1.1, 0, out)).toBeNull();
  });
...
describe('pickScene3DAll', () => {
  it('collects both overlapping meshes sorted by ascending distance', () => {
    ...
    invalidateNodeLocalTransform(meshNear);
    addNodeChild(scene, meshNear);
    const outArray: Scene3DHit[] = [];

    const hits = pickScene3DAll(scene, camera, 0, 0, outArray);
  ...
  it('empties the array on a miss', () => {
    const camera = makeCamera();
    const { scene } = sceneWithCenteredBox();
    const outArray: Scene3DHit[] = [createScene3DHit(), createScene3DHit()];

    const hits = pickScene3DAll(scene, camera, 0.99, 0.99, outArray);
...
describe('pickScene3DAllWithRay3D', () => {
  ...
  it('keeps only front faces when cullBackfaces is set', () => {
    ...
    const hits = pickScene3DAllWithRay3D(scene, ray, outArray, { cullBackfaces: true });
...
describe('pickScene3DWithRay3D', () => {
  it('resolves the nearest hit for a world-space ray', () => {
    ...
    const hit = pickScene3DWithRay3D(scene, ray, out, {});
  ...
  it('culls a back-facing triangle but keeps a front-facing one', () => {
    const front = createNode3D(Node3DKind);
    addNodeChild(front, triangleMesh(true));
    const back = createNode3D(Node3DKind);
    addNodeChild(back, triangleMesh(false));
    const out = createScene3DHit();

    expect(pickScene3DWithRay3D(front, makeCenterRay(), out, { cullBackfaces: true })).not.toBeNull();
    expect(pickScene3DWithRay3D(back, makeCenterRay(), out, { cullBackfaces: true })).toBeNull();
    // Double-sided (default) hits either winding.
    expect(pickScene3DWithRay3D(back, makeCenterRay(), out)).not.toBeNull();
  });
  ...
  it('picks the second triangle of an indexed strip with its CCW winding', () => {
    ...
    const hit = pickScene3DWithRay3D(scene, ray, createScene3DHit(), { cullBackfaces: true });
  ...
  it('observes morphed vertices and refreshed broad-phase bounds after explicit update', () => {
    ...
    expect(pickScene3DWithRay3D(scene, ray, out)).toBeNull();
    ...
    expect(pickScene3DWithRay3D(scene, ray, out)?.node).toBe(mesh);
  });
  ...
  it('broad-phases against the posed deformedLocalBounds slot when a deform pass wrote one', () => {
    ...
    // Simulate a deform pass (prepareScene3DSkinning) writing a posed local box far off the ray. The
