...
// node-free fixtures; these cases exercise them through the real ParticleEmitter2D node driven by
// updateParticleEmitter2D, which is only possible from this package.
...
describe('attractor forces via updateParticleEmitter2D', () => {
  it('integrates with the core update so attractors curve trajectories', () => {
    ...
    updateParticleEmitter2D(emitter, state, spawnConfig, 1); // spawn one stationary particle at origin
...
describe('collision settling via updateParticleEmitter2D', () => {
  it('settles a falling particle onto a floor over multiple frames', () => {
    ...
    updateParticleEmitter2D(emitter, state, spawnConfig, 1); // spawn one particle
...
describe('lifetime curves in updateParticleEmitter2D', () => {
  function spawnOne(configOverrides: Parameters<typeof createParticleEmitterConfig>[0]) {
    ...
    updateParticleEmitter2D(emitter, state, config, 1); // spawn one particle
  ...
  it('alphaCurve drives alpha non-linearly over lifetime', () => {
    ...
    updateParticleEmitter2D(emitter, state, config, 0.5); // → mid-life
  ...
  it('colorCurve overrides the start/end gradient', () => {
    ...
    updateParticleEmitter2D(emitter, state, config, 0.5);
  ...
  it('scaleCurve multiplies the spawn scale', () => {
    ...
    updateParticleEmitter2D(emitter, state, config, 0.5); // mid-life → factor 0.5
...
describe('normalized config simulates via updateParticleEmitter2D', () => {
  it('produces a config that simulates without NaN poisoning', () => {
    ...
    const emitter = createParticleEmitter2D({ data: { atlas: makeAtlas() } });
    const state = createParticleEmitterState();
    for (let i = 0; i < 30; i++) updateParticleEmitter2D(emitter, state, safe, 1 / 60);
...
describe('onEmitterComplete signal', () => {
  it('fires once when a finite non-looping emitter completes', () => {
    ...
    updateParticleEmitter2D(emitter, state, config, 1); // spawn + age past duration
    updateParticleEmitter2D(emitter, state, config, 1); // particles die → complete
  ...
  it('does not fire for infinite emitters', () => {
    ...
    const completes: number[] = [];
    signals.onEmitterComplete.emit = () => completes.push(1);
    for (let i = 0; i < 10; i++) updateParticleEmitter2D(emitter, state, config, 1);
...
describe('onParticleDeath signal', () => {
  it('fires with the particle position when a particle dies', () => {
    ...
    updateParticleEmitter2D(emitter, state, config, 1.1); // age past lifetime → death
...
describe('seeded state determinism via updateParticleEmitter2D', () => {
  ...
  function simulate(seed: number): number[] {
    ...
    for (let f = 0; f < 30; f++) updateParticleEmitter2D(emitter, state, config, 1 / 60);
