...
//
// `registry` is the one field NOT kept as the captured literal. RenderRegistryTable takes its values from
...
function miss(kind: string, registry: RenderRegistryTable) {
...
describe('findUndrawnRegistryMisses', () => {
  it('finds a shape rasterizer miss', () => {
    ...
    expect(findUndrawnRegistryMisses([miss('Shape', RenderRegistryTable.ShapeRasterizer)])).toEqual([
      { kind: 'Shape', registry: RenderRegistryTable.ShapeRasterizer },
    ]);
  });

  it('finds a material renderer miss', () => {
    expect(findUndrawnRegistryMisses([miss('Standard', RenderRegistryTable.MaterialRenderer)])).toEqual([
      { kind: 'Standard', registry: RenderRegistryTable.MaterialRenderer },
    ]);
  });
  ...
  it('ignores the registries whose absence does not mean undrawn output', () => {
    expect(
      findUndrawnRegistryMisses([
        miss('Blur', RenderRegistryTable.EffectPaddingResolver),
        miss('drawRoundedRectangle', RenderRegistryTable.ShapeCommandHandler),
        miss('Image', RenderRegistryTable.TextureResolver),
      ]),
    ).toEqual([]);
  });
  ...
  it('reports one defect once when a target drives more than one state', () => {
    const logs = [
      miss('Shape', RenderRegistryTable.ShapeRasterizer),
      miss('Shape', RenderRegistryTable.ShapeRasterizer),
    ];
  ...
  it('keeps distinct kinds and distinct registries apart', () => {
    const logs = [
      miss('Shape', RenderRegistryTable.ShapeRasterizer),
      miss('MorphShape', RenderRegistryTable.ShapeRasterizer),
      miss('Shape', RenderRegistryTable.MaterialRenderer),
    ];
...
describe('formatUndrawnRegistryMisses', () => {
  it('names the registry rather than printing its numeric enum value', () => {
    const message = formatUndrawnRegistryMisses([{ kind: 'Shape', registry: RenderRegistryTable.ShapeRasterizer }]);
