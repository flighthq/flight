...
describe('registerWgpuCustomShaderMaterial', () => {
  it('installs the renderer for CustomShaderMaterialKind', () => {
    ...
    expect(getWgpuMeshMaterialRenderer(state, CustomShaderMaterialKind)).toBe(wgpuCustomShaderMeshMaterialRenderer);
  });
});

describe('wgpuCustomShaderMeshMaterialRenderer', () => {
  it('reuses Frame/Draw layouts and binds reserved user + texture groups', () => {
    ...
    wgpuCustomShaderMeshMaterialRenderer.bind(
      state,
  ...
  it('reuses the group(3) bind group when the texture bag is unchanged', () => {
    ...
    wgpuCustomShaderMeshMaterialRenderer.bind(state, material, NO_LIGHTS, makeCamera());
  ...
  it('draws an indexed subset after a valid bind and skips a missing shader', () => {
    ...
    wgpuCustomShaderMeshMaterialRenderer.bind(state, proxy.material, NO_LIGHTS, makeCamera());
    wgpuCustomShaderMeshMaterialRenderer.draw(state, proxy, geometry);
