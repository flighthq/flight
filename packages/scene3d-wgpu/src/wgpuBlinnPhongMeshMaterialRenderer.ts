...
// BlinnPhongMaterialKind) — the WGSL mirror of glBlinnPhongMeshMaterialRenderer. Lambert diffuse plus
...
export const wgpuBlinnPhongMeshMaterialRenderer: WgpuMeshMaterialRenderer = {
  bind(
...
// top-level side effect); call once per WgpuRenderState before renderWgpuScene3D so meshes with BlinnPhong
// materials draw.
export function registerWgpuBlinnPhongMaterial(state: WgpuRenderState): void {
  registerWgpuBitmapTextureResolver(state);
  registerWgpuImageTextureResolver(state);
  registerWgpuMeshMaterialRenderer(state, BlinnPhongMaterialKind, wgpuBlinnPhongMeshMaterialRenderer);
}
