...
import { createTransmissionVolumePbrMaterial } from '@flighthq/materials/contract';
...
} from './transmissionVolumePbrWgpuMeshMaterialRenderer';
...
function makeProxy(): Scene3DRenderProxy {
  ...
  return {
    material: createTransmissionVolumePbrMaterial({ transmission: 0.9 }),
...
describe('registerTransmissionVolumePbrWgpuMaterial', () => {
  it('installs the renderer for TransmissionVolumePbrMaterialKind', () => {
    const { state } = makeWgpuScene3DState();
    registerTransmissionVolumePbrWgpuMaterial(state);
    expect(getWgpuMeshMaterialRenderer(state, TransmissionVolumePbrMaterialKind)).toBe(
      transmissionVolumePbrWgpuMeshMaterialRenderer,
    );
  });
});

describe('transmissionVolumePbrWgpuMeshMaterialRenderer', () => {
  it('bind selects a pipeline and binds frame + material groups + uploads uniforms', () => {
    const { fake, state } = makeWgpuScene3DState();
    transmissionVolumePbrWgpuMeshMaterialRenderer.bind(
      state,
      createTransmissionVolumePbrMaterial({ transmission: 0.9 }),
      makeLights(),
      makeCamera(),
    );
  ...
  it('bind compiles the TRANSMISSION extension variant', () => {
    const { fake, state } = makeWgpuScene3DState();
    transmissionVolumePbrWgpuMeshMaterialRenderer.bind(
      state,
    ...
    expect(code).toContain('const TRANSMISSION : bool = true;');
  });

  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    const { fake, state } = makeWgpuScene3DState();
    const proxy = makeProxy();
    transmissionVolumePbrWgpuMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    transmissionVolumePbrWgpuMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind has not selected a pipeline', () => {
    const { fake, state } = makeWgpuScene3DState();
    transmissionVolumePbrWgpuMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
