...
describe('registerWgpuBlinnPhongMaterial', () => {
  it('installs the renderer for BlinnPhongMaterialKind', () => {
    ...
    expect(getWgpuMeshMaterialRenderer(state, BlinnPhongMaterialKind)).toBe(wgpuBlinnPhongMeshMaterialRenderer);
  });
});

describe('wgpuBlinnPhongMeshMaterialRenderer', () => {
  it('bind selects a blinnphong pipeline and binds frame + material groups + uploads uniforms', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuBlinnPhongMeshMaterialRenderer.bind(state, createBlinnPhongMaterial(), makeLights(), makeCamera());
  ...
  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    ...
    const geometry = createBoxMeshGeometry();
    wgpuBlinnPhongMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    wgpuBlinnPhongMeshMaterialRenderer.draw(state, proxy, geometry);
  ...
  it('draw is a no-op when bind has not selected a pipeline', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuBlinnPhongMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
