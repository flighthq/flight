...
import {
  ...
  wgpuStandardPbrMeshMaterialRenderer,
...
describe('wgpuStandardPbrMeshMaterialRenderer', () => {
  it('bind selects a pipeline and binds frame + material groups + uploads uniforms', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuStandardPbrMeshMaterialRenderer.bind(state, createStandardPbrMaterial(), makeLights(), makeCamera());
  ...
  it('bind compiles a cull-none pipeline for a double-sided material', () => {
    ...
    const material = createStandardPbrMaterial();
    material.doubleSided = true;
    wgpuStandardPbrMeshMaterialRenderer.bind(state, material, makeLights(), makeCamera());
  ...
  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    ...
    const geometry = createBoxMeshGeometry();
    wgpuStandardPbrMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    wgpuStandardPbrMeshMaterialRenderer.draw(state, proxy, geometry);
  ...
  it('draw is a no-op when bind has not selected a pipeline', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuStandardPbrMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
