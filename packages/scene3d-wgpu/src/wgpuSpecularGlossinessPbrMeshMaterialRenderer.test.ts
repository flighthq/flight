...
describe('wgpuSpecularGlossinessPbrMeshMaterialRenderer', () => {
  it('bind selects a pipeline and binds frame + material groups + uploads uniforms', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuSpecularGlossinessPbrMeshMaterialRenderer.bind(
      state,
  ...
  it('drives the base PBR program with no extension flag (CPU spec-gloss conversion)', () => {
    ...
    expect(code).toContain('const CLEARCOAT : bool = false;');
    expect(code).toContain('const SHEEN : bool = false;');
    expect(code).toContain('const SPECULAR_EXT : bool = false;');
    expect(code).toContain('const TRANSMISSION : bool = false;');
  });

  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    const { fake, state } = makeWgpuScene3DState();
    const proxy = makeProxy();
    wgpuSpecularGlossinessPbrMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    wgpuSpecularGlossinessPbrMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind has not selected a pipeline', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuSpecularGlossinessPbrMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
