...
import { wgpuMatcapMeshMaterialRenderer, registerWgpuMatcapMaterial } from './wgpuMatcapMeshMaterialRenderer.ts';
...
describe('registerWgpuMatcapMaterial', () => {
  it('installs the renderer for MatcapMaterialKind', () => {
    ...
    expect(getWgpuMeshMaterialRenderer(state, MatcapMaterialKind)).toBe(wgpuMatcapMeshMaterialRenderer);
  });
});

describe('wgpuMatcapMeshMaterialRenderer', () => {
  it('bind selects a pipeline and binds the frame + material groups', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuMatcapMeshMaterialRenderer.bind(state, createMatcapMaterial(), NO_LIGHTS, makeCamera());
  ...
  it('draw issues an indexed draw over the subset after bind', () => {
    ...
    wgpuMatcapMeshMaterialRenderer.bind(state, proxy.material, NO_LIGHTS, makeCamera());
    wgpuMatcapMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind has not selected a pipeline', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuMatcapMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
