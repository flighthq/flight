...
import { createPhongMaterial } from '@flighthq/materials/contract';
...
import { wgpuPhongMeshMaterialRenderer, registerWgpuPhongMaterial } from './wgpuPhongMeshMaterialRenderer.ts';
...
describe('registerWgpuPhongMaterial', () => {
  it('installs the renderer for PhongMaterialKind', () => {
    ...
    expect(getWgpuMeshMaterialRenderer(state, PhongMaterialKind)).toBe(wgpuPhongMeshMaterialRenderer);
  });
});

describe('wgpuPhongMeshMaterialRenderer', () => {
  it('bind selects a phong pipeline and binds frame + material groups + uploads uniforms', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuPhongMeshMaterialRenderer.bind(state, createPhongMaterial(), makeLights(), makeCamera());

    const moduleCall = fake.calls.find((c) => c.name === 'createShaderModule');
    const code = (moduleCall!.args[0] as { code: string }).code;
    expect(code).toContain('const LIGHTING_PHONG : bool = true;');
  ...
  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    ...
    const geometry = createBoxMeshGeometry();
    wgpuPhongMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    wgpuPhongMeshMaterialRenderer.draw(state, proxy, geometry);
  ...
  it('draw is a no-op when bind has not selected a pipeline', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuPhongMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
