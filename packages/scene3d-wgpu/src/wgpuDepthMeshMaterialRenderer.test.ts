...
import { wgpuDepthMeshMaterialRenderer, registerWgpuDepthMaterial } from './wgpuDepthMeshMaterialRenderer.ts';
...
describe('registerWgpuDepthMaterial', () => {
  it('installs the renderer for DepthMaterialKind', () => {
    const { state } = makeWgpuScene3DState();
    registerWgpuDepthMaterial(state);
    expect(getWgpuMeshMaterialRenderer(state, DepthMaterialKind)).toBe(wgpuDepthMeshMaterialRenderer);
  });
});

describe('wgpuDepthMeshMaterialRenderer', () => {
  it('bind selects the depth pipeline and writes the frame + material uniforms', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuDepthMeshMaterialRenderer.bind(state, createDepthMaterial({ far: 50, near: 1 }), NO_LIGHTS, makeCamera());
  ...
  it('bind disables back-face culling for a double-sided material', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuDepthMeshMaterialRenderer.bind(state, createDepthMaterial({ doubleSided: true }), NO_LIGHTS, makeCamera());
  ...
  it('draw issues an indexed draw over the subset after bind', () => {
    ...
    wgpuDepthMeshMaterialRenderer.bind(state, proxy.material, NO_LIGHTS, makeCamera());
    wgpuDepthMeshMaterialRenderer.draw(state, proxy, createBoxMeshGeometry());
  ...
  it('draw is a no-op when bind has not selected a pipeline', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuDepthMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
