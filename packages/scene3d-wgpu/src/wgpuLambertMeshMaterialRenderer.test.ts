...
describe('registerWgpuLambertMaterial', () => {
  it('installs the renderer for LambertMaterialKind', () => {
    ...
    expect(getWgpuMeshMaterialRenderer(state, LambertMaterialKind)).toBe(wgpuLambertMeshMaterialRenderer);
  });
});

describe('wgpuLambertMeshMaterialRenderer', () => {
  it('bind selects a pipeline and binds frame + material groups + uploads uniforms', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuLambertMeshMaterialRenderer.bind(state, createLambertMaterial(), makeLights(), makeCamera());
  ...
  it('draw uploads geometry and issues an indexed draw over the subset range', () => {
    ...
    const geometry = createBoxMeshGeometry();
    wgpuLambertMeshMaterialRenderer.bind(state, proxy.material, makeLights(), makeCamera());
    wgpuLambertMeshMaterialRenderer.draw(state, proxy, geometry);
  ...
  it('draw is a no-op when bind has not selected a pipeline', () => {
    const { fake, state } = makeWgpuScene3DState();
    wgpuLambertMeshMaterialRenderer.draw(state, makeProxy(), createBoxMeshGeometry());
