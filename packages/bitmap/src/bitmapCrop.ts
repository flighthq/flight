 ...
 * `rect`. `rect` coordinates are relative to the `source` bitmap origin
...
/**
 * Allocates a new `Bitmap` with `source` padded by `left`, `top`, `right`,
...
export function extendBitmap(
  ...
  right: number,
  bottom: number,
  edgeMode: BitmapEdgeMode = 'transparent',
  fillColor = 0,
): Bitmap {
  const sw = source.width;
  const sh = source.height;
...
/**
 * Allocates a new `Bitmap` with the transparent border of `source` removed.
 * Finds the tightest bounding box of all pixels with alpha > 0 and returns a
 * new bitmap cropped to that box. If the entire bitmap is transparent,
 * returns a 1×1 transparent bitmap.
 */
export function trimBitmap(source: Readonly<Bitmap>): Bitmap {
  const sw = source.width;
  const sh = source.height;
  const sd = source.data;
  let minX = sw;
...
function resolveEdge(v: number, size: number, mode: BitmapEdgeMode): number | null {
