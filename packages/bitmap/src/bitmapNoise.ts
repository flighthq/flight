 ...
 * tests and a future C port). Region pixels outside the bitmap are skipped.
 *
...
export function fillBitmapNoise(
  ...
  for (let py = 0; py < dest.height; py++) {
    ...
    for (let px = 0; px < dest.width; px++) {
      ...
      // deterministic regardless of which pixels fall outside the bitmap.
      ...
      const x = dest.x + px;
      if (y < 0 || y >= dest.bitmap.height || x < 0 || x >= bitmapWidth) continue;
 ...
 *
 * Parameters are identical to `fillBitmapPerlinNoise` except there is no
