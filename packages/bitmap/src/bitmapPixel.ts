...
export function getBitmapPixel(source: Readonly<Bitmap>, x: number, y: number): number {
  const i = (y * source.width + x) * 4;
  return ((source.data[i] << 24) | (source.data[i + 1] << 16) | (source.data[i + 2] << 8) | source.data[i + 3]) >>> 0;
}

export function getBitmapPixelChannel(source: Readonly<Bitmap>, x: number, y: number, channel: ImageChannel): number {
  return source.data[(y * source.width + x) * 4 + channel];
}

export function getBitmapPixelLuminance(source: Readonly<Bitmap>, x: number, y: number): number {
  const i = (y * source.width + x) * 4;
  return Math.round(source.data[i] * LUMA_R + source.data[i + 1] * LUMA_G + source.data[i + 2] * LUMA_B);
}

export function getBitmapPixelRgb(source: Readonly<Bitmap>, x: number, y: number): number {
  const i = (y * source.width + x) * 4;
  return ((source.data[i] << 16) | (source.data[i + 1] << 8) | source.data[i + 2]) >>> 0;
}

export function setBitmapPixel(out: Bitmap, x: number, y: number, color: number): void {
