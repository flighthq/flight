...
export function warpBitmap(
  ...
  for (let dy = 0; dy < dh; dy++) {
    const oy = dest.y + dy;
    if (oy < 0 || oy >= dest.bitmap.height) continue;
 ...
 * inverts it, and delegates to `warpBitmap`. Edge and sample modes follow the
 * same rules as `warpBitmap`.
 *
 * `dest` must not alias `source` when their regions overlap.
 */
export function warpBitmapQuad(
  dest: Readonly<BitmapRegion>,
  source: Readonly<BitmapRegion>,
  ...
  if (Hinv === null) return;
  warpBitmap(dest, source, Hinv, edgeMode, sampleMode);
}
...
function warpResolveEdge(v: number, size: number, mode: BitmapEdgeMode): number | null {
