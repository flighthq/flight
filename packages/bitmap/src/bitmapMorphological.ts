 ...
 * `erodeBitmap` — applying erosion then dilation gives a morphological opening,
 * the reverse gives a closing.
 *
 * `out` must be at least `source.width * source.height * 4` bytes and must NOT
 * alias `source.bitmap.data` — each output pixel reads a neighbourhood of
 * source pixels.
 */
  ...
  for (let py = 0; py < h; py++) {
    for (let px = 0; px < w; px++) {
      ...
      for (let ky = -r; ky <= r; ky++) {
        const sy = Math.max(0, Math.min(bitmapHeight - 1, source.y + py + ky));
        for (let kx = -r; kx <= r; kx++) {
          const sx = Math.max(0, Math.min(bitmapWidth - 1, source.x + px + kx));
