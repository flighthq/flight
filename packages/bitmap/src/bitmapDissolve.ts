 ...
 *
 * When `source` is the same region as `dest` (same bitmap and identical
 * bounds), dissolved pixels are set to `fillColor` (packed `0xRRGGBBAA`);
 * otherwise each is copied from the matching pixel in
 * `source` and `fillColor` is ignored. Pixels that fall outside the bitmap are
 * clipped but still consume a step of the sequence.
 */
export function dissolveBitmapPixels(
  ...
  const destData = dest.bitmap.data;
  const destStride = dest.bitmap.width;
  const destBitmapHeight = dest.bitmap.height;
  const sourceData = source.bitmap.data;
  const sourceStride = source.bitmap.width;
  const sourceBitmapHeight = source.bitmap.height;
  ...
  while (dissolved < pixelCount && cursor < period) {
    ...
    const dx = dest.x + px;
    const dy = dest.y + py;
    if (dx < 0 || dx >= destStride || dy < 0 || dy >= destBitmapHeight) continue;
    ...
    if (sx < 0 || sx >= sourceStride || sy < 0 || sy >= sourceBitmapHeight) continue;
