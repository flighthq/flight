...
export function resizeBitmap(
  ...
  const opts: Readonly<BitmapResizeOptions> = typeof options === 'string' ? { mode: options } : options;
...
function resolveResizeEdge(v: number, size: number, mode: BitmapEdgeMode): number | null {
