import type { BitmapConvolutionOptions, BitmapRegion } from '@flighthq/types/contract';

/**
 * Applies a convolution kernel to `source` and writes into `out`.
 * `out` must be at least `source.width * source.height * 4` bytes.
 *
 * `out` must not alias `source.bitmap.data` — convolution reads neighboring
 ...
 *   - `'wrap'`: tile the bitmap toroidally
    ...
    for (let px = 0; px < source.width; px++) {
      ...
      for (let ky = 0; ky < matrixY; ky++) {
        ...
        for (let kx = 0; kx < matrixX; kx++) {
          ...
          if (rawSampleY < 0 || rawSampleY >= bitmapHeight || rawSampleX < 0 || rawSampleX >= bitmapWidth) {
            if (edge === 'transparent') {
              continue;
            } else if (edge === 'wrap') {
              sampleX = ((rawSampleX % bitmapWidth) + bitmapWidth) % bitmapWidth;
              sampleY = ((rawSampleY % bitmapHeight) + bitmapHeight) % bitmapHeight;
            } else if (edge === 'mirror') {
              sampleX = resolveConvolutionMirror(rawSampleX, bitmapWidth);
              sampleY = resolveConvolutionMirror(rawSampleY, bitmapHeight);
            } else {
              sampleX = rawSampleX < 0 ? 0 : rawSampleX >= bitmapWidth ? bitmapWidth - 1 : rawSampleX;
              sampleY = rawSampleY < 0 ? 0 : rawSampleY >= bitmapHeight ? bitmapHeight - 1 : rawSampleY;
            }
          } else {
            sampleX = rawSampleX;
            sampleY = rawSampleY;
          ...
          const i = (sampleY * bitmapWidth + sampleX) * 4;
          r += data[i] * weight;
          g += data[i + 1] * weight;
          b += data[i + 2] * weight;
          a += data[i + 3] * weight;
      ...
      if (preserveAlpha) {
        const cy = Math.max(0, Math.min(bitmapHeight - 1, source.y + py));
        const cx = Math.max(0, Math.min(bitmapWidth - 1, source.x + px));
        out[di + 3] = data[(cy * bitmapWidth + cx) * 4 + 3];
      } else {
