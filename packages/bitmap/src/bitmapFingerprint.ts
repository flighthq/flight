 ...
 */
export function compareBitmapFingerprints(a: Readonly<BitmapFingerprint>, b: Readonly<BitmapFingerprint>): number {
  if (a.gridSize !== b.gridSize) {
    throw new Error(`compareBitmapFingerprints: gridSize mismatch (${a.gridSize} vs ${b.gridSize})`);
  }
...
/**
 * Reduces a bitmap to a gridSize × gridSize grid of averaged RGB cells. Each cell averages the RGB
 ...
 * while staying ~1 KB as text. An empty bitmap yields an all-zero grid.
 */
export function createBitmapFingerprint(source: Readonly<Bitmap>, gridSize: number = 16): BitmapFingerprint {
  if (gridSize < 1) throw new Error(`createBitmapFingerprint: gridSize must be >= 1 (got ${gridSize})`);
 ...
 */
export function formatBitmapFingerprint(fingerprint: Readonly<BitmapFingerprint>): string {
...
/**
 * Parses the text form produced by formatBitmapFingerprint. Returns `null` for malformed input
 ...
 */
export function parseBitmapFingerprint(text: string): BitmapFingerprint | null {
