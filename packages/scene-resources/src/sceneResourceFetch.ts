...
// The swappable external-fetch seam an ExternalSceneResourceRef resolves through. Given a ref and an
...
export type SceneResourceFetch = (
  ref: Readonly<ExternalSceneResourceRef>,
  signal: AbortSignal,
...
export function createWebSceneResourceFetch(): SceneResourceFetch {
  return async (ref, signal) => {
    const url = resolveSceneResourceUri(ref.uri, ref.basePath);
    try {
...
export function resolveSceneResourceUri(uri: string, basePath: string | null): string {
  if (basePath === null || isAbsoluteSceneResourceUri(uri)) return uri;
  if (basePath.endsWith('/') || uri.startsWith('/')) return `${basePath}${uri}`;
  return `${basePath}/${uri}`;
}

function isAbsoluteSceneResourceUri(uri: string): boolean {
