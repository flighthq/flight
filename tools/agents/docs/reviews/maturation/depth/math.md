# Maturation Roadmap: @flighthq/math

**Current verdict:** stub — completeness 8/100. Two solid free functions (`createRandomSource`, `nextPowerOfTwo`) against a package name that claims the entire scalar-math domain; the scalar toolbox (clamp/lerp/angles/approxEqual/random conveniences) is essentially empty, and the package is absent from the Package Map.

The depth review already enumerates the missing scalar surface. This roadmap groups those into shippable tiers, adds the items it did not call out (constants module, deterministic-shuffle correctness, hash/noise frontier, Rust mirror), and flags the one design decision that gates everything: **resolve the `math`-vs-`random` scope question before Bronze**, and add the package to the Package Map. The roadmap below assumes the decision is "build out to AAA `math` scope" (the review's preferred path).
