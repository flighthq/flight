# Maturation Roadmap: @flighthq/shell

**Current verdict**: solid — 78/100. A faithful, correctly-shaped command-capability cell (open URL/path, reveal, trash, beep over a swappable `ShellBackend`) that meets its declared scope but stops a few functions short of the full Electron-class shell surface.

This package is small by nature — the shell domain is small. Bronze/Silver here are deliberately modest (the canonical surface is nearly covered already); the genuine work is at Gold, which is mostly about options-object fidelity, the Windows shortcut-link family, and Rust-port parity. All additions are native-host concerns that sentinel on the web, so they are additive and tree-shakable and cost nothing for users who do not import them.
