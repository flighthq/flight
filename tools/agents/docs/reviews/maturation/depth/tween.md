# Maturation Roadmap: @flighthq/tween

**Current verdict**: solid — 64/100. A competent property tweener with a real manager, easing, repeat/reflect/reverse, smart-rotation, snapping, pause/resume at three scopes, and a color tween — but short of an authoritative GSAP/TweenJS-class library on interpolation breadth, sequencing, and time control.

The three tiers below are cumulative. They build directly on the depth review's recommendation order: close interpolation and time-control gaps first, then add a value-interpolator seam and timeline, then push to AAA with full edge-case/error handling, perf, docs, and Rust parity. Each tier names concrete additions; types land in `@flighthq/types` first per the header-layer rule.
