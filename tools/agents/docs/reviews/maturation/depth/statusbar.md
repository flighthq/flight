# Maturation Roadmap: @flighthq/statusbar

**Current verdict**: solid — 72/100. The command (write) surface is clean and canonical; the package is missing the query (read) half, the height/safe-area dimension, animation, and change notification.

This is a deliberately narrow domain (the canonical references are Capacitor's `StatusBar` plugin and React Native's `StatusBar`). "AAA" here means matching that small-but-real surface exactly, with full backend-seam symmetry and Rust-port parity — not inventing scope the domain does not have. The tiers below are therefore small in absolute terms; Gold is genuinely reachable.
