# Maturation Roadmap: @flighthq/notification
...
All shared types land in `@flighthq/types/src/Notification.ts` first (the header layer), then the package implements free functions against them. The Rust mirror is `flighthq-notification`; every tier addition is expected to land in the crate to keep 1:1 conformance.
