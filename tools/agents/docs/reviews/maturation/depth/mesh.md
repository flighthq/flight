# Maturation Roadmap: @flighthq/mesh

**Current verdict:** partial — 52/100. A correct, clean, well-documented core (data model, canonical PBR record, the seven common primitives with proper normals/tangents/bounds), but its surface is narrow against the canonical bar: no attribute accessors, no geometry transforms, no merge/index ops, and only half the canonical primitive set.

The package owns CPU-side geometry only — interleaved vertex/index buffers, layouts, parametric primitive builders, and per-vertex attribute computation. GPU upload/draw (`scene-gl`/`scene-wgpu`), materials, lighting, cameras, and asset loading (glTF/OBJ) are correctly out of scope. The three tiers below grow breadth and depth _within_ that CPU-geometry boundary, and treat the existing private helpers (`buildCanonicalMeshGeometry`, `addDisc`) as already-paid work to surface.
