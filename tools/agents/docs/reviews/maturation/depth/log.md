# Maturation Roadmap: @flighthq/log

**Current verdict:** partial — completeness 48/100; a clean, well-built logging _seam_ sized for one job (feed the capture harness `logs.jsonl` + print gated console lines), well short of the canonical logging-library feature set the package name implies.

The package today is a two-faced contract: an emit side (`log` + `logError/Warn/Info/Debug/Verbose`) that tree-shakes to a forwarder when no sink is installed, and a listener side (`createConsoleCaptureSink`, `setLogSink`, `getLogConsoleLevel`/`setLogConsoleLevel`). Types (`LogLevel`, `LogData`, `LogEntry`, `LogSink`) already live in `@flighthq/types`. The tiers below grow that seam into a full library without abandoning the emit/listener split or the tree-shaking guarantee — every addition stays opt-in and side-effect-free.

A guiding constraint throughout: **the emit side must stay free.** New capability is added on the listener/sink side and behind explicit opt-in functions, never as eager work in `log()`. The single largest design decision (Bronze) is moving from one replaceable sink to a sink list; sequence everything else after it.
