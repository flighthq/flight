# Maturation Roadmap: @flighthq/path

**Current verdict:** partial — completeness 38/100; a strong curve flattener plus a basic ear-clipping fill tessellator, but well short of an authoritative path library (the entire analytic, transform, stroking, and boolean surface is missing-by-omission).

The present surface is 7 functions: `createPath`, `appendPathMoveTo`/`appendPathLineTo`/`appendPathCurveTo` (quadratic)/`appendPathCubicCurveTo`, `flattenPath`, `tessellatePath`. Types `Path`, `PathCommand`, `PathMesh`, `PathWinding` live in `@flighthq/types`. The Rust crate `flighthq-path` already mirrors the same three source files (`path.rs`, `flatten_path.rs`, `tessellate_path.rs`), so every tier below carries a parallel Rust obligation.

The work is overwhelmingly additive — none of it conflicts with the plain-data / free-function / tree-shakable style; it is simply unbuilt. Each tier below is cumulative.
