# Maturation Roadmap: @flighthq/haptics

**Current verdict:** solid — 72/100; covers the core impact/notification/selection/vibrate surface cleanly, but falls short of authoritative on a handful of widely-expected secondary operations (pattern vibration, cancel, availability query, intensity).

This is a platform-integration _cell_, not an open-ended algorithm library, so the canonical bar is narrow and reachable. The references are Apple's `UIFeedbackGenerator` family + Core Haptics, Android `VibrationEffect` / `VibratorManager` / `HapticFeedbackConstants`, Capacitor `Haptics`, and the Web Vibration API. Three of the four named gaps have their primitive already in hand (the web backend builds patterns internally and `navigator.vibrate(0)` cancels), so Bronze is genuinely small.

All tiers follow the platform-suite command-capability shape: types in `@flighthq/types` first, free functions over a swappable `HapticsBackend`, `create*`/`get*`/`set*` seam, lazy web default, `"sideEffects": false`, single `.` export, sentinels (`false`/`-1`) on unavailable/denied, never throw.
