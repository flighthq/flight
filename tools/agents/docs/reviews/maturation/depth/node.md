# Maturation Roadmap: @flighthq/node

**Current verdict** (from depth review): solid — 78/100; the canonical spine of a scene-graph base is present with real engineering depth (fine-grained revision/dirty channels, cached lazy transforms, offset-only world-bounds fast path), but traversal/query primitives, lifecycle teardown, and 3D parity are missing-by-omission rather than excluded-by-design.

The three tiers below are cumulative. Each names concrete functions/types to add. Per Flight rules, every new cross-package type is defined in `@flighthq/types` first (the header layer); functions are free functions with the full unabbreviated `Node` word, `out`-param / `create*` allocation discipline, `Readonly<>` by default, sentinels for expected failure, and a mirrored `flighthq-node` Rust crate.
