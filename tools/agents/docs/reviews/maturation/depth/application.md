# Maturation Roadmap: @flighthq/application

**Current verdict**: solid — 72/100. A robust, near-authoritative _windowing_ library bolted to a _stub_ main loop; the windowing half is essentially done while the application/loop/lifecycle half is a thin `requestAnimationFrame` wrapper.

The asymmetry is the whole story. The windowing surface needs only small polish; the loop/lifecycle surface needs to be genuinely built. This roadmap concentrates effort there, and treats the existing `WindowBackend` seam as the template for a parallel loop backend seam. Each tier is cumulative.
