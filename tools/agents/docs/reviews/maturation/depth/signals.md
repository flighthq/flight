# Maturation Roadmap: @flighthq/signals

**Current verdict:** solid — 72/100. A clean, complete, tree-shakable core signal/slot primitive that covers connect/disconnect/emit/priority/once/cancel without bloat, but lacks the connection-handle, introspection, and named-`once` features that mature signal libraries treat as table stakes.

This roadmap builds on the depth review (`reviews/depth/signals.md`) rather than restating it. The depth review's six recommendations map onto Bronze; Silver and Gold push past them into operator coverage, re-entrancy/ordering guarantees, and full Rust-crate parity.

Design constraints honored throughout: types land in `@flighthq/types` first (the header layer); plain-data `Signal`/`SignalData` over runtime objects; free functions with the full unabbreviated type word (`getSignalSlotCount`, not `getSlotCount`); explicit allocation (`create*`, returned handles are explicit); `Readonly<>` on read-only params; sentinels (`-1`/`false`/`null`) for expected failure and `throw` only on misuse; single root `.` export, `sideEffects: false`; and 1:1 mirroring in the `flighthq-signals` Rust crate (which already mirrors the `signal.rs`/`slot.rs`/`emitter.rs`/`throttle.rs` file split).
