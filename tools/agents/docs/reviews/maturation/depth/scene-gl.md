# Maturation Roadmap: @flighthq/scene-gl

**Current verdict:** solid — 62/100; a genuinely deep material/shader library bolted to a deliberately minimal one-light forward renderer (one directional + one ambient, no shadows, no IBL, no transparency sort, no instancing/skinning).

The shading **catalog** is already near-AAA (full Cook-Torrance PBR + 7 KHR extension lobes + classic Lambert/Phong/BlinnPhong + toon + matcap + unlit/emissive/vertex-color + debug + wireframe, all opt-in `register*GlMaterial`). The deficit is entirely in the **renderer**: the dimensions that make 3D look real. Critically, the header layer is already ahead of the renderer — `@flighthq/lighting` ships `pointLight`, `spotLight`, `hemisphereLight`, `areaLight`, and `environment` descriptors, and `@flighthq/types` already defines `Environment`, `CubeTexture`, and the punctual light types. The renderer is the lagging layer, and `bindGlMeshLightBlock` / `SceneLightBlock` are the choke point: they pack exactly one directional + one ambient.

A near-identical sibling, `@flighthq/scene-wgpu`, mirrors scene-gl one-for-one. Every type-layer and lighting/shadow/IBL decision below is **cross-backend by construction** — design in `@flighthq/types` + `@flighthq/lighting` + `@flighthq/render` once, then implement twice (gl and wgpu). Treat that pairing as a hard constraint, not an afterthought.
