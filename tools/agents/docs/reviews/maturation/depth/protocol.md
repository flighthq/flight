# Maturation Roadmap: @flighthq/protocol

**Current verdict**: solid — 70/100. Covers the core registration + warm-open delivery surface cleanly and idiomatically, but drops cold-start launch URLs, lacks the default-handler query/clear half of the control triplet, and ships no deep-link URL payload helpers.

This is a platform-integration capability (a `*Backend` seam), not a self-contained algorithm library, so the honest authoritative bar is "covers what a mature deep-link integration is expected to expose" — measured against Electron (`setAsDefaultProtocolClient` family + `open-url`/`argv`), Tauri `deep-link` (`register`/`unregister`/`isRegistered`/`getCurrent`/`onOpenUrl`), Capacitor (`appUrlOpen` + `getLaunchUrl`), and RN/Expo `Linking` (`getInitialURL`/`addEventListener`/`parse`/`createURL`). The seam shape, entity/runtime split, web degrade-to-sentinel behavior, and full colocated tests are already correct; maturation is filling named gaps in the surface, not restructuring it.

Note: the underlying Electron host primitives for the default-handler triplet (`removeAsDefaultProtocolClient`, `isDefaultProtocolClient`) already exist in `host-electron/src/electronModule.ts`; Bronze item 2 is mostly surfacing them through the seam, not building native plumbing.
