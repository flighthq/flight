# Maturation Roadmap: @flighthq/updater

**Current verdict:** solid — 72/100; a well-shaped event-capability seam over a host updater, complete for the lifecycle but event-only, with a thin `UpdateInfo`, an untyped error, and no queryable state, channels, policy, or integrity contract.

This package is intentionally a _seam_, not an updater engine — the hard mechanism (signing, differential download, staging, rollback) lives behind `UpdaterBackend` in a `host-*` adapter. Maturation here means making the **contract** authoritative and queryable, not implementing an updater. Every tier below stays in `@flighthq/updater` + `@flighthq/types`; the actual work it unlocks lands in `host-electron` (and future `host-tauri`). Following the codebase rule, all type additions land in `@flighthq/types/src/Updater.ts` first, then the package implements free functions against them.
