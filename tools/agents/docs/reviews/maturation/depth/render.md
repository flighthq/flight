# Maturation Roadmap: @flighthq/render

**Current verdict**: solid — 74/100. A deliberate, well-factored render _core_ that owns preparation, registration, the proxy/update pipeline, caching, and target math with strong tests, but is one major piece short of authoritative: it does not own the backend-agnostic _draw/submit driver_ its own `Renderer.submit` / `BatchFormat` contract implies, and has no render-queue or render-pass abstraction.

The single highest-value gap is structural, not cosmetic: the `Renderer` interface already declares `submit(state, node)` and an optional `format?: BatchFormat`, the `RenderTargetDescriptor` already declares MRT/MSAA/HDR/depth axes, and the GPU backend _cores_ (`render-gl`, `render-wgpu`) already exist — but nothing in this package walks prepared proxies and dispatches `submit` with auto-flush. Every backend is expected to re-derive that walk. Closing that is the spine of this roadmap. The per-subject leaf renderers (`displayobject-gl`, `displayobject-wgpu`) are not yet built; the driver this package is missing is exactly what they will plug leaf `submit` calls into, so building it now shapes their seam correctly rather than retrofitting.
