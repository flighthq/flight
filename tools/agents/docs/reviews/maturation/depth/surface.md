# Maturation Roadmap: @flighthq/surface

**Current verdict:** authoritative — 88/100. The clearest "AAA" package in the depth set: 86 exported functions across 34 files, every one colocated-tested, covering the full `BitmapData` surface area plus a large slice of canonical software-raster image processing. The remaining points are a handful of in-domain canonical gaps, not structural thinness.

Because the package is already authoritative, Bronze and Silver are close-the-gap polish; Gold is the genuine frontier (wide-gamut, performance, true parity instrumentation, format importers). The Rust crate `flighthq-surface` already mirrors the TS package 1:1 (86 `pub fn`, same file layout), so Rust parity must be maintained step-for-step as each tier lands — every TS addition below has a paired Rust function.
