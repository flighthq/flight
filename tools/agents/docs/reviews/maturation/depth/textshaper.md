# Maturation Roadmap: @flighthq/textshaper

**Current verdict:** stub — completeness 18/100 (a correct, well-shaped registration seam that _measures_ rather than _shapes_; the depth gap is policy — the advances-only "measure tier" the text design doc owns — not oversight).

This roadmap treats the package as the canonical home of the **shape** layer in Flight's text stack (itemize/bidi → **shape** → layout → rasterize). Layout stays owned by `@flighthq/textlayout`; this package's job is to turn a text run into positioned glyphs (ids, advances, offsets, clusters) plus the font metrics layout and rasterization need. The heavy full-glyph backend (HarfBuzz/rustybuzz) stays an opt-in `-formats`/`-backend` neighbor package per the bundle-size rule. Each tier below is cumulative.
