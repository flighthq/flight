# Maturation Roadmap: @flighthq/keyboard

**Current verdict** (from depth review): solid — 80/100; a near-complete, idiomatic cell for a deliberately narrow domain (soft-keyboard visibility/height + show/hide/resize signals over a swappable web/native backend), missing only animation-aware events and a handful of native-only controls.

This package is already close to authoritative. Bronze and Silver are therefore narrow, high-leverage additions rather than foundational build-out; Gold describes the genuine frontier (full animation curves, native control surface, and 1:1 Rust parity). Throughout, the physical-key boundary stays where it is: key codes, modifiers, and IME composition remain `@flighthq/input`, never this package.
