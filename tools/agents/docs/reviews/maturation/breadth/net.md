# New Package Spec: @flighthq/net

**Represents:** General-purpose HTTP request/response — the OpenFL `URLLoader`/`URLRequest`/`URLVariables` home: method, headers, body, text/binary/JSON/form responses, progress + completion signals, abort, retry/backoff — over a swappable transport backend (web `fetch`, native host, Rust `reqwest`).

**Requested by:** missing-domains, openfl-lime-parity, asset-pipeline

## Fits

- **Distinct from `@flighthq/network`.** `network` is already taken by the _connectivity-status_ event capability (`isNetworkOnline`, `NetworkStatus`, online/offline signals). `net` is the _transport_ package — arbitrary HTTP. The two are siblings, not the same cell; the short, unambiguous transport name the reviews asked for is `net`.
- **Architecture slot:** a command-style platform capability with a `*Backend` seam, exactly like `dialog`/`clipboard`/`filesystem`. Web backend over `fetch` is always lazily available; native hosts swap it via `setHttpBackend`. It is the transport seam that `resources`/`loader` should sit _on top of_ — those packages currently reach for browser primitives directly, and the asset-pipeline review flags the absence of this shared layer for the native/Rust target where `fetch` does not exist.
- **Dependencies:** `@flighthq/types` (header layer), `@flighthq/signals` (progress/completion signal groups, opt-in). No dependency on `@flighthq/network`, `resources`, or `loader` — the dependency arrow points the other way (consumers depend on `net`, not vice versa). `surface`-style value-typed seam: requests and responses are plain data, so it is a near-zero-copy mixable leaf for the Rust port.
- **Neighbor packages:** `@flighthq/net-formats` for body codecs that are _not_ core HTTP — multipart builders, URL-encoded form parsing, query-string utilities — following the `-formats` importer/parser convention. Future transport siblings live in their own cells, not here: `@flighthq/socket` (TCP/WebSocket, OpenFL `Socket`/`XMLSocket`), `@flighthq/sse` or stream extensions for Server-Sent Events.
- **Rust crate:** `flighthq-net` (reqwest transport, native default; `host-web` provides the wasm `fetch` fill). 1:1 conformance against the TS package; request/response are plain-data value seams ideal for the conformance instrument.
- **`*Kind` strings** for response-body interpretation modes (`TextResponseKind`, `BinaryResponseKind`, `JsonResponseKind`, `FormResponseKind`), mirroring OpenFL's `URLLoaderDataFormat` as a string registry value.
