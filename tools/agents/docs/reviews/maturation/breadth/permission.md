# New Package Spec: @flighthq/permission

**Represents:** Unified permission capability — query, request, and observe the OS permission state (granted/denied/prompt) for the capabilities that gate behind a grant (geolocation, notifications, camera/microphone, sensors, persistent filesystem, and more) over a single swappable backend seam, so an app drives all permission UX from one place instead of per-capability calls.

**Requested by:** application-platform

## Fits

- **Architecture slot:** a **command-style platform capability** with a `*Backend` seam, exactly like `notification`/`geolocation`/`clipboard`/`dialog`. It exposes flat free functions plus `getPermissionBackend` / `setPermissionBackend` / `createWebPermissionBackend`. A web backend over the [Permissions API](https://developer.mozilla.org/docs/Web/API/Permissions_API) (`navigator.permissions.query`) is always lazily available; native hosts swap it via `setPermissionBackend`. It _also_ receives inbound change events (the web `PermissionStatus.onchange`), so — like `tray`/`menu`'s `on*` subscriptions — it carries a flat `onPermissionChange(kind, listener)` over a backend `subscribe*` method, with the richer multi-listener form gated behind an `enable*` signal group.
- **Distinct from the capability packages themselves.** `notification.requestNotificationPermission()`, `geolocation.requestGeolocationPermission()`, `webcam.requestPermission()`, and `sensors.requestPermission()` already exist and **stay** — they are the capability-local "ask right before I use this" call. `permission` is the _aggregating, observing, status-querying_ layer that unifies them under one `PermissionKind` vocabulary so an onboarding/settings screen can drive the whole set. The dependency arrow points **into** `permission` from app code, not from the capability packages (no capability package imports `permission`).
- **Dependencies:** `@flighthq/types` (header layer — all shapes live there first) and `@flighthq/signals` (opt-in change-signal group). No dependency on `geolocation`/`notification`/`webcam`/`sensors`/`filesystem` — `permission` speaks only the `PermissionKind` string vocabulary and the backend seam, never importing the concrete capabilities. This keeps it a tree-shakable, value-typed leaf.
- **Neighbor packages:** none of the `-formats` kind (there is nothing to parse). Its natural neighbors are the gated capability packages it describes (`geolocation`, `notification`, `webcam`, `sensors`, `filesystem`, plus future `contacts`/`calendar`/`bluetooth`/`microphone`). Each native `host-*` adapter fills the `PermissionBackend` alongside the other seams it already implements.
- **Backend seam:** `PermissionBackend` in `@flighthq/types`, set via `setPermissionBackend`, web default via `createWebPermissionBackend`. Mirrors the command-capability pattern uniformly applied across the suite.
- **Rust crate:** `flighthq-permission` (native default backend gated behind the `native` cargo feature; `host-web` provides the wasm fill over `navigator.permissions`). Plain-data value seam (`PermissionKind` strings, `PermissionState` enum-strings) → ideal 1:1 conformance target.
- **`"sideEffects": false`, single root `.` export, no module-top-level state** — the active backend is a lazily-created web default behind `getPermissionBackend`, never registered at import time.
