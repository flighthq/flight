# New Package Spec: @flighthq/collision

**Represents:** Value-typed 2D collision — AABB/circle/polygon overlap and swept tests, penetration/separation vectors (contact manifolds), a tile-collision resolver, and a minimal body integrator (velocity, gravity, restitution). Data and free functions, no runtime objects.

**Requested by:** game-2d, missing-domains

**Fits:**

`@flighthq/collision` is the gameplay-runtime layer that sits between rendering and the game, the home for the platformer core loop (move → test → resolve → integrate) that both reviews flagged as having no SDK home today. It is a value-math leaf package, not a graph or render package.

- **Dependencies:** `@flighthq/types` (header layer) and `@flighthq/geometry` (it reuses `Vector2`/`Vector2Like`, `Rectangle`/`RectangleLike`, `Aabb`, `getVector2Distance`, `containsRectanglePoint`, `intersectsRectangle`, and the `acquire*`/`release*` pools rather than re-deriving them). It must **not** import `node`, `displayobject`, `sprite`, `render*`, or `@flighthq/sdk`. Plain-data-in / plain-data-out, so it is in the mixable leaf set (deterministic, no GPU, headlessly fingerprintable).
- **Position in the SDK:** one layer above `geometry`, parallel to `interaction`. `interaction` is point-in-object **hit-testing** wired into the scene graph (UI, click-to-select); `collision` is gameplay **overlap/resolution** over standalone value shapes. The two are complementary and must not be merged. `@flighthq/sdk` re-exports `collision`. The future `camera2d` (culling visible bounds) and a future `spatial` broadphase (quadtree/grid/hash) are siblings, not part of this package.
- **Neighbor packages:** a `spatial` broadphase package (quadtree / uniform grid / spatial hash) is the natural neighbor for "what is near X" — kept out of `collision` so the narrowphase tests tree-shake without pulling a partitioning structure. No `-formats` neighbor is warranted: collision shapes have no authored file format of their own (collider shapes ride along in Tiled/LDtk levels, which belong to a future `tilemap-formats`, not here). A future rigid-body **solver** (joints, contact islands, sequential impulse) is a heavier neighbor (`@flighthq/physics2d`); `collision` deliberately stops at single-body integration + per-pair resolution.
- **Backend seam:** none. Collision is deterministic CPU math with no platform capability behind it, so there is no `*Backend` / `get*Backend` / `set*Backend` / `createWeb*Backend` surface (same reasoning as `geometry`, `easing`, `spring`).
- **Rust crate:** `flighthq-collision`, a 1:1 value-math mirror under `crates/`. Free functions over `&Shape` + `&mut Body` / `out` params; bit-comparable to TS under the conformance suite. Belongs to the mixable leaf set.

Types are defined in `@flighthq/types` first (the header layer), then implemented in `@flighthq/collision` against them.
