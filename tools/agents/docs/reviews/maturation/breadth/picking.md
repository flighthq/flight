# New Package Spec: @flighthq/picking

**Represents:** 3D raycasting and scene selection — ray construction from a camera + screen point, ray-vs-primitive intersection (AABB, sphere, plane, triangle, mesh), and a `pickScene` query that walks the scene graph and returns sorted hits. The 3D counterpart to `@flighthq/interaction`.

**Requested by:** spatial-3d

## Fits

- **Position in the architecture.** A leaf consumer of the 3D math + scene stack. It sits beside `@flighthq/interaction` (which is 2D display-object hit testing only) as the 3D selection layer. It is value-in / value-out for the math core (a mixable leaf in the Rust sense — deterministic, no GPU) and graph-walking for the scene query.
- **Dependencies.** `@flighthq/types` (header layer), `@flighthq/geometry` (Vector3, Matrix4, Aabb, BoundingSphere, Plane, Frustum — and the new `Ray3` primitive, see below), `@flighthq/camera` (`getCameraInverseViewProjectionMatrix4` is the unproject hook), `@flighthq/scene` (`SceneNode`/`Mesh` traversal, world transforms, runtime bounds), `@flighthq/mesh` (`MeshGeometry` index/position buffers + `computeMeshGeometryBounds` for triangle-level picking). No dependency on any renderer package or on `@flighthq/sdk`.
- **The `Ray3` primitive belongs in `@flighthq/geometry`, not here.** `geometry` already ships `Aabb`, `BoundingSphere`, `Plane`, `Frustum`, and `Vector3` with their pools and intersection helpers, but there is no ray type yet. A ray is a foundational math value (origin + direction), peer to `Plane`/`Aabb`, and other subsystems (culling debug, gizmo dragging, physics-lite) will want it. So **`Ray3` + the pure ray-vs-primitive math live in `geometry`**, and `@flighthq/picking` is the camera-aware, scene-graph-aware layer on top. This keeps `picking` a thin, graph-coupled cell and keeps the reusable math where the rest of the math is. (Named `Ray3`, not `Ray`, to leave room for a 2D ray and to read unambiguously next to `Vector3`/`Matrix3`.)
- **No backend seam.** Picking is pure CPU math over scene data; there is no platform capability to swap, so no `*Backend` / `get*` / `set*` / `createWeb*`. (GPU color-buffer picking is a possible _optional_ Gold add over the render packages — see Open questions — but the default path is CPU and backend-free.)
- **Signals.** Picking itself returns values; it does not dispatch. Hover/enter/leave selection _events_ over time belong to a manager (see Silver `PickManager`) and opt in via an `enablePickSignals`-style group on the manager, mirroring `@flighthq/interaction`'s `enableInteractionSignals`. The low-level query functions stay event-free.
- **Rust crate.** `flighthq-picking`, mirroring this package 1:1. The ray math lands in `flighthq-geometry` (`Ray3`, `ray3_intersect_aabb`, …). `flighthq-picking` is a value/graph crate with a substrate in the box (it conforms via the parity differ, `rust:skia`/headless as reference). A GPU-buffer pick path, if added, would live behind a `render-wgpu` feature, not in the core crate.
- **Neighbor `-formats`?** None. There is no file format to parse here — picking is computation, not import. (Contrast `particles-formats`/`spritesheet-formats`.) The closest "neighbor" is the new ray math in `geometry`.
