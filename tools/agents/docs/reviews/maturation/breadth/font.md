# New Package Spec: @flighthq/font

**Represents:** A real font subsystem — family/weight/style/stretch matching, fallback chains, codepoint coverage queries, variable-font axes, font metrics, and missing-glyph (tofu) handling — distinct from today's single `Font { name }` interface and the loaders bolted onto `@flighthq/resources`.

**Requested by:** text-typography

## Fits

`@flighthq/font` is the **font-identity and font-selection layer** that sits between raw font resources and the text stack. Today there is no home for "given a styled run, which physical face do I shape with, and what does that face cover?" — `Font` is `{ name }`, `FontResource` is `{ family, face }`, and selection/fallback/metrics have nowhere to live. This package owns the _font model_; the shaper consumes its output, layout consumes its metrics, and renderers consume the resolved face.

- **Depends on:** `@flighthq/types` (header layer — all shapes below land there first), `@flighthq/entity` (the `FontFamily` / `FontCollection` entity+runtime pair), and `@flighthq/geometry` (metric rects / `out`-param math). It does **not** depend on `@flighthq/textshaper`, `@flighthq/textlayout`, `@flighthq/text`, or any renderer — those depend _on it_. It must not import `@flighthq/sdk`.
- **Relationship to `@flighthq/resources`:** `resources` keeps the _loading_ of bytes (`loadFontFromUrl`, `loadFontFromArrayBuffer`, `FontResource`). `font` keeps the _model and matching_ over already-loaded faces. The current `Font { name }` interface is **retired** into a richer `FontFamily` / `FontFace` model owned here; `resources` loaders are rewritten to register the loaded face into a `FontCollection` rather than returning a bare `Font`. (This is the pre-release "rename it, don't accumulate workarounds" path — `Font { name }` is wrong and gets replaced, not extended.)
- **Neighbor package — `@flighthq/font-formats`:** the `-formats` pattern (like `spritesheet-formats`, `particles-formats`). Parsing the OpenType/TrueType binary tables — `name`, `head`, `hhea`, `OS/2`, `cmap` (coverage), `fvar`/`STAT` (variable axes), `post` — to populate `FontFace` metadata/metrics/coverage **without** a host font engine. This is the importer/parser cell; it stays out of the root `font` package so codepoint-coverage and axis introspection are tree-shakable away from apps that only do family matching against system fonts.
- **Backend seam — `FontBackend`:** font _discovery/enumeration and system-face metrics_ are a platform capability (the OS has fonts; the web exposes a thin slice via `document.fonts` + the Font Access API). So `font` follows the command-capability seam pattern: `getFontBackend()` / `setFontBackend(backend | null)` / `createWebFontBackend()`, web default lazily available, native host swaps it. Pure-data operations (matching over an in-memory collection, parsed-table coverage) need **no** backend and stay synchronous free functions.
- **Signals:** font _loading/registration_ into a live `FontCollection` can have multiple listeners (a layout cache must invalidate when a fallback face becomes available). Opt-in via `enableFontCollectionSignals(collection)` exposing `onFontFaceRegistered` / `onFontCollectionChanged` — Gold tier; not paid by default.
- **Rust crate:** `flighthq-font`, mirroring 1:1. The matching/coverage/metrics core is a **value-typed, deterministic leaf** (collection in, resolved face out) — squarely in the mixable/conformance set; bit-deterministic, no GPU. `flighthq-font-formats` mirrors the parser (ttf-parser is the natural Rust backend). The `FontBackend` seam maps to a `FontBackend` trait + `set_font_backend`, with a native default behind the `native` cargo feature (the OS font enumerator) and the web fill in `host-web`.
