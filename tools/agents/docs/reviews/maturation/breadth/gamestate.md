# New Package Spec: @flighthq/gamestate

**Represents** — Game flow as plain data: a screen/scene **stack** (push/pop/replace/set with enter/exit lifecycle and transitions) and a reusable **finite-state-machine** helper (states, guarded transitions, enter/update/exit) for app flow and per-entity behavior. The runtime layer that decides _which screen is active and what state an entity is in_ — not what it renders.

**Requested by** — game-2d (the "No scene/screen state management" gap: _"No screen stack or state machine (menu/level/pause/game-over). `scene` is the 3D world graph, not a game-state manager. Most engines ship this; here every game reinvents it."_ Listed under missing packages as **`gamestate` / `screen`** — a screen/scene stack and FSM helper for game flow and entity state).

## Fits

`@flighthq/gamestate` is the **game-flow control layer** that sits above rendering and below the application loop. It holds the active-screen stack and the entity/app state machines; it advances them with an explicit delta and dispatches enter/exit; it never draws anything itself. The hard line: this package owns **control flow over states**, not the content of any state. A `GameScreen` is a plain descriptor with caller-supplied lifecycle hooks (`onEnter`/`onUpdate`/`onExit`/`onDraw`), so the package depends on no renderer and no display package.

- **Depends on:** `@flighthq/types` (header — every `gamestate` type and `*Kind` string lives here first), and optionally `@flighthq/signals` for the opt-in signal group. It does **not** depend on `@flighthq/displayobject`, `@flighthq/sprite`, `@flighthq/scene`, any `render*` package, `@flighthq/application`, `@flighthq/tween`, or `@flighthq/sdk`. Transitions reference `@flighthq/easing` _types_ only if at all (an easing function is a plain `(t: number) => number` the caller passes in), preserving import-in-isolation.
- **Relationship to `@flighthq/application`:** `application` owns the **frame source** (`startApplicationLoop`/`stopApplicationLoop`, the wall-clock tick). `gamestate` is advanced _from_ that tick: the caller wires `updateGameStateStack(stack, deltaSeconds)` (and `updateStateMachine(machine, deltaSeconds)`) into their loop, exactly as the motion subsystems are. `gamestate` never reads `performance.now()` or owns a loop — it is deterministic and host-agnostic (the rule the Rust headless conformance harness needs).
- **Relationship to `@flighthq/scene`:** name-collision caveat called out in the review — `scene` is the **3D world graph**, not game flow. `gamestate` deliberately uses **`GameScreen`** (not `Scene`) as its primary noun to avoid the collision; "scene stack" in game terms maps to `GameScreenStack` here.
- **Neighbor packages:** none required at Bronze/Silver. There is no parse/serialize surface for screens (they are caller code, not authored assets), so **no `-formats` neighbor**. A _serializable_ state-machine graph (authored in a visual FSM editor → `StateMachineGraphData`) is the one plausible future neighbor, **`@flighthq/gamestate-formats`** (mirroring `particles-formats`/`spritesheet-formats`); deferred to Gold/Open-questions.
- **Backend seam:** **none.** State/flow control is pure CPU logic with no platform capability — a `*Backend` here would be premature. (Persisting the active state to disk is `@flighthq/storage`/`@flighthq/filesystem`, not a `gamestate` backend.)
- **Signals:** opt-in via `enableGameStateStackSignals(stack)` and `enableStateMachineSignals(machine)` (the `enable*`-group pattern), surfacing screen push/pop/transition and state-change/transition-rejected events for devtools and audio. Bare lifecycle uses direct callbacks on the descriptor (`onEnter`, etc.) — the hot path carries no signal-dispatch cost unless enabled.
- **Rust crate:** `flighthq-gamestate`. A value-typed control crate (no GPU, deterministic, headlessly fingerprintable) — a clean 1:1 conformance target. The stack is `Vec<GameScreen>` over slotmap-friendly ids; the FSM is `HashMap<StateId, StateNode>` + free functions over `(&mut StateMachine, ...)`. Lifecycle hooks become `Box<dyn Fn>` callbacks, mirroring the TS descriptor.

Types are defined in `@flighthq/types` first, then implemented against here. Exported names carry the full unabbreviated type word (`GameScreen`, `GameScreenStack`, `StateMachine`, `pushGameScreen`, `transitionStateMachine`).
