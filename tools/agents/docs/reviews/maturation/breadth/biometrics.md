# New Package Spec: @flighthq/biometrics

**Represents** Biometric authentication and OS-backed secure credential storage: a Touch ID / Face ID / fingerprint prompt, and a keychain/keystore-encrypted secret store, distinct from the plaintext `@flighthq/storage` key/value seam.

**Requested by** application-platform

## Fits

This is a command-style platform capability in the OS-integration suite, sitting alongside `@flighthq/storage` (its plaintext sibling) and `@flighthq/notification` (its permission/async-prompt sibling). It is two cleanly separable concerns in one cell — a one-shot **authentication prompt** and a **secure secret store** — but they belong together because the secure store's headline feature is gating each secret read behind a biometric prompt, so they share the backend seam.

- **Dependencies.** `@flighthq/types` (header layer — all shapes defined here first). No other Flight package dependency: this is a leaf OS-integration capability, not a graph/render package. Like the rest of the suite it must be import side-effect-free and `"sideEffects": false` with a single root `.` export.
- **Neighbor packages.** `@flighthq/storage` (plaintext key/value — biometrics is the encrypted, auth-gated counterpart; the two stay separate so an app does not pull keychain code to read a plaintext setting), `@flighthq/notification` (the canonical async-prompt + `is*Supported`/`request*Permission` shape this package mirrors), `@flighthq/app` (app identity that scopes the keychain service name). No `-formats` neighbor — there are no asset files to parse; credentials are opaque platform-managed blobs.
- **Backend seam.** A single `BiometricsBackend` interface in `@flighthq/types`, with `getBiometricsBackend` / `setBiometricsBackend` / `createWebBiometricsBackend`. The web default backend guards every call and returns sentinels (`false` / `null` / `[]`) when no platform API is present: WebAuthn / `PublicKeyCredential` (platform authenticator) backs the auth prompt where available, and the secure store falls back to a clearly-labeled non-hardware path (see Open questions) or returns `null` so the caller knows it is unavailable. Native hosts replace it via `setBiometricsBackend`.
- **Host adapter.** `@flighthq/host-electron` gains `createElectronBiometricsBackend(electron)` over `safeStorage` (encrypted secret store) plus `systemPreferences.promptTouchID` (macOS auth); future `host-tauri` / `host-capacitor` fill the keychain/keystore + `BiometricPrompt`/`LAContext` paths. The adapter carries no hard host dependency — the runtime module is injected, matching `registerElectronBackends`.
- **Rust crate.** `flighthq-biometrics`, mirroring this package 1:1 per the conformance map. Native default backend gated behind a `native` cargo feature (keychain-rs / keyring on desktop, platform APIs elsewhere); the web/wasm build uses the WebAuthn path. Same `*Backend` trait + `set_biometrics_backend` seam as the rest of the host layer.
