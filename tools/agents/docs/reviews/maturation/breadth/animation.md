# New Package Spec: @flighthq/animation

**Represents** — Data-driven node animation for the 3D / general scene graph: animation clips made of per-target channels and keyframe samplers (step/linear/cubic-spline), an evaluator that drives `SceneNode` TRS, mesh morph-target weights, and skeleton joint poses, plus clip blending, layering, and a lightweight state-machine over named clips. Distinct from the 2D `@flighthq/timeline` (MovieClip frames) and `@flighthq/tween` (per-property easing tweens).

**Requested by** — spatial-3d (the "no 3D animation system at all" gap: `timeline`/`tween` are 2D/display-object oriented; nothing drives node TRS, morph weights, or skeleton poses; `joints0`/`weights0` are reserved but unbacked).

## Fits

- **Position in the architecture.** A value/math + evaluator cell over the existing 3D foundation. It animates the scene graph but does not own it, render it, or import it.
- **Dependencies.** `@flighthq/types` (header), `@flighthq/geometry` (Vector3/Quaternion/Matrix4 compose/decompose, slerp, pools), `@flighthq/node` (TRS↔`localMatrix` via `HasTransform3D`), `@flighthq/easing` (optional easing on linear segments), `@flighthq/signals` (opt-in clip-event signals). It must **not** depend on `@flighthq/scene`, `@flighthq/mesh`, `@flighthq/render*`, or `@flighthq/sdk` — it writes into `Transform3DNode` / arena-resolved targets through the `@flighthq/node` feature aliases, keeping it graph-family-agnostic and tree-shakable. A skeleton lives in a neighbor package (below), and `animation` consumes it through types only.
- **Neighbor packages.**
  - `@flighthq/skeleton` (net-new sibling, also owed by spatial-3d) — joint hierarchy, inverse-bind matrices, skin palette computation, the renderer-facing `joints0`/`weights0` consumer. `animation` poses a `Skeleton`; `skeleton` turns the pose into a palette. Split so a static skinned mesh can ship without the animation evaluator and vice-versa.
  - `@flighthq/animation-formats` (the `-formats` parser sibling, mirroring `particles-formats` / `spritesheet-formats`) — parse/serialize clips from authored sources (glTF `animations`, future Collada/FBX-export JSON) into `AnimationClip` + `Skeleton`. Keeps the parser weight out of the runtime. The glTF _mesh/material/scene_ import is owed to a separate `@flighthq/gltf` package (spatial-3d's top ask); `animation-formats` exposes only the animation/skeleton extraction so both can share it.
- **Backend seam.** None in Bronze/Silver: animation is deterministic CPU math with no platform capability, so no `*Backend` is warranted (a backend seam here would be premature). Gold's optional GPU skinning/morph evaluation is a _render-package_ concern (`skeleton-gl`/`skeleton-wgpu` palette upload), not an `animation` `set*Backend` — the evaluator stays pure.
- **Rust crate.** `flighthq-animation` (1:1 mirror), with `flighthq-skeleton` and `flighthq-animation-formats` siblings. Channels/samplers are value types over slotmap `NodeId` targets; the evaluator is free functions over `(&mut NodeArena, &AnimationClip, time)`. Conformance-testable as a near-leaf (deterministic, no GPU) once targets resolve.

Naming follows the house rule that exported names carry the full unabbreviated type word (`AnimationClip`, `AnimationChannel`, `createAnimationPlayer`, `sampleAnimationChannel`, `evaluateAnimationClip`). Types are defined in `@flighthq/types` first.
