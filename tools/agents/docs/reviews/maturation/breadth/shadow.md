# New Package Spec: @flighthq/shadow

**Represents:** Shadow mapping for the 3D scene pipeline — light-space view-projection derivation, shadow-map render targets, the per-light depth (shadow-caster) pass, cascade/atlas packing, the sampler-compare (PCF/PCSS) seam, and the light/material opt-in flags that wire shadows into the shared scene light block.

**Requested by:** rendering-gpu, spatial-3d

## Fits

- **Architecture slot.** A 3D-render-technique cell that sits _between_ the backend-agnostic `@flighthq/render` core and the per-subject scene leaf renderers (`scene-gl` / `scene-wgpu`). It owns the CPU-side, backend-agnostic shadow math and the descriptor data; the actual depth-pass draw and compare-sampler binding are realized per backend. This mirrors how `render` owns `packSceneLightBlock` / `prepareSceneRender` (backend-agnostic) while `scene-gl`/`scene-wgpu` own the GPU draw. Shadow follows the same split rather than living inside one backend.
- **Why a package, not a fold-in.** The reviews offer "shadow support inside `lighting`/`scene-*`" as an alternative, but shadow mapping is a self-contained subsystem (light-space projection, target allocation, cascade splitting, atlas packing, compare-sampler seam, bias/filter policy) that both backends and the Rust port consume identically. A bounded cell keeps it tree-shakable: a 2D-only app, or a 3D app with unlit/baked lighting, never pulls a line of shadow code. Folding it into `lighting` (pure light _data_) or `scene-*` (per-backend draw) would either pollute a data package with technique or duplicate the math across backends.
- **Backend realization, not a platform `*Backend` seam.** Unlike platform capabilities (`dialog`, `clipboard`), shadow has no swappable web/native runtime seam. Its "backends" are GPU backends, addressed the same way the rest of the render stack is: backend-agnostic core in `@flighthq/shadow`, GL realization functions in `@flighthq/scene-gl` (or a thin `shadow-gl` neighbor — see Open questions), WGPU in `@flighthq/scene-wgpu`. The one true _seam_ is the **sampler-compare** abstraction (`ShadowCompareSampler`), a render-resource descriptor the GPU cores translate to a GL compare-mode sampler / `GPUSampler { compare }`.
- **Dependencies:** `@flighthq/types` (header layer — all shapes first), `@flighthq/geometry` (Matrix4, Frustum, Aabb, BoundingSphere, Vector3 — light-space math and cascade fitting), `@flighthq/lighting` (the light descriptors that opt in), `@flighthq/camera` (cascade splits are derived from the view camera's near/far + frustum corners), `@flighthq/render` (the `RenderTargetDescriptor` / `RenderState` it allocates against). It must **not** depend on `scene-gl`/`scene-wgpu` — the arrow points the other way (the backends depend on `shadow`).
- **Neighbor packages:** none of the `-formats` kind (shadow data is not imported/parsed). Its natural neighbors are the existing `lighting`, `camera`, `texture` (the depth/compare sampler types), and the `render-gl`/`render-wgpu` cores it relies on for target plumbing.
- **Existing hooks it completes.** The lighting descriptors already carry shadow params today: `DirectionalLight`/`SpotLight`/`PointLight` have `castsShadow`, `shadowBias`, `normalBias`, `pcfRadius`. `RenderTargetDescriptor` already supports `depth: 'depth-stencil-sampled'` and HDR `format`s. `SceneLightBlock` is explicitly designed to grow "behind feature defines in later passes without changing this type's shape." This package is the _runtime that backs those reserved fields_ — the data design already anticipates it.
- **Rust crate:** `flighthq-shadow`, 1:1 conformance. Backend-agnostic shadow math is value/matrix-typed (cascade splits, light-space matrices, atlas rects) — deterministic and headlessly fingerprint-able, so it is a strong early conformance target. GL realization mirrors into `displayobject-gl`/`scene-gl` (glow); WGPU into `scene-wgpu` (wgpu); `displayobject-skia` provides the software/reference shadow path. Render correctness is validated through the parity matrix (shadowed scene: `rust:wgpu` vs `ts:wgpu` vs `rust:skia` reference).
