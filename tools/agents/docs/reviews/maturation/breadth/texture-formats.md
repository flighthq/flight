# New Package Spec: @flighthq/texture-formats

**Represents:** KTX2 / Basis Universal / DDS / PVR compressed-texture container parsing, transcode-target selection, mip-chain extraction, and per-backend upload of compressed blocks — the `-formats` neighbor of `@flighthq/texture`.

**Requested by:** asset-pipeline

## Fits

This is the GPU-native asset front door that the asset-pipeline review (Coverage 68/100) calls the largest production gap: "No compressed / GPU-native texture format support … everything funnels through `CanvasImageSource`/decoded RGBA, which is memory-heavy and slow to upload." The `ImageResource` and `PixelFormat` types already **reserve a `compressed` slot** for exactly this ("compressed payloads are reserved for a separate `compressed` slot on `ImageResource`"), so the seam is pre-designed; this package fills it.

- **Package name & shape:** `@flighthq/texture-formats`, the `-formats` neighbor of `@flighthq/texture`, mirroring `spritesheet-formats`/`particles-formats`. `"sideEffects": false`, single root `.` export, tree-shakable.
- **Types-first:** all cross-package contracts (`CompressedTexture`, `CompressedTextureFormat`, `TextureContainerFormat`, `TranscodeTarget`, `*Backend` seams) land in `@flighthq/types` before implementation. The `compressed` slot is added to the existing `ImageResource`/`Texture` types there.
- **Dependencies (parse/select core):** `@flighthq/types`, `@flighthq/entity`, `@flighthq/resources` (it produces/augments `ImageResource`). It does **not** depend on any renderer.
- **Upload is per-backend, not here.** Following the existing GPU-upload seam (`bindGlTexture`/`createWgpuTextureEntry`), the compressed-block upload functions live in the renderer packages: `uploadGlCompressedTexture` in `@flighthq/render-gl`, `uploadWgpuCompressedTexture` in `@flighthq/render-wgpu`. `texture-formats` defines the data they consume and the GPU-capability query they answer against; it owns no GPU handle.
- **Transcoder is a swappable `*Backend` seam.** Basis Universal transcoding ships as a ~1 MB wasm payload, so it must stay off the default bundle (same posture as the HarfBuzz shaper). `registerBasisTranscoder` / `set*TranscoderBackend` over a swappable backend keeps the wasm opt-in; the container **parsers** (KTX2/DDS/PVR header + block layout) are pure TS and always present.
- **Rust crate mirror:** `flighthq-texture-formats`. Native uses the same containers; transcode backend is `basis-universal`/`basisu` (or `ktx2`+`basis_universal` crates) gated behind a cargo feature, mirroring the wasm gate. Block upload is in `render-gl`/`render-wgpu` crates. This is a **value-typed leaf** (bytes in → bytes/descriptor out), so it is on the _mixable_ conformance path alongside `surface`.
