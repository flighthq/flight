# New Package Spec: @flighthq/spring

**Represents:** A value-typed, frame-stepped procedural-motion solver — critically-damped springs plus decay/inertia and rubber-band clamping — usable standalone (step a value toward a target each frame) and as a duration-free driver for `@flighthq/tween`.

**Requested by:** animation-motion

**Fits:**

`@flighthq/spring` is a small, math-only leaf package that slots beside `@flighthq/easing` in the motion stack. Where `easing` maps a normalized `t → value` over a fixed duration, `spring` integrates a continuous `(value, velocity) → next value` over a real `deltaTime` with no fixed duration — the two are complementary motion models, not competitors.

- **Dependencies:** `@flighthq/types` only (the header layer). No runtime, no node, no render dependency — like `easing`, it is pure number-in / number-out value math, so it is trivially mixable across the JS↔wasm boundary and headlessly testable.
- **Position in the SDK:** sits one layer below `@flighthq/tween`. `tween` may optionally consume `spring` to offer a spring-driven tween, but `spring` must never import `tween` (dependency points downward only). The `@flighthq/sdk` barrel re-exports `spring`.
- **Neighbor packages:** `@flighthq/easing` (sibling motion primitive), `@flighthq/tween` (consumer/driver host), `@flighthq/velocity` (measures motion; `spring` produces it — a natural future bridge). No `-formats` neighbor is warranted: springs have no authored file format to import. Preset _catalogs_ (iOS/Material spring presets) stay in-package as named constants, not a `-formats` split.
- **Backend seam:** none. Spring integration is deterministic CPU math with no platform capability behind it, so there is no `*Backend` / `get*` / `set*` / `createWeb*` surface. (This is the same reason `easing` has no backend seam.)
- **Rust crate:** `flighthq-spring`, a 1:1 value-math mirror under `crates/`. Free functions over `&SpringConfig` + `&mut SpringState` / out-params; bit-comparable to TS under the conformance suite (deterministic, no GPU, fingerprintable — it belongs to the mixable leaf set).

Types are defined in `@flighthq/types` first (the header layer), then implemented in `@flighthq/spring` against them.
