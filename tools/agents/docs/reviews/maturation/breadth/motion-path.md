# New Package Spec: @flighthq/motion-path

**Represents:** Animate a `Transform2DNode` along an authored `Path` with orient-to-tangent and arc-length (constant-speed) parameterization — the bridge between `@flighthq/path` geometry and `@flighthq/tween` time control.

**Requested by:** animation-motion

## Fits

`@flighthq/motion-path` is the procedural "move this thing along that curve" layer the animation review found missing between `path` (geometry) and `tween`/`timeline` (time). `path` can build and flatten an outline; `tween` can drive a scalar `0→1` over a duration with easing/repeat/yoyo; nothing today samples a point + tangent on the curve at a given distance and writes it into a node's transform. This package is that sampler plus the node-binding and tween-driver glue.

- **Depends on:** `@flighthq/types` (header layer — all shapes defined here first), `@flighthq/geometry` (`Vector2`, `Matrix`, pools, `out`-param math, `getVector2AngleBetween`), `@flighthq/path` (`Path`, `flattenPath` for the polyline the sampler walks), and `@flighthq/node` (the `Transform2DNode` feature alias and its transform writers — `x`, `y`, `rotation`, pivot). It optionally composes with `@flighthq/tween` (a `MotionPathTween` driver) but does **not** hard-depend on it: the core sampler/follow API is tween-free so a caller can advance a motion path from a timeline, a spring, or a raw `deltaTime` loop. It must **not** depend on any renderer or on `@flighthq/sdk`.
- **Neighbor package:** none at Bronze/Silver. A future `@flighthq/motion-path-formats` (the `-formats` pattern used by `spritesheet-formats`/`particles-formats`) would parse authored motion guides from external tools — Flash/Animate motion guides, After Effects mask-path keyframes, SVG `<animateMotion>`/`mpath` — into a `MotionPath`. Listed as an open question; not built until a concrete interchange format earns it.
- **Backend seam:** none. This is pure CPU geometry + math; there is no platform capability to swap, so the package has no `*Backend`/`get*Backend`/`set*Backend`/`createWeb*Backend`. (Contrast with the platform suite.)
- **Signals:** path following is a synchronous per-frame sample, so no signal group at Bronze/Silver. Gold may expose an `enableMotionPathSignals(follow)` group for `onMotionPathComplete` / `onMotionPathLoop` / `onMotionPathWaypoint` so sequencing layers can react without polling progress.
- **Rust crate:** `flighthq-motion-path`. The arc-length table + sampler is a value-typed, GPU-free, bit-deterministic leaf — squarely in the mixable set and a 1:1 conformance target. The node-binding half mirrors the Rust `flighthq-node` arena transform writers (`&mut NodeArena`, `NodeId`).
