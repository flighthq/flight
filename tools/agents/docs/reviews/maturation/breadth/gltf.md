# New Package Spec: @flighthq/gltf

**Represents:** glTF 2.0 / GLB (and OBJ) import that turns a model file into Flight's own value types — `MeshGeometry` + `Material` + a `SceneNode` graph, with PBR materials, textures, skins, and animation clips — using the codebase's existing 3D primitives rather than a parallel object model.

**Requested by:** spatial-3d

## Fits

`@flighthq/gltf` is the **logical import layer** that sits on top of the already-mature 3D front half (`mesh`, `materials`, `texture`, `scene`, `camera`, `lighting`). It is the 3D analogue of `@flighthq/spritesheet` (logical) + `@flighthq/spritesheet-formats` (parser), so it ships as a pair:

- **`@flighthq/gltf`** — the import surface: `importGltfDocument(...) -> ImportedModel`, glue that maps parsed glTF data onto Flight `MeshGeometry`/`Material`/`SceneNode`/`Skin`/`AnimationClip`, plus the buffer/image resolution seam.
- **`@flighthq/model-formats`** — the pure, value-in/value-out **parsers**: `parseGltf` / `parseGlb` / `parseObj` producing a neutral `ParsedModel` intermediate (the `-formats` neighbor pattern). No SDK entity construction here — it returns plain parsed data, exactly as `spritesheet-formats` returns `SpritesheetData` and `particles-formats` returns `ParticleEmitterConfig`. This is the mixable, deterministic, headlessly-fingerprintable leaf.

**Dependencies:** `@flighthq/types` (header layer for all new types), `@flighthq/mesh`, `@flighthq/materials`, `@flighthq/texture`, `@flighthq/scene`, `@flighthq/geometry`. `model-formats` depends only on `@flighthq/types`. Must **not** import `@flighthq/sdk`. Both packages are `"sideEffects": false`, single root `.` export, tree-shakable.

**Backend seam:** glTF references external buffers (`.bin`) and images by URI. Decoding those is environment-specific, so import takes an explicit **`GltfResourceBackend`** (the URI/buffer/image resolution seam) following the platform-capability pattern: `getGltfResourceBackend()` / `setGltfResourceBackend()` / `createWebGltfResourceBackend()`. The web backend resolves URIs via `fetch` and decodes images via the DOM image path already used by `@flighthq/resources`; a native/Rust host registers its own (filesystem + codec). Image **decode** itself is delegated to `@flighthq/resources` (and a future `image-codec` seam), not re-implemented here. Skinning/animation **playback** is delegated to the skinning/animation packages (see Boundaries); this package only produces the data.

**Rust crates:** `flighthq-gltf` and `flighthq-model-formats`, mirroring the TS pair 1:1. `model-formats` is a value-typed leaf (parse bytes → `ParsedModel`), so it is a clean conformance target and is _mixable_ into a TS app as wasm; `flighthq-gltf`'s entity construction is all-or-nothing with the rest of the Rust scene runtime. Native default resource backend reads files via `std::fs` and decodes via the resources codec, gated behind a `native` cargo feature.
