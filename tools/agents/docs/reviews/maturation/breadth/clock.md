# New Package Spec: @flighthq/clock

**Represents:** A shared, scalable, pausable animation clock / time domain so tween, timeline, spritesheet, and particles advance under one scrubbable, time-scaled root — global slow-motion, pause, and seek across every motion subsystem at once.

**Requested by:** animation-motion

## Fits

`@flighthq/clock` is the missing _time orchestrator_ under the motion stack. Today each subsystem owns its own advance call — `updateTweens(manager, deltaTime)`, `updateSpritesheetPlayer(player, deltaTime)`, and the timeline/particles equivalents — and every caller threads a raw `deltaTime` number into each by hand. There is no shared object that scales, pauses, or seeks that delta once for all of them. The breadth review names this directly: "no shared animation clock unifying tween, timeline, spritesheet, and particles under one time domain. Each subsystem has its own update call with no orchestrator." This package is that orchestrator.

The design line: `clock` produces and shapes **time** (a plain, scaled, pausable delta and an absolute time position); it does **not** know what consumes that time. Subsystems stay decoupled — they still expose their own `update*(…, deltaTime)` functions — and `clock` feeds them through small, explicit subscription hooks. This keeps `tween`/`spritesheet`/`timeline`/`particles` free of any clock dependency (the dependency points the other way, or through `@flighthq/types`).

- **Depends on:** `@flighthq/types` (header — all clock types and `*Kind` strings live here first), and optionally `@flighthq/signals` for the opt-in signal group. It does **not** depend on `tween`, `timeline`, `spritesheet`, `particles`, any renderer, `@flighthq/application`, or `@flighthq/sdk`. The motion packages do not depend on `clock` either; the wiring is a caller concern (or a thin SDK-level convenience), preserving each package's import-in-isolation property.
- **Relationship to `@flighthq/application`:** `application` owns the _frame source_ (`startApplicationLoop` / `stopApplicationLoop` and the wall-clock `requestAnimationFrame` tick). `clock` owns _time semantics_ (scale, pause, seek, fixed-step accumulation) downstream of that raw tick. The intended flow is: app loop emits a raw wall-clock delta → `advanceClock(clock, rawDeltaSeconds)` → scaled/clamped delta fans out to subscribers. `clock` never reads `performance.now()` itself by default; the caller supplies the raw delta, keeping it deterministic, testable, and host-agnostic (the same rule the Rust port needs for headless capture).
- **Neighbor packages:** none required. There is no parse/serialize surface, so no `-formats` neighbor. (A timeline-recording / keyframe-of-time-scale artifact would belong in `timeline`, not a clock neighbor.)
- **Backend seam:** none by default — time math is pure CPU and host-independent. The _one_ optional platform concern is a high-resolution monotonic time source (`performance.now` on web, `std::time::Instant` native). That is exposed as an opt-in `ClockTimeSourceBackend` with `getClockTimeSourceBackend` / `setClockTimeSourceBackend` / `createWebClockTimeSourceBackend`, used only by the convenience `tickClock(clock)` path; the core `advanceClock(clock, rawDelta)` path needs no backend and stays the canonical, deterministic entry point.
- **Signals:** opt-in via `enableClockSignals(clock)` (the `enable*`-group pattern), surfacing `onClockPause` / `onClockResume` / `onClockScaleChange` / `onClockSeek`. Bare per-frame advance uses direct callbacks, not signals, so the hot path carries no dispatch cost unless enabled.
- **Rust crate:** `flighthq-clock`. A value-typed leaf (delta in, scaled delta + subscriber fan-out), no GPU, headlessly fingerprintable — a clean 1:1 conformance target. The frame-source seam mirrors the host split: native default time source in-crate behind a `native` feature, web source in `host-web`.
