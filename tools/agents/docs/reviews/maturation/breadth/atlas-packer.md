# New Package Spec: @flighthq/atlas-packer

**Represents:** Runtime bin-packing that builds a `TextureAtlas`/`Tileset` from loose regions (MaxRects / Guillotine / Skyline), with rotation, padding, and extrude, feeding glyph atlases and dynamic content.

**Requested by:** asset-pipeline

## Fits

`@flighthq/atlas-packer` is the runtime _construction_ counterpart to `@flighthq/resources`' atlas _descriptors_. `resources` can describe and consume pre-packed `TextureAtlas`/`Tileset` (regions, pivots, `buildTilesetRegions`); this package is the missing piece that _produces_ those layouts from a set of loose rectangles at runtime — for dynamically generated content, coalescing many small textures, and the glyph-atlas path the text stack needs.

- **Depends on:** `@flighthq/types` (header), `@flighthq/geometry` (`Rectangle`, `Vector2`, pools, `out`-param math), `@flighthq/resources` (to emit/populate `TextureAtlas`/`Tileset` and read `ImageResource` sizing). It does **not** depend on any renderer or on `@flighthq/sdk`.
- **Composites pixels via** `@flighthq/surface`: actually blitting source regions into a packed sheet (and extrude/padding bleed) is a `Surface` operation. The packer computes layout (pure geometry, no pixels); a thin set of `compositeAtlas*` helpers uses `surfaceDraw`/`surfaceCopy` to realize the sheet. Keeping layout pixel-free preserves a value-typed leaf that is mixable/conformance-friendly.
- **Neighbor package:** none required at Bronze. A future `@flighthq/atlas-packer-formats` would parse/serialize external packer layout files (the `-formats` pattern used by `spritesheet-formats`/`particles-formats`) — but the canonical interchange formats (TexturePacker/Aseprite/Starling) already live in `spritesheet-formats`, so this neighbor only earns its place for packer-config artifacts (e.g. a `.tps` settings descriptor), not duplicate atlas parsers. Listed as an open question below.
- **Backend seam:** none. Packing is pure CPU geometry; there is no platform capability to swap, so this package has no `*Backend` / `createWeb*Backend`. (Contrast with `image-codec`, which _does_ need a seam — out of scope here.)
- **Signals:** packing is a synchronous request/response, so no signal group at Bronze/Silver. An incremental/streaming packer (Gold) may expose an `enableAtlasPackerSignals` group for `onAtlasGrow` / `onAtlasRepack`.
- **Rust crate:** `flighthq-atlas-packer` — a value-typed leaf (rectangles in, layout out), squarely in the mixable set: deterministic, no GPU, headlessly fingerprintable. 1:1 conformance target.
