# New Package Spec: @flighthq/scene-format

**Represents:** Scene serialization — save/load the scene graph to a portable, versioned document, plus the opt-in migration step the string-kind model already promises.

**Requested by:** missing-domains

**Fits:**

`@flighthq/scene-format` is the persistence seam for the scene graph. It sits directly above `@flighthq/node` (the shared hierarchy/transform/bounds spine) and the entity packages whose kinds it must reconstruct (`@flighthq/displayobject`, `@flighthq/sprite`, `@flighthq/scene`, `@flighthq/text`). It does **not** depend on any renderer or render-state package — serialization is a data-graph concern, decoupled from how a scene is drawn.

- **Dependencies:** `@flighthq/types` (header), `@flighthq/node` (hierarchy walk + `createNode`), `@flighthq/geometry` (matrix/rectangle (de)serialization helpers). Optionally `@flighthq/signals` for `enable*` progress groups. It must **not** import `@flighthq/sdk`.
- **The string-kind model is the whole point.** The docs already state a kind is "simultaneously the registry key, the serialized form, and the user-facing intent vocabulary," and that scenes are "versioned intent, migrated at load." This package is the code that makes that real: a node's `kind` string is written verbatim into the document and used at load to look up a registered reconstructor. No symbol↔string seam.
- **Neighbor package — the importer/parser belt:** `@flighthq/scene-format-formats` is the place for foreign scene formats (Flash `.swf` symbol trees, OpenFL/Starling display lists, Spine skeletons-as-scene, glTF→`scene`-graph). This mirrors the established `particles-formats` / `spritesheet-formats` split: the core package owns Flight's own canonical document; the `-formats` neighbor owns lossy bridges from other ecosystems, each as a `xxxParse` / `xxxSerialize` / `xxxSchema` trio.
- **Backend seam:** `SceneStoreBackend` (`get*`/`set*`/`createWeb*`) abstracts _where bytes land_ — `localStorage`/OPFS on web, files natively. The document model itself is backend-free plain data; the backend only moves the serialized blob. This keeps the (de)serializer pure and testable while the persistence target is swappable, exactly like the platform suite. Most users will hand the string/bytes to `@flighthq/filesystem` or `@flighthq/storage` directly, so the backend is a convenience, not a requirement.
- **Rust crate:** `flighthq-scene-format` (+ `flighthq-scene-format-formats`). A near-1:1 value-typed port — this is one of the "mixable, value-in/value-out" candidates, since the document is plain data and (de)serialization is deterministic. Serde is the natural backbone on the Rust side.
