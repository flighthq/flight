# New Package Spec: @flighthq/text-markup

**Represents:** HTML/markup parsing for rich text (the OpenFL `htmlText` equivalent): parse a defined tag subset into plain text plus `TextFormatRange[]` (and link/image/break metadata) ready to feed `setRichTextString` + `setRichTextFormatRange`, plus the inverse serializer back to markup.

**Requested by:** text-typography, openfl-lime-parity

## Fits

The text-typography review (Coverage 58/100) calls the markup path explicitly thin: "`RichTextStyleSheet` is `Record<string, TextFormat>` but there is no HTML-to-rich-text parser (OpenFL's `htmlText`), so the common 'set styled text from markup' path is unbuilt," and lists "An HTML/markup parser for rich text (`htmlText`-equivalent) feeding `setRichTextFormatRange`" among the missing pieces. The openfl-lime-parity review requires `TextField` with `htmlText`, hyperlinks (`a href` / `TextEvent.LINK`), and CSS-style stylesheets for full parity.

The package converts markup into the **plain-data range model that already exists** (`TextFormat`, `TextFormatRange`, `RichTextStyleSheet`) — it does not introduce a parallel styling system. It is a pure value-in / value-out transform (string in → text + ranges out), so it carries no entity/runtime state and sits on the _mixable_ conformance path.

- **Package name & shape:** `@flighthq/text-markup`. It is the markup-ingest neighbor of `@flighthq/text`, paralleling the `-formats` family (`spritesheet-formats`, `particles-formats`) as the "importer/parser" cell for a domain. `"sideEffects": false`, single root `.` export, tree-shakable. (`-markup` rather than `text-formats` because the domain word here is _markup_, not a binary asset format; the OpenFL feature is literally `htmlText`.)
- **Types-first:** every cross-package contract (`TextMarkupDocument`, `TextMarkupNode`, `TextMarkupTag`, `TextMarkupParseResult`, `TextMarkupLink`, `TextMarkupImagePlacement`, `TextMarkupDialect`, `TextMarkupIssue`, `TextMarkupBackend`) lands in `@flighthq/types` before implementation. The existing `TextFormat` may gain optional fields markup needs (see Open questions); those additions are made in `@flighthq/types` too.
- **Dependencies:** `@flighthq/types` only for the parse/serialize core. It produces `TextFormat`/`TextFormatRange` _data_ — it does **not** depend on `@flighthq/text`, `@flighthq/textlayout`, or any renderer. A consumer (an example, or `@flighthq/sdk`) glues `parseTextMarkup` output to `setRichTextString` + `setRichTextFormatRange`; the parser stays display-object-free so it is reusable for measurement, search, or server-side use.
- **Backend seam:** the _default_ parser is a self-contained, dependency-free tag tokenizer (no DOM, works native and headless). A swappable `TextMarkupBackend` seam (`getTextMarkupBackend`/`setTextMarkupBackend`/`createWebTextMarkupBackend`) lets a host substitute the browser's real HTML parser (`DOMParser`) when permissive, security-hardened, spec-grade HTML parsing is wanted. The default backend is registered lazily and is not the web one (native-first), matching the platform-suite seam posture.
- **Rust crate mirror:** `flighthq-text-markup`. The default tokenizer ports 1:1 (pure value transform); `createWebTextMarkupBackend` has no native analogue (the web backend is wasm/`host-web` only). On the _mixable_ leaf path — a future `text-markup-rs` wasm drop-in is feasible because the seam is plain data.
