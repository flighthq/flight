...
Resources: `@flighthq/image` (ImageResource entity lifecycle — create, clone, dispose, invalidate, DOM-based loading (URL/bytes/Base64/Blob), MIME detection, same-origin check), `@flighthq/font`, `@flighthq/video`, `@flighthq/audio`, `@flighthq/textureatlas`, `@flighthq/tileset`, `@flighthq/loader` (batch queue).
