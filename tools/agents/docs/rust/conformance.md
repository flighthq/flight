...
- **`entity` — `get_entity_runtime` return type.** TS `getEntityRuntime` uses a non-null assertion (`source[EntityRuntimeKey]!`), always returning `Readonly<EntityRuntime>` with the expectation that the caller knows a runtime exists (the companion `hasEntityRuntime` is the safe guard). Rust `get_entity_runtime` returns `Option<&EntityRuntime>`, matching Rust's borrow idioms. This is an intentional TS/Rust seam divergence: TS keeps the asserting form (faster, matches callers-that-know; `hasEntityRuntime` is the guard) and Rust keeps `Option` (borrow-checker-safe). Recorded so the conformance checker does not flag it as a missing function.
- **`entity` — `get_entity_runtime_mut` (Rust-only).** Rust exposes `get_entity_runtime_mut(entity: &mut Entity) -> Option<&mut EntityRuntime>` for borrow-checker ergonomics — subsystems that need to write a slot field call `get_entity_runtime_mut` to get a mutable borrow rather than using `unsafe`. TS mutates the slot object directly (via `entity[EntityRuntimeKey]!.field = value`) without needing a separate mutable accessor. The TS counterpart is intentionally omitted: there is no caller benefit in TS, and adding it would add surface without purpose. Recorded so the conformance checker tracks it as `RUST_ONLY_FUNCTIONS["entity"] = ["get_entity_runtime_mut"]` (or equivalent) rather than as a coverage gap.
- **`interaction` — `NodeInteractionState` storage.** TS stores per-node interaction state (`selfInteractive`, `childrenInteractive`, `hitArea`, `cursor`) in a `WeakMap<NodeAny, NodeInteractionState>` — lazy allocation on first setter, GC-collected with the node. Rust has no `WeakMap` equivalent; the state is stored in a `HashMap<NodeId, NodeInteractionState>` (slotmap arena id) with explicit cleanup via `dispose_node_interaction`. The API is identical (`set_node_interactive`, `are_node_children_interactive`, `get_node_hit_area`, `get_node_cursor`, etc.); only the storage differs.
- **`interaction` — `HitArea` union type.** TS `HitArea = Readonly<Rectangle> | Readonly<NodeAny>` is a structural union (discriminated at runtime by the presence of a `kind` field). Rust uses an explicit enum `HitArea { Rectangle(Rectangle), Node(NodeId) }`. The discrimination rule is the same; the surface difference is that TS inspects `'kind' in hitArea` while Rust uses `match`.
- **`interaction` — `CursorBackend` is a trait, not an interface object.** TS `CursorBackend { setCursor(cursor: Cursor | null): void }` maps to Rust `trait CursorBackend { fn set_cursor(&self, cursor: Option<&str>); }`, stored as `Arc<dyn CursorBackend>` via `set_cursor_backend`. Web cursor is relocated to `host-web`, matching the platform-suite backend-seam pattern; there is no in-box DOM element to set `style.cursor` on.
