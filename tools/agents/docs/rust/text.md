...
- **`text-shaping`** — the shaper seam (`set_text_shaper` / `register_text_shaper`), the shaped-run header types (in `flighthq-types`), and the lightweight default (`ttf-parser` advances) shaper.
- A **full-glyph shaper backend** — rustybuzz, as a registerable backend kept out of the base bundle. Either a `text-shaping` cargo feature or a focused sibling crate (e.g. `text-shaping-harfbuzz`), per the `-subpackage` neighbor-crate convention.
