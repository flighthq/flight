...
`displayobject-canvas` is intentionally TS/host-web-only. The Canvas2D substrate does not exist in the Rust box; software-render parity is provided by `displayobject-skia`. The conformance map (`tools/agents/docs/rust/conformance.md`) should record: `displayobject-canvas` has no Rust crate; `rust:skia ~ ts:canvas` is the reference pairing; the canvas functional scenes serve as the TS side of that cross-impl comparison. Documentation decision for whoever next owns the Rust conformance map.
