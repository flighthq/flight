...
8. **Package Map staleness (doc revision, user's gate).** `tools/agents/docs/index.md` still lists the renderers as `render-canvas`/`render-dom`/`render-webgl`; the `<subject>-<backend>` reorg has landed in code but the Package Map has no `displayobject-dom` line. The map should be updated to the `displayobject-<backend>` naming.
