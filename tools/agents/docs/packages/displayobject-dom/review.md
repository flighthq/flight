...
- **CSS-filter equivalence + exact SVG color-matrix path.** `hasDomCssFilterEquivalent` reports the native-CSS set (`Blur`, `DropShadow`, `OuterGlow` → `drop-shadow(0 0 …)`). `domSvgFilter.ts` adds the DOM-native exact path for `ColorMatrixFilter`: `getDomSvgColorMatrixFilter` injects a `<filter>`/ `<feColorMatrix>` into a shared hidden `<svg>` at `document.body`, converts Flash's 0–255 offset column to SVG's 0–1, sets `color-interpolation-filters: sRGB`, returns `url(#id)` (or `null` under SSR), with `releaseDomSvgColorMatrixFilter(id)` to remove the node. No GPU readback, no rasterization.
