...
3. **Refresh the Package Map line for `@flighthq/easing`.** "easing functions for use with tween or any animation system" no longer hints at the combinator / factory / piecewise / LUT / derivative surface. One-line description refresh in `tools/agents/docs/index.md`.
