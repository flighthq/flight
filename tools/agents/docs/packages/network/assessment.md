...
2. **Async on the `NetworkBackend` seam** — `detectReachability` is the first async surface; confirm the trait may carry async and record the Rust `Send`/`!Send` posture. The header restore (Recommended) is what makes this trait method real — settle the async posture alongside it. (charter Open direction 2.)
3. **Fallback routing in `detectNetworkReachability`** (fork D seam) — when a native backend lacks `detectReachability`, the fallback silently probes over the web `fetch` path and allocates a fresh web backend per call (`b2824e3d8:head/packages/network/src/network.ts:193`). Rule it: web-fetch fallback always vs. sentinel-when-backend-lacks-probe. (charter Open direction 3.)
