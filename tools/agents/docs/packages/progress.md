...
| Package     | State | Last visited | Note |
| ----------- | ----- | ------------ | ---- |
| application | direction | 2026-07-02 | 4 decisions blessed: rebuild missing types (lost work), remove dead LoopState.accumulated, entry-point orchestrator candidate for decomposition, TS-leads. Approved: 3 items (types, dead code, Package Map). Review was reject/38 — blocking type issues. |
| log | direction | 2026-07-02 | 5 decisions blessed: rebuild missing types (lost work), decompose 61-export monolith, near scope ceiling, remove divider comments, TS-leads. Approved: 3 items (types, dividers, Package Map). Review was reject/40 — blocking type issues. |
| media | direction | 2026-07-02 | 4 decisions blessed: fix correctness holes (bugs), lost work rebuild (destination TBD), AudioContext ownership open, TS-leads. Approved: 4 items (pause/resume fix, destroyAudioMixer, bound runtime map, Package Map). Central open direction: should media exist if audio/video own resources? |
