...
- The **Package Map** (`tools/agents/docs/index.md`) has an entry for `@flighthq/filters-gl` but **no entry for `@flighthq/effects-gl`** (nor `@flighthq/effects`, `effects-wgpu`, `effects-canvas`). A 44-runner post-process backend is a substantial package and should have a Package Map line. Candidate revision: add `effects` + the three `effects-<backend>` entries.
