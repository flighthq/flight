...
8. **Admin-doc gaps surfaced by the review.** The Package Map (`tools/agents/docs/index.md`) has no entry for `effects-gl` (nor `effects`, `effects-wgpu`, `effects-canvas`), and `render-backend-support.md` does not record that GL effects exist but Ssr/Taa/Ssao/Smaa are stand-ins. Worth a doc revision so a scoping agent does not assume real depth-driven AO/reflections on GL. (Not a code question, but it touches this package's discoverability.)
