...
- **Package Map entries** — add `effects`, `effects-gl`, `effects-wgpu`, `effects-canvas` lines to `tools/agents/docs/index.md`. _Parked:_ not a `render-backend-support.md`-style factual note about this one package — it adds entries for sibling packages too, so it is a cross-package admin-doc edit better made once across the `effects` family than piecemeal here.
