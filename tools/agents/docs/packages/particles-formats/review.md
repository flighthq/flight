...
- The **Package Map** line in `tools/agents/docs/index.md` for `@flighthq/particles-formats` lists only "Particle Designer plist, Spine 4.x particle JSON, Unity Shuriken JSON." It is stale — the package now also ships libGDX, Starling PEX, and Pixi. Update to the six-format set.
