...
The `math` vs `random` scope question was resolved in favor of **build out to AAA `math` scope**, matching the depth review's preferred path. The package is added to the Package Map in `tools/agents/docs/index.md` and the `package.json` description reflects the full scope.
