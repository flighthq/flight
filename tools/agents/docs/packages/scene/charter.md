...
13. **Stale Package Map line (fork G doc reconciliation).** `tools/agents/docs/index.md` still calls `scene` "a doorway… the road mostly untaken… not yet built out," contradicting fork G's 2026-06-24 ruling. The line should be rewritten to describe the realized scene-graph + query surface — a doc-revision call for the user's gate.
