...
- **[2026-06-25] Pruned-item rulings (the OpenFL display surface, decomposed).** Settled with the user; resolves Open directions #1–#6:
  ...
  - **`Bitmap` = `image` + `smoothing` only.** `sourceRectangle` dropped — it is not an AS3 `Bitmap` field, and sub-rect display is the **Sprite + `textureatlas`** path (a sprite frame is a sub-rect of an atlas). `smoothing` stays (cleanly honored on every backend as the sampler filter mode).
