...
//
//   node tools/agents/docs/packages/todo.mjs
//
...
lines.push('');
lines.push(`_Generated ${today} by \`node tools/agents/docs/packages/todo.mjs\` — do not edit by hand. Sources: each cell's \`review.md\` (status/score), \`assessment.md › Recommended\` (the sweep-safe work queue), \`charter.md\` (chartered-unbuilt detection), and \`register.md › Build queue\`. Regenerate after assessments or the register change._`);
lines.push('');
lines.push('One line per actionable item. For detail, read only the named package\'s cell: `tools/agents/docs/packages/<name>/assessment.md` (and its `charter.md` for the rules). `Recommended` items are pre-sorted as sweep-safe but **not yet approved**; check `assessment.md › Approved` for blessed work.');
lines.push('');
