---
id: timeline
title: '@flighthq/timeline'
type: depth
target: timeline
status: proposed
tier: bronze
source:
  - tools/agents/docs/reviews/maturation/depth/timeline.md
  - tools/agents/docs/reviews/depth/timeline.md
depends_on: []
updated: 2026-06-23
---
...
partial — 48/100. A clean, correctly-factored playback spine (`TimelineSource` seam / `Timeline` playhead / `MovieClip` node), but two defining MovieClip features are absent — frame scripts and a working per-frame event lifecycle (the signals exist as dead fields nothing emits) — plus no loop/play-mode control.
...
> Build `@flighthq/timeline` up to the **Bronze** tier per the Scope + Design above (the package exists — extend it). Define any new shared types in `@flighthq/types` first. Follow the CLAUDE.md conventions. Satisfy every Acceptance checkbox. Surface cross-package or design decisions rather than guessing.
