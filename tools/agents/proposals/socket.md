---
id: socket
title: '@flighthq/socket'
type: new-package
target: socket
status: proposed
tier: bronze
source:
  - tools/agents/docs/reviews/maturation/breadth/socket.md
...
WebSocket / message-stream transport: open a connection, send and receive text and binary frames, observe state and backpressure, reconnect, and close with codes — all over a swappable `SocketBackend` seam, exactly as the platform suite swaps every other host capability.
