---
id: interaction
title: '@flighthq/interaction'
type: depth
target: interaction
status: proposed
tier: bronze
source:
  - tools/agents/docs/reviews/maturation/depth/interaction.md
  - tools/agents/docs/reviews/depth/interaction.md
depends_on: []
updated: 2026-06-23
---
...
Solid — 68/100. A genuinely deep pointer-dispatch layer bolted to a placeholder, bounding-box-only hit-tester (`shapeFlag` is dead wiring; tilemap/quad-batch are `// TODO` stubs).
...
> Build `@flighthq/interaction` up to the **Bronze** tier per the Scope + Design above (the package exists — extend it). Define any new shared types in `@flighthq/types` first. Follow the CLAUDE.md conventions. Satisfy every Acceptance checkbox. Surface cross-package or design decisions rather than guessing.
