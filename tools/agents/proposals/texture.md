---
id: texture
title: '@flighthq/texture'
type: depth
target: texture
status: proposed
tier: bronze
source:
  - tools/agents/docs/reviews/maturation/depth/texture.md
  - tools/agents/docs/reviews/depth/texture.md
depends_on: []
updated: 2026-06-23
---
...
> Build `@flighthq/texture` up to the **Bronze** tier per the Scope + Design above (the package exists — extend it). Define any new shared types in `@flighthq/types` first. Follow the CLAUDE.md conventions. Satisfy every Acceptance checkbox. Surface cross-package or design decisions rather than guessing.
