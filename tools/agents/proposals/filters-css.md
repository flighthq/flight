---
id: filters-css
title: '@flighthq/filters-css'
type: depth
target: filters-css
status: proposed
tier: bronze
source:
  - tools/agents/docs/reviews/maturation/depth/filters-css.md
  - tools/agents/docs/reviews/depth/filters-css.md
depends_on: []
updated: 2026-06-23
---
...
partial — 35/100. A correct-but-narrow shadow/blur translator (3 of 15 descriptors, 2 of 10 native CSS shorthand functions, no SVG `url()` path), not yet a full CSS-filter library.
...
> Build `@flighthq/filters-css` up to the **Bronze** tier per the Scope + Design above (the package exists — extend it). Define any new shared types in `@flighthq/types` first. Follow the CLAUDE.md conventions. Satisfy every Acceptance checkbox. Surface cross-package or design decisions rather than guessing.
