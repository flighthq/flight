---
id: lifecycle
title: '@flighthq/lifecycle'
type: depth
target: lifecycle
status: proposed
tier: bronze
source:
  - tools/agents/docs/reviews/maturation/depth/lifecycle.md
  - tools/agents/docs/reviews/depth/lifecycle.md
depends_on: []
updated: 2026-06-23
---
...
> Build `@flighthq/lifecycle` up to the **Bronze** tier per the Scope + Design above (the package exists — extend it). Define any new shared types in `@flighthq/types` first. Follow the CLAUDE.md conventions. Satisfy every Acceptance checkbox. Surface cross-package or design decisions rather than guessing.
