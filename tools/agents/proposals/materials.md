---
id: materials
title: '@flighthq/materials'
type: depth
target: materials
status: proposed
tier: bronze
source:
  - tools/agents/docs/reviews/maturation/depth/materials.md
  - tools/agents/docs/reviews/depth/materials.md
depends_on: []
updated: 2026-06-23
---
...
solid — 74/100. A complete OpenFL `ColorTransform` algebra fused with a broad, glTF-aligned 3D material _constructor_ catalog; falls short of authoritative because it is a descriptor layer with no material math, no model conversions, no clone/copy/equals for the 3D families, and a thin color seam.
...
> Build `@flighthq/materials` up to the **Bronze** tier per the Scope + Design above (the package exists — extend it). Define any new shared types in `@flighthq/types` first. Follow the CLAUDE.md conventions. Satisfy every Acceptance checkbox. Surface cross-package or design decisions rather than guessing.
