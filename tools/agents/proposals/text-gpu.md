---
id: text-gpu
title: '@flighthq/text-gpu'
type: new-package
target: text-gpu
status: proposed
tier: bronze
source:
  - tools/agents/docs/reviews/maturation/breadth/text-gpu.md
...
GPU glyph rendering — a shared glyph/SDF/MSDF atlas plus per-glyph quad batching so the `render-gl` and `render-wgpu` backends draw real shaped text from glyph runs instead of per-node Canvas2D `<canvas>` overlays uploaded as textures.
