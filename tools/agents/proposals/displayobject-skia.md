---
id: displayobject-skia
title: '@flighthq/displayobject-skia'
type: new-package
target: displayobject-skia
status: proposed
tier: bronze
source:
  - tools/agents/docs/reviews/maturation/breadth/displayobject-skia.md
...
The Rust in-box software display-object renderer (tiny-skia) over `render` — rasterizes shape/path/text/bitmap/sprite leaves into a `flighthq-surface` RGBA buffer; bit-deterministic across machines, serving as the conformance _reference_ the GPU backends are checked against and the universal no-GPU fallback.
