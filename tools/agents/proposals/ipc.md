...
tier: bronze
source:
  - tools/agents/docs/reviews/maturation/depth/ipc.md
  - tools/agents/docs/reviews/depth/ipc.md
depends_on: []
updated: 2026-06-23
---
...
solid — 72/100. The canonical three-verb host-IPC seam (`send`/`invoke`/`subscribe`) is correct, idiomatic, fully tested, and properly shaped as a swappable backend, but it is missing several primitives a mature host-IPC layer is expected to expose, most missing-by-omission rather than by design.
...
> Build `@flighthq/ipc` up to the **Bronze** tier per the Scope + Design above (the package exists — extend it). Define any new shared types in `@flighthq/types` first. Follow the CLAUDE.md conventions. Satisfy every Acceptance checkbox. Surface cross-package or design decisions rather than guessing.
