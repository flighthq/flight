---
id: textshaper-harfbuzz
title: '@flighthq/textshaper-harfbuzz'
type: new-package
target: textshaper-harfbuzz
status: proposed
tier: bronze
source:
  - tools/agents/docs/reviews/maturation/breadth/textshaper-harfbuzz.md
...
The full-glyph text-shaper backend — a HarfBuzz-class shaper (HarfBuzz-wasm on web, rustybuzz in the Rust port) that implements `TextShaperBackend` at full strength: positioned glyphs (ids, advances, offsets, clusters), GSUB/GPOS feature application, and complex-script support (Arabic/Indic/Thai/Hebrew), upgrading `shapeText` from advances-only width to real `ShapedRun`s and thereby unblocking GPU/software text and correct international typography.
