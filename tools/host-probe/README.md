...
The automatic lane is intentionally non-invasive: provider transitions, exact notification slot profiles, app/platform identity, window/screen availability, scheduling, glyph rasterization, and canvas readback. It does not prompt or deliver a notification. Clipboard mutation, OS notification permission/delivery, dialogs, shortcuts, tray interaction, haptics, geolocation permission, lifecycle effects, and updater behavior belong to explicit interactive/device lanes. Injected-facade behavior remains covered by the colocated `packages/host-*/src/*.test.ts` suites.
