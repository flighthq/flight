...
- `tools/agents/docs/reviews/depth/<name>.md` — depth review: present capabilities, gaps.
- `tools/agents/docs/reviews/maturation/depth/<name>.md` — the Bronze/Silver/Gold roadmap.
...
- cross-cutting mentions in `reviews/breadth/*` and `reviews/alignment/*`.

Synthesize a **short briefing** (a synthesis, not a dump): what the package is, what the worker just did and _claimed_ (flag unverified claims), the most important gaps the reviews name, and — the point of the session — the **open questions only the user can answer**: boundaries, North star, which roadmap items are wanted, and any design forks.
...
- **Direction on the candidate changes** — which roadmap/worker items are wanted, deferred, or rejected, and _why_.
...
- → `tools/agents/docs/packages/<name>/assessment.md`: the **Recommended** (sweep-safe) + **Backlog** of possible changes, drawn from the roadmap + worker work + the discussion. Freeze any verbal approval into **Approved** with a provenance stamp (`CONTRACT.md`). Keep design-fork / cross-package items **out** of `Recommended` — they belong in the charter's Open directions.
...
- When a fragment is fully absorbed into the cell, note it for removal — the depth review and roadmap are one-time seed, not a permanent parallel (see `index.md`).
