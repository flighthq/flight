...
grep "^  [A-Z].*Effect:" packages/effects/src/effectDefaults.ts | sed 's/:.*$//' | sed 's/^  //' | while read effect; do
  typefile="packages/types/src/${effect}.ts"
  if [ -f "$typefile" ]; then
    # Extract all field names from defaults entry
    defaults_line=$(grep "^  ${effect}:" packages/effects/src/effectDefaults.ts)
