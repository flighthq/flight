...
if (!scoped) {
  ...
  // Integration tests (child-process scripts) are not part of the unit lane. They need a dedicated
  // runner; this gate is a placeholder until one exists.
  ...
  add('opaque-values:check', 'tsx', ['scripts/check-opaque-values.ts']);
  add('double-casts:check', 'tsx', ['scripts/check-double-casts.ts']);
