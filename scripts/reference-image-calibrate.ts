 ...
 * deliberate one — see `reference-image-check.ts`'s requirement join for the same rule applied to the same gap.
 */
 ...
 * real and useful and live in `agents/render-reference-image-calibration-record.md`, because this tool did not
...
const RECORD_COMMENT =
  ...
  '`npm run reference-image:calibrate -- <rootA> <rootB> --record <path>`; every field here was read from the ' +
  'captures. The run and commit it came from are in agents/render-reference-image-calibration-record.md, which ' +
...
if (process.argv[1] !== undefined && import.meta.url.endsWith(process.argv[1].replace(/^.*?(?=scripts\/)/, ''))) {
  ...
  if (recordFlag >= 0 && recordPath === undefined) {
    console.error('reference-image-calibrate: --record needs a path to write the named-cell record to');
    process.exit(2);
  }
  const roots = recordFlag < 0 ? argv : [...argv.slice(0, recordFlag), ...argv.slice(recordFlag + 2)];
  if (roots.length < 2) {
    console.error('usage: reference-image-calibrate <run-root> <run-root> [more…] [--record <path>]');
  ...
  if (duplicate !== null) {
    console.error(`reference-image-calibrate: ${duplicate} was given more than once.`);
  ...
  if (report.agreed.length === 0 && report.disagreed.length === 0) {
    console.error('reference-image-calibrate: no cell was captured by every run — nothing was compared');
    process.exit(1);
  }
}
