...
const AGGREGATES = ['defaultScene2DCanvasRenderRegistry', 'canvasShapeCommandTable', 'enableHostWeb'] as const;
...
describe('size-only control fixture', () => {
  ...
  it('still drives the pre-render pass it is the floor for', () => {
    for (const fixture of CONTROL_FIXTURES) {
      ...
      // The background is the pass's clear, so what proves the fixture actually paints a frame is that
      // it opens a pass on a screen target.
