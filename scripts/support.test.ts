...
describe('loadBaselineCoverage', () => {
  ...
  it('changes exactly the manifest unpinned identities from ticks to dots, by name', () => {
    ...
    // Non-empty, so the equality below cannot pass vacuously on two empty sides.
    expect(unpinnedIdentities.length).toBeGreaterThan(0);
