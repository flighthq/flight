...
describe('writeFixtureTreeStamp', () => {
  it('round-trips the tag, the variant, and the packs already in the tree', () => {
    ...
    writeFixtureTreeStamp(treeDirectory, {
      packs: [{ file: 'b-full-0.1.0.tar.gz', files: 2, pack: 'b-fixtures', sha256: 'b'.repeat(64) }],
      tag: '0.1.0',
      variant: 'full',
    });
  ...
  it('records the packs in a stable order so two runs write the same stamp', () => {
    const treeDirectory = join(workspace, 'tree');
    const packs = [
      { file: 'z-full-0.1.0.tar.gz', files: 1, pack: 'z-fixtures', sha256: 'z'.replaceAll('z', 'c').repeat(64) },
      { file: 'a-full-0.1.0.tar.gz', files: 1, pack: 'a-fixtures', sha256: 'a'.repeat(64) },
    ];
