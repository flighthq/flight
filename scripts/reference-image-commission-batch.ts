...
// `reference-image-eligibility.ts`, and — only on `--write` — file the request and the coverage identity that
// together commission a batch (agents/render-reference-image-repository.md §5, §7 step 1).
//
...
//   write   → additionally file reference-image-requests/<id>.json and add the referenceImage coverage identities
...
if (subcommand !== 'report' && subcommand !== 'write') {
  console.error('usage: reference-image-commission-batch <report|write> --runs <dir,dir>');
...
// ★ A BOUND ON BATCH SIZE IS NOT A CONVENIENCE. `MAX_PENDING_DAYS` in reference-image-check.ts is 14: every cell
...
if (runs.length < 2) {
  console.error(
    'reference-image-commission-batch: --runs needs at least two capture roots; determinism is measured, not assumed',
  );
...
if (duplicateRoot !== null) {
  console.error(`reference-image-commission-batch: ${duplicateRoot} was given more than once in --runs.`);
...
//
// ★ THE READ AND THE DERIVATION BOTH LIVE IN `reference-image-calibrate.ts`, WHICH IS NOT TIDINESS. That tool
...
if (mixedHostRoot >= 0) {
  console.error(`reference-image-commission-batch: ${runs[mixedHostRoot]} contains more than one hostInstanceId.`);
...
if (mixedEnvironmentRoot >= 0) {
  console.error(
    `reference-image-commission-batch: ${runs[mixedEnvironmentRoot]} declares more than one environmentId.`,
  );
...
if (relationship.environment === 'environment-mismatch') {
  console.error('reference-image-commission-batch: the roots report different environmentId values:');
...
const scope: ReferenceImageDeterminismScope =
...
if (subcommand === 'write' && scope === 'one-host') {
  console.error('reference-image-commission-batch: both roots report the same hostInstanceId, so this measured ONE');
...
if (subcommand === 'write' && scope === 'host-identity-missing') {
  console.error('reference-image-commission-batch: the captures carry no provenance.hostInstanceId, so independence');
...
if (batch.length === 0) {
  ...
  // empty bundle, which `reference-image-commission.ts` already refuses. Refusing one step earlier keeps an empty
  // batch from looking like a successful round in the report the user reads.
  console.error('reference-image-commission-batch: nothing is eligible, so there is nothing to commission');
  process.exit(1);
}
...
if (id === undefined) {
  console.error(
    'reference-image-commission-batch: --id is required; it dates the request and bounds its pending window',
  );
  process.exit(2);
}
const reason = readOption('--reason');
if (reason === undefined) {
  console.error('reference-image-commission-batch: --reason is required; a request states why the cells should move');
  process.exit(2);
}

const selectedRoot = hosts[0];
const selectedCapture: ReferenceImageRequestCaptureIdentity | null =
...
if (selectedCapture === null) {
  console.error(
    'reference-image-commission-batch: the selected capture root has no complete host/environment identity',
  );
  console.error('  Refusing to file a request that cannot name which capture run supplied its pixels.');
  process.exit(1);
}
const targets: ReferenceImageRequestTarget[] = [];
for (const identity of batch) {
  const target = readBoundOracleRequestTarget(runs[0]!, identity, selectedCapture);
  if ('problem' in target) {
    console.error(`reference-image-commission-batch: ${identity}: ${target.problem}`);
...
if ('missing' in updated) {
  console.error(
    `reference-image-commission-batch: ${updated.missing} vanished from the coverage manifest between read and write`,
  );
  process.exit(1);
}
...
function readCaptureFacts(root: string, name: string): ReferenceImageCaptureFact[] {
  const facts: ReferenceImageCaptureFact[] = [];
...
/**
 * The determinism verdict per identity, from the same comparison `reference-image-calibrate` publishes.
 *
 ...
 * agreement.
 */
function readDeterminism(roots: readonly string[]): Map<string, ReferenceImageDeterminismVerdict> {
  const calibration = compareCalibrationRuns(roots);
  const out = new Map<string, ReferenceImageDeterminismVerdict>();
...
/**
 * Cells the locked release already supplies blessed bytes for, read from `scripts/reference-image-lock.json`.
 *
...
function readOutstanding(): Set<string> {
  const queueRoot = join(repoRoot, 'reference-image-requests');
  ...
  for (const file of readdirSync(queueRoot).filter((name) => name.endsWith('.json'))) {
    ...
    if ('problems' in parsed) {
      for (const problem of parsed.problems) console.error(`  ${problem.kind}: ${problem.detail}`);
      console.error(`reference-image-commission-batch: ${file} is not a valid request`);
      process.exit(1);
    }
...
function readParityWithholdings(root: string, name: string): Map<string, ReferenceImageParityWithholding> {
  const path = join(root, name, 'validation-report.json');
  if (!existsSync(path)) {
    console.error(
      `reference-image-commission-batch: no parity report at ${path}; run \`npm run test:functional:parity\``,
    );
    process.exit(1);
  }
  const parsed = JSON.parse(readFileSync(path, 'utf8')) as {
    result?: { checks?: ReferenceImageParityCheck[] };
  };
  const found = findParityWithholdings(parsed.result?.checks ?? [], coverage.keys());
  if ('refused' in found) {
    console.error(`reference-image-commission-batch: ${path}: ${found.refused}`);
