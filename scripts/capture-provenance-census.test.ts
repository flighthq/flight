...
describe('capture provenance census', () => {
  it('keeps exact matches as a visible control and names the cross-leg gap', () => {
    const census = censusCaptureProvenance(
      [
        column('one', { current: 'current', fingerprint: 'current', sha256: 'current' }),
        column('two', { current: 'current', fingerprint: 'old', sha256: 'current' }),
        column('three', { current: 'current', fingerprint: null, sha256: null }),
        column('four', { current: null, fingerprint: 'old', sha256: 'old' }),
      ],
      ['functional'],
    );
    ...
    const output = formatCaptureProvenanceCensus(census, 'fixture');
