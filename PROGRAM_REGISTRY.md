...
Registry-catalog contents are not part of Thread A. The caller-filled versus self-filling ownership lane remains a user decision, so this stage provides the empty caller-owned catalog mechanism without populating built-in registrations or encoding either population policy. Backend, facet, registrar import/symbol, and implementation import/symbol are factual inventory fields under either lane. Argument/source expressions and a single generated-module emission form would choose a lane, so the codegen cell stops at an emission-neutral planning kernel.
