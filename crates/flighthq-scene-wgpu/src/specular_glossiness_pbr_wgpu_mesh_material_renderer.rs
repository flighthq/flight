...
//! KHR_materials_pbrSpecularGlossiness workflow) — the WGSL mirror of
//! `specular_glossiness_pbr_gl_mesh_material_renderer`.
//!
//! Ports `@flighthq/scene-wgpu` `specularGlossinessPbrWgpuMeshMaterialRenderer.ts`.
//! There is no spec-gloss path in the shader: this renderer converts the material
//! to a [`StandardPbrMaterialProperties`] block on the CPU at bind time and drives
//! the SAME base PBR program (no extension flag), so spec-gloss assets render
//! through the one metallic-roughness uber-shader.
//!
//! CONVERSION (Khronos glTF spec-gloss → metallic-roughness). `roughness` is
//! `1 - glossiness` (glossiness is the inverse of roughness). `metallic` comes
//! from `solve_metallic(diffuseBrightness, specularBrightness)` — the quadratic
//! that recovers the metallic factor from how far the specular reflectance sits
//! above the 0.04 dielectric floor. `baseColor` lerps from the dielectric estimate
...
//! specular-glossiness map is NOT remapped to a metallic-roughness map here
//! (different packing); only the scalar conversion is applied, while diffuse/
//! emissive/normal/occlusion maps pass straight through to the standard block
//! (reserved, NOT sampled on wgpu yet — maps deferred). Colors are decoded to
//! linear on the CPU before the conversion math and re-packed to RGBA8 for the
//! standard block to decode, so nothing is double-decoded.
//!
...
use flighthq_materials::specular_glossiness_pbr_material_kind;
...
/// The built-in SpecularGlossiness forward-lit mesh-material renderer.
pub struct SpecularGlossinessPbrWgpuMeshMaterialRenderer;

impl WgpuMeshMaterialRenderer for SpecularGlossinessPbrWgpuMeshMaterialRenderer {
    fn bind(
        &self,
        ...
        let spec_gloss = material.and_then(downcast_specular_glossiness_pbr);
        let standard = spec_gloss.map(convert_specular_glossiness_to_standard);

        // No extension flag — spec-gloss renders through the base metallic-roughness
        // uber-shader after the CPU conversion.
        let key = WgpuPbrDefineKey {
            alpha_mask_enabled: spec_gloss.is_some_and(|s| s.alpha_mode == MaterialAlphaMode::Mask),
            double_sided: spec_gloss.is_some_and(|s| s.double_sided),
            ..Default::default()
        };
        ...
        let material_key =
            material.map_or_else(specular_glossiness_pbr_material_kind, |m| m.kind());
...
/// Returns the singleton SpecularGlossiness renderer instance.
pub fn specular_glossiness_pbr_wgpu_mesh_material_renderer()
-> SpecularGlossinessPbrWgpuMeshMaterialRenderer {
    SpecularGlossinessPbrWgpuMeshMaterialRenderer
}

/// Registers the built-in SpecularGlossiness renderer for the SpecularGlossiness
/// material kind on this scene runtime. Opt-in (no top-level side effect):
/// `draw_wgpu_scene` only draws SpecularGlossiness subsets once this is called.
pub fn register_specular_glossiness_pbr_wgpu_material(
    state: &mut WgpuRenderState,
    scene: &mut WgpuSceneRuntime,
) {
    register_wgpu_mesh_material_renderer(
        ...
        Box::new(SpecularGlossinessPbrWgpuMeshMaterialRenderer),
    );
}
...
/// [`SpecularGlossinessPbrMaterial`] via the `Material: Any` seam. Returns `None`
/// for any other material.
fn downcast_specular_glossiness_pbr(
    material: &dyn Material,
) -> Option<&SpecularGlossinessPbrMaterial> {
    (material as &dyn core::any::Any).downcast_ref::<SpecularGlossinessPbrMaterial>()
}

/// Builds a [`StandardPbrMaterialProperties`] block from a spec-gloss material via
/// the Khronos reference conversion. Writes a packed RGBA8 baseColor/emissive (the
/// standard block re-decodes them), so the downstream bind path stays identical to
/// a native StandardPbr material. Ports `convertSpecularGlossinessToStandard`.
...
/// metallic-roughness F0 model implies. Below the 0.04 dielectric floor the surface
/// is fully dielectric (metallic 0). Ports `solveMetallic`.
...
mod tests {
    ...
    // register_specular_glossiness_pbr_wgpu_material

    #[test]
    fn registers_the_renderer_under_the_specular_glossiness_kind() {
        ...
        assert!(
            scene
                .material_registry
                .contains_key(&specular_glossiness_pbr_material_kind())
        );
    }
