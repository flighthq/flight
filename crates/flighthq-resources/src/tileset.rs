...
use crate::texture_atlas::create_texture_atlas;
use crate::texture_atlas_region::{create_texture_atlas_region, set_texture_atlas_region};
...
///
/// Columns and rows are computed from the atlas image dimensions and tile
/// size. `build_tileset_regions` is called automatically. Returns a
...
pub fn create_tileset_from_atlas(
    ...
    let (columns, rows) = match atlas.image.as_ref() {
        Some(image) => (
            (image.width as f32 / tile_width).floor() as u32,
            (image.height as f32 / tile_height).floor() as u32,
        ),
        None => (0, 0),
    };
    let mut tileset = create_tileset(Some(atlas), tile_width, tile_height, columns, rows);
...
mod tests {
    ...
    fn build_tileset_regions_noop_when_no_atlas() {
        let mut ts = create_tileset(None, 16.0, 16.0, 0, 0);
        build_tileset_regions(&mut ts); // must not panic
    }

    #[test]
    fn create_tileset_empty() {
        let ts = create_tileset(None, 16.0, 16.0, 0, 0);
    ...
    fn create_tileset_from_atlas_builds_regions() {
        ...
        let atlas = crate::texture_atlas::create_texture_atlas_from_image_resource(img);
        let ts = create_tileset_from_atlas(atlas, 16.0, 16.0);
    ...
    fn build_tileset_regions_positions_by_column_and_row() {
        ...
        let atlas = crate::texture_atlas::create_texture_atlas_from_image_resource(img);
        let mut ts = create_tileset(Some(atlas), 32.0, 32.0, 2, 1);
    ...
    fn build_tileset_regions_advances_y_by_row() {
        ...
        let atlas = crate::texture_atlas::create_texture_atlas_from_image_resource(img);
        let mut ts = create_tileset(Some(atlas), 32.0, 32.0, 1, 2);
        build_tileset_regions(&mut ts);
        let regions = &ts.atlas.as_ref().unwrap().regions;
        assert_eq!(regions[0].y, 0.0);
    ...
    fn build_tileset_regions_reuses_existing_regions() {
        use crate::texture_atlas_region::create_texture_atlas_region;
        use flighthq_types::{AlphaType, PixelFormat};
        let img = crate::image_resource::create_image_resource(
            64,
            32,
        ...
        let mut atlas = crate::texture_atlas::create_texture_atlas_from_image_resource(img);
        atlas.regions.push(create_texture_atlas_region(None));
        atlas.regions.push(create_texture_atlas_region(None));
        let mut ts = create_tileset(Some(atlas), 32.0, 32.0, 2, 1);
        build_tileset_regions(&mut ts);
    ...
    fn create_tileset_from_atlas_zero_when_no_image() {
        ...
        let ts = create_tileset_from_atlas(atlas, 32.0, 32.0);
        assert_eq!(ts.columns, 0);
        assert_eq!(ts.rows, 0);
