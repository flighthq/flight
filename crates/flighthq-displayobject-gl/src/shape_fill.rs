...
use flighthq_render_gl::{
    GlRenderState, GlShapeFillMesh, GlShapeFillMeshCacheEntry, GlShapeFillProgram, bytemuck_f32,
    bytemuck_u32, pack_gl_ndc_matrix, viewport_dimensions,
};
...
// `GlShapeFillProgram`, `GlShapeFillMesh`, and `GlShapeFillMeshCacheEntry` are
// GPU-state slot types owned by the `flighthq-render-gl` core (embedded in
// `GlRenderStateRuntime` and freed by `destroy_gl_render_state`). This leaf crate
// tessellates into and draws from them.
...
pub fn draw_gl_shape_fill(
    ...
    for i in 0..mesh_count {
        ...
        unsafe {
            ...
            state.gl.enable_vertex_attrib_array(program.loc_position);
            state
                .gl
...
fn compile_gl_shape_fill_program(gl: &glow::Context) -> GlShapeFillProgram {
    unsafe {
        ...
        GlShapeFillProgram {
            program,
            loc_position: gl
                .get_attrib_location(program, "a_fillPosition")
                .unwrap_or(0),
