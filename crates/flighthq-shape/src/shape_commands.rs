...
use crate::shape::{ShapeArena, invalidate_shape_geometry};
...
pub fn append_shape_begin_bitmap_fill(
    ...
    let cmds = &mut arena[source].data.commands;
...
pub fn append_shape_begin_fill(arena: &mut ShapeArena, source: NodeId, color: u32, alpha: f32) {
    let cmds = &mut arena[source].data.commands;
...
pub fn append_shape_circle(arena: &mut ShapeArena, source: NodeId, x: f32, y: f32, radius: f32) {
    let cmds = &mut arena[source].data.commands;
...
pub fn append_shape_end_fill(arena: &mut ShapeArena, source: NodeId) {
    let cmds = &mut arena[source].data.commands;
...
pub fn append_shape_line_to(arena: &mut ShapeArena, source: NodeId, x: f32, y: f32) {
    let cmds = &mut arena[source].data.commands;
...
pub fn append_shape_move_to(arena: &mut ShapeArena, source: NodeId, x: f32, y: f32) {
    let cmds = &mut arena[source].data.commands;
...
mod tests {
    ...
    fn append_shape_begin_bitmap_fill_pushes_command() {
        ...
        let cmds = &arena[shape].data.commands;
