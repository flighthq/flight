//! Drop-shadow GL filter pass.
//!
//! Compositing order: shadow at offset → source (unless `filter.hide_object`).
//!
//! `scratch` must contain three render targets of the same dimensions as `dest`
//! (tint mask, blurred shadow, blur ping-pong temp). This function allocates
//! nothing itself. `knockout` is not yet supported by the GL path.

use flighthq_filters::DropShadowFilter;
use flighthq_render_gl::clear_gl_render_target;

use crate::blur_filter::apply_box_blur_filter_to_gl;
use crate::tint_shader::{apply_gl_blit_offset_pass, apply_gl_blit_pass, apply_gl_tint_pass};
use crate::{GlRenderState, GlRenderTarget};

/// Applies a drop shadow filter to `source`, writing the result to `dest`.
pub fn apply_drop_shadow_filter_to_gl(
    state: &GlRenderState,
    source: &GlRenderTarget,
    dest: &GlRenderTarget,
    scratch: &[&GlRenderTarget; 3],
    filter: &DropShadowFilter,
) {
    if filter.knockout.unwrap_or(false) {
        return;
    }

    let angle = filter.angle.unwrap_or(45.0).to_radians();
    let distance = filter.distance.unwrap_or(4.0);
    let dx = angle.cos() * distance;
    let dy = angle.sin() * distance;
    let color = filter.color.unwrap_or(0);
    let alpha = filter.alpha.unwrap_or(1.0);
    let strength = filter.strength.unwrap_or(1.0);
    let quality = filter.quality.unwrap_or(1).max(1);
    let hide_object = filter.hide_object.unwrap_or(false);

    let tint_strength = strength.min(1.0);
    let shadow_passes = (strength.floor() as u32).max(1);

    let mask = scratch[0];
    let blurred = scratch[1];
    let blur_temp = scratch[2];

    apply_gl_tint_pass(state, source, mask, color, alpha, tint_strength);
    apply_box_blur_filter_to_gl(
        state,
        mask,
        blurred,
        blur_temp,
        filter.blur_x.unwrap_or(4.0),
        filter.blur_y.unwrap_or(4.0),
        quality,
    );

    clear_gl_render_target(state, dest);
    for _ in 0..shadow_passes {
        apply_gl_blit_offset_pass(state, blurred, dest, dx, dy);
    }

    if !hide_object {
        apply_gl_blit_pass(state, source, dest);
    }
}

#[cfg(test)]
mod tests {}
