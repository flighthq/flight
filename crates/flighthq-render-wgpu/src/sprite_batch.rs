...
use crate::shader::{WgpuStencilMode, normal_wgpu_blend_state, wgpu_blend_state};
...
pub fn pack_wgpu_sprite_batch_material_instance(
    ...
    let renderer = crate::material_registry::get_wgpu_material_renderer(state, material_kind);
...
fn quad_batch_pipeline_key(
    ...
    crate::shader::wgpu_pipeline_cache_key(blend_mode, stencil_mode, format)
}
...
fn resolve_material_floats(state: &WgpuRenderState, _material_renderer_id: u64) -> u32 {
    ...
    crate::material_registry::resolve_wgpu_material_renderer(state, None)
