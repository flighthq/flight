...
- `electronHost(electron, { platform })` — builds the Electron Host from the injected API and platform fact; Shell shortcut links are present only when `platform` is `windows`.
- Opens the window through Flight's seam (`createAppWindow` + `openWindow`), then reaches the real `BrowserWindow` back out with `getElectronBrowserWindow(win)` purely to `loadFile`/`loadURL` the page. Everything else (title/size/state) would go through the Flight window API.
